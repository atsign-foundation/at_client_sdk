...
Related drafts in this directory: [`mechanisms.md`](mechanisms.md) (algorithms/protocols catalog), [`ML-KEM.md`](ML-KEM.md) (mental model for ML-KEM's role), [`dependencies.md`](dependencies.md) (native dep candidates).
...
**Dependency:** Component 12 (PQXDH) must produce the initial `root_key`. Without PQ session-init, SPQR's PQ guarantees collapse — see `ML-KEM.md` for the mental model and `components.md` §12.
