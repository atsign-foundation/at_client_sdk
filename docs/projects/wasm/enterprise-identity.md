...
1. **The enrollment lifecycle is storage-free.** Every `AtEnrollment` method takes only an
   ...
   It works in a browser with no local store.
...
| **E4** | **No new process-global state**; one client per atSign, explicitly | An IdP integration is inherently multi-identity: one service holding many users' atSigns |
