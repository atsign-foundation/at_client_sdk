...
1. **The npm package is not a Dart package.** It is a build artifact — the compiled Dart
   ...
   shipped `.js` and the `.d.ts` are generated (§8, D-23). Publishing to npm implies no
   pub package.
...
with an in-memory cache ([`decisions.md`](decisions.md) D-17), and Dart runs on the main
thread (D-19). Some operations *could* therefore return synchronously today. If any of them
...
> **Corrected 2026-08-30 (D-23).** An earlier version of this section specified a
...
Under D-17 the default V1 payload carries no such asset, which is a real payload win. It
...
| Header | Status |
| --- | --- |
| `COOP: same-origin` / `COEP: require-corp` | **Not required, and deliberately avoided.** Only a `SharedArrayBuffer`-based VFS would need them, and D-21 rejects that option. Cross-origin isolation severs `window.opener`, which would break popup OIDC flows |
