...
| T6.3 | **No unconverted `Future`, `Stream`, `List` or `Uint8List` escapes the facade.** Static check over `lib/src/js/`: every exported member returns a JS type. | Zero violations. This is the process-killer (§1.6) — a static gate, not a runtime one.                       |
| T6.4 | Every thrown Dart exception arrives as a catchable `AtError` with a stable `code`, never as the default boxed rejection.                                   | Mapped for every exception class the facade can raise; the generic message appears nowhere.                  |
| T6.5 | `subscribe`-shaped APIs deliver events and stop delivering after unsubscribe.                                                                              | Events observed, then none after cancel.                                                                     |
