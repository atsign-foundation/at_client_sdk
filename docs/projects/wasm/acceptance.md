...
**Purpose:** the tiered gate ladder **T0–T5** for the implementation-neutral
...
[`decisions.md`](decisions.md); for the thesis see [`roadmap.md`](roadmap.md).
...
- [8. Cross-cutting gates](#8-cross-cutting-gates)
- [9. The honest limit](#9-the-honest-limit)
...
the sweep progresses, package by package.
...
| ID | Gate | Done when |
|---|---|---|
| T1.1 | A program importing `package:at_client/at_client.dart` and constructing nothing compiles under `dart compile wasm`. | Exit 0. |
| T1.2 | A program that builds an `AtClient`, onboards from in-memory `.atKeys` material, and calls `put`/`get`/`delete`/`getKeys` compiles. | Exit 0. |

**What T1 proves:** that no `dart:ffi`, `dart:html`, `dart:js` or `dart:mirrors` is
reachable. For this codebase that is exactly one thing — the at_chops OpenSSL island
stays quarantined behind `at_chops_ffi.dart`. Valuable, narrow.

**What T1 does not prove:** anything at all about `dart:io` or `dart:isolate` (§1.1).
Do not cite a green T1 as evidence of WASM-readiness. Whenever T1 appears in a PR
