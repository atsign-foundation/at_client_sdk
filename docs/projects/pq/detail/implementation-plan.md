...
`AtClientPreference`'s default posture from `PqPosture.migration()` to
`PqPosture.postQuantum()` — one edit that flips all five rollout axes
...
3. ~~**The wire half, client side — none of it exists.**~~ ⚠️ **STOP — this
   ...
   - ~~**The in-use signing set** on `AtClientPreference`~~ — ✅ **BUILT
     ...
     `inUseSigningAlgorithms`, defaulted from `PqPosture`. The deprecated
