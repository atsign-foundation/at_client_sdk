...
### The legacy-posture build, as it stood when it left the live plan on 2026-09-08

All five pieces done and live-proven on four rails. It left P0 by the table's own
rule — a row leaves when it is done rather than gaining a status marker — and the
record of what each piece cost is here rather than in `git log` because several
pieces landed somewhere other than where the row predicted.

⚠️ **Two residues did NOT come with it, and neither is this row's work.**
`hashingAlgoType` is dead state on both `AuthRequest` doors — read by nothing in
the tree — and removing it is a second breaking change no ruling asked for. And
`at_onboarding_cli_test.dart`'s activation test is fixed but UNVERIFIED locally:
analyze-clean, and that pack needs its own virtualenv. ✅ It passed in CI run
34273592723, in both onboarding-CLI jobs including the proxy one this Mac cannot
run at all.

**Design DONE 2026-09-08**, rulings [135](decisions.md#135-legacy-is-the-control-arm-no-wire-write-no-subscription-no-keyfile-change-2026-09-08), [136](decisions.md#136-at_auth-stays-posture-blind-and-both-its-doors-demand-the-algorithm-2026-09-08), [137](decisions.md#137-auth_cli-has-two-roles-and-they-take-opposite-postures-2026-09-08) and [138](decisions.md#138-the-posture-ladder-moves-back-a-stage-2026-09-08); this row is the build. ✅ **⓵ DONE 2026-09-08.** ⚠️ **The row's own instruction was wrong and the measurement it asked for is what showed it.** It said not to construct or run `PqClientBootstrap`; the answer is to construct it and run it with every gate off. `_adoptEraCryptoDefault` returns at `CryptoConfig.legacy()` **before** it reaches `_pqBootstrap!.ring`, so the crypto config never needs the ring — that reading is now a measurement. But the bootstrap has **35 references**, many of them `pqBootstrap!` force-unwraps in the live packs and one `await pqBootstrap?.startupComplete` in `create_at_client_cli.dart`: a null bootstrap breaks them, and one whose `startupComplete` never completed would hang them instead. Constructing is inert — `AtClientSecretSharing.forClient` is a per-client cache, and registration and listening are steps — so the object is built, `startup()` is called, and every step returns at once. **`PqStartupGates` gains five gates** (`hydrateHeldSecrets`, `collectConveyedKeys`, `startEnvelopeListener`, `seedNamespaceKeys`, `reconcileEnrollmentSnapshot`) for a total of 13, plus `PqStartupGates.inert()`. Keyed on `configuresPqProviders` rather than on `PqPosture.legacy` by identity, so a deployment building its own provider-less posture gets the same client. ⚠️ **A rail's control had to move**: `at_client_impl_test.dart`'s legacy arm asserted `mintInUseSigningKeys` stays TRUE, with the reason *"the posture switches off exactly the three post-quantum steps, not the startup generally"* — which this ruling inverts. Both arms now assert the whole 13-gate list, one all-false and one all-true, so the control can stay green while the assertion goes red; a gate-count tripwire fails if a gate is added without a row. ⚠️ **The tripwire's FIRST mutation did not redden it** — the regex matched only `final bool name;` and the mutant carried an initialiser — so the rail was widened and re-proven against the shape that escaped. **⛔ This also corrects the catalogue audit's sequencing**: five sentences it had filed under the default flip are falsified by *this* piece, because they say a provider-less client is still conveyed nskey privates. All five corrected here — the `configuresPqProviders` dartdoc, the `legacy` constant's own, UC-B5.12's clause, `pq_posture_test.dart`'s comment, and the two pack helpers, whose fixture sets `configuresPqProviders: true` so only their *reason* was wrong and their conclusion stands. Green after: at_client 1,821, at_onboarding_cli 78. **Live-proven 2026-09-08 on all three packs**: functional **201/201**, e2e **72/72** across 21 files (both sets, bare `./runLocal.sh` on base port 26000), onboarding-CLI **21/21** (`./runLocal.sh 47000`) — the last being what exercised the retired implicit-onboard shim and the role postures. ⚠️ **The functional pack needed a second run, and its first-run reds were the finding rather than noise**: `signing_root_pull_two_enrollments_test`'s *a holder answers another enrollment and the private is filed*, and two in `nskey_self_heal_live_test` including *a client START primes what it holds*. All three built their conveying clients at `PqPosture.legacy` and relied on that client's startup to hydrate, listen and answer. **The consequence, ruled INTENDED by gkc 2026-09-08: a `legacy` enrollment can no longer act as a key HOLDER for its own atSign's other enrollments** — answering a pull means sweeping the atServer, sealing a reply and signing it, which is doing something. The fixtures moved to the pack's own `legacyPlusPqProviders`, which keeps `rsa2048` authentication so no retrofit fires and the enrollment id stays stable, which is what `PqPosture.legacy` was being used for. ⛔ **A grep heuristic over the packs for other at-risk fixtures was worthless** — it matched nearly every file, because "mentions nskey or sharing" does not discriminate. The pack is the instrument: 198 tests passed on that first run with their clients at the two legacy-era postures named above — `PqPosture.legacy` and the pack's own `legacyPlusPqProviders`, which are not one constant — and that is the real evidence that most fixtures pinned to those two do not depend on conveyance. ✅ **The legacy-server arm ran too: 3/3.** `test/pq/legacy_server_abort_test.dart` carries the `legacy-server` tag and needs a pinned pre-post-quantum atServer, which the bare runner excludes because against a post-quantum image it does not fail usefully — it stops testing anything. Run as `VIRTUALENV_IMAGE=atsigncompany/virtualenv:vip-p3.15.0 ./runLocal.sh 26000 test/pq -t legacy-server`, and the log confirms that image was pulled and used. Its CI job is `legacy_server_tests`, beside `pqe2e_tests` which runs the same directory with `-x legacy-server`. ⚠️ **The run reports 11 files and 3 tests**: `dart test` LOADS every file under the path to find the tag, so a file list is not a list of what ran — the `+3` is. ✅ **⓶ DONE 2026-09-08.** The field did not become required where it sat — it **moved**, off the sealed `AuthRequest` and onto `AtOnboardingRequest` as a required constructor argument, because every one of its six reads is `atOnboardingRequest.signingAlgoType` and the authenticate door never read it at all: `at_auth_impl.dart` resolves that algorithm from the keyfile and says so. So `AtAuthRequest` lost a settable field that could only ever be wrong silently. The compiler enumerated **17 call sites** across at_auth, at_client, at_client_flutter, at_onboarding_cli and at_functional_test; each states the algorithm it was previously inheriting, and the CLI's onboard boundary reads `atOnboardingPreference.authenticationKeyAlgorithm` — the same source `sendEnrollRequest` already uses, so it moves with the ladder rather than pinning a constant. ⚠️ **One fixture was strengthened rather than merely fixed**: `pq_native_onboard_test`'s request helper now starts at `rsa2048`, because both its tests assert the request comes back carrying `mldsa65` and a fixture starting there would have been green whether or not `makeActivationPqNative` moved it. Green after: at_auth 369, at_client 1,820, at_onboarding_cli 72, at_client_flutter 40; both CI format gates exit 0 under `dart:3.13.3`; `flutter analyze` 0 errors and the same 79 pre-existing infos it had before. ⚠️ **Found in passing and NOT done**: `hashingAlgoType` sits beside it on the base and is read by **nothing** in the tree — dead state on both doors, and removing it is a second breaking change no ruling asked for. `mintLegacyMaterial` keeps its opt-out shape. ✅ **⓷ DONE 2026-09-08.** `postureForEnroller` (legacy, returning the notice the command prints) and `postureForApprover` (pqReady, refusing `legacy` with the reason) replace the raw `postureIn` at every role site — **11 approver call sites** rewired, plus `enroll`'s `signingAlgo` and `createOnboardingService`. `postureIn` survives as the raw answer, because the two roles resolve the unnamed case in opposite directions and collapsing it would make one of them unstateable. The implicit-onboard shim is retired: a leading option now prints the command list and exits 1, with `--help` and `--version` still served. ⚠️ **A rail had to change its specification, not just its expectation**: *the CLI names a posture constant in exactly one place* forbade any `PqPosture.*` outside `postureNames`, and the ruling requires the CLI to state its own role defaults. It was **tightened rather than relaxed** — it now asserts that `auth_cli.dart` and `create_at_client_cli.dart` name **zero**, which catches a construction site restating a default wherever it sits rather than only when a total passes three; a mutant planted in `auth_cli.dart` reddened it, quoting that reason string. Six new tests; the pack is 78, up from 72. ⚠️ **The shim's own mutation proof is WEAKER than the other**: restoring the shim does not fail the assertion, it *hangs*, because the invocation then really does reach the network to onboard — which is the evidence, delivered as a timeout rather than as a red. ⚠️ **The one pack site relying on the shim is FIXED but UNVERIFIED**: `at_onboarding_cli_test.dart`'s activation test built its args starting `-a`, and now names `onboard` first. The edit is not blocked; running it is — that pack needs its own virtualenv, so this is analyze-clean and untested. A sweep of both CLI packs finds **zero** remaining `runCliCommand` sites with no command word nearby. ✅ **⓸ DONE 2026-09-08, and NOT where the row said it would go.** It is not an auth_cli change: `FileAtKeysIo._writeValidated` is the one place both write paths funnel through and the only layer holding the old document and the new one at once, so the copy is keyed on the **shape transition** — existing document has no `version`, candidate has one — rather than on a flag a caller has to remember to pass. ⚠️ **A rolling `.bak` already existed and was NOT the answer**: it is replaced by the next write, and a client's startup makes several within seconds of the one that changed the shape, so it preserves nothing an operator could come back to a day later. `<keyfile>.pre-v1` is written once and never overwritten — a second copy would be a document that is already upgraded. Announced at `shout`, which is the one level a CLI that sets `root_level = 'shout'` still shows, so a `warning` would have been invisible where it matters. ⚠️ **Two existing tests pinned the old file set** — `at_keys_io_test.dart`'s upgrade test and `legacy_at_keys_test.dart`'s, each in a passphrase and a no-passphrase variant — and both were edited in the same commit, which is the review. The second only surfaced when the whole suite ran; the single file was green. Proven by mutation: disabling the mechanism reddens both assertion arms and both pinned tests while the control — the same writer over the same fixture, differing only in whether typed material is added — stays green. ⚠️ **Residue: predicted in the wrong pack, and found by running.** This row first named the PROXY pack's `cleanup_utils.dart` as where a `.pre-v1` would survive cleanup. The live runs found **15 of them, none in the proxy pack**: 7 in at_functional_test's `test/testData/` — the only ones git did not already ignore — and 8 in at_end2end_test under `test/testData/` and `test/hive/client/`, both covered by that pack's directory ignores. The functional pack ignores `test/testData/*.atKeys.bak` for the ROLLING backup and had no entry for this one, so a run left seven untracked keyfile copies in `git status`, one `git add -A` away from being committed. Fixed by one `.gitignore` line beside the `.bak` entry. The onboarding-CLI pack needed nothing: its keyfiles live under `/test/.tmp_keys/`, ignored wholesale. Green after: at_auth 372 (up from 369), at_client 1,820, at_onboarding_cli 78. ✅ **⓹ DONE 2026-09-08.** One character of production change — `AtClientPreference`'s `this.posture = PqPosture.pqReady` becomes `PqPosture.legacy`, reverting `06c2b8b22`, which is unpublished and spike-only so nothing in the field moves. ⚠️ **The blast radius was 13 test failures and NONE of them was a compile error**, which is why the flip was made first and the suite asked to enumerate. Three were about the default and had their assertions AND reason strings rewritten — `pq_posture_test`'s bare-preference arm, `nskey_seeding_test`'s seeding arm (an assertion now inverted twice, and the note says so), and `at_client_impl_test`'s era-default arm, which gained a `pqReady` control so it is not a claim about every posture. Two more in `pq_posture_test` were the COHERENCE RULES firing correctly: an empty data signing set forces `rsa2048` authentication, so an axis-override arm that relied on the default supplying a non-empty set now names `pqReady`. The remaining eight used a bare preference as a convenience and now got an inert client; each names its stage. ⚠️ **One of those eight mattered more than the rest**: `no_atkeysio_inertness_test` asserts a client writes nothing BECAUSE it has no `AtKeysIo`, and under the new default it would have been inert for two reasons — green whether or not the missing key source was doing any work. It names `pqReady` so the absent source is the only cause. Two acceptance citations broke because renaming a test is the same loss of evidence as deleting it, and were re-pointed. The catalogue's two flip-sensitive sentences — UC-C1.6's and UC-B5.12's — are corrected here as the audit said they must be, and both record a ROUND TRIP rather than a correction: the claim they struck in August is true again. The at_client CHANGELOG's consumer warning that a default-preference client self-enrols and rewrites its `.atKeys` is corrected in the same commit — it now describes what a caller gets when it NAMES a post-quantum stage. Green after: at_client 1,821, at_onboarding_cli 78, acceptance rails 159; five packages analyze clean. **Live-proven on all four rails after the flip**: functional **201/201**, e2e **73/73**, onboarding-CLI **21/21**, and the pinned-image legacy-server arm **3/3**. ⚠️ **The packs cost only two more fixes, and both were the same shape as the unit ones — a test whose subject was one thing and whose posture had silently become a second cause.** `crypto_era_default_test` is deliberately ABOUT the default (it reads `TestUtils.sdkDefaultPosture` rather than naming a constant, so it cannot go on passing for a posture the SDK has left), so its assertions were rewritten to the new truth — the default resolves NO post-quantum providers — and it gained a `pqReady` control so the rows are not a claim about a build that dropped the providers everywhere. ⛔ **The sharper one is the e2e long-lived-atSign guard.** Its *the shipped default is refused there* row existed because a `pqReady` default would retrofit `@ce2e1..@ce2e4` permanently — the exact damage the guard was built for — and the flip removes that hazard from the DEFAULT path. The row is now two: a post-quantum posture is still refused, and the shipped default is asserted ALLOWED, so "the default is safe here" goes red if the ladder ever moves again. Its neighbour, *a throwaway atSign is not restricted*, was passing a bare preference: under a legacy default that row would have been green for a guard that refused nothing at all, so it names `pqReady` and the atSign is once more the only reason it passes. One acceptance citation was re-pointed for the renamed functional test
...
correctly, because the client can name no enrollment to update (⚠️ **corrected
2026-09-04:** this said `there is no enrollment record`; `primary` is a real
record, and what such a client lacks is any way to cite its id). Yet a verifier
...
3. ~~**The wire half, client side — none of it exists.**~~ ⚠️ **STOP — this
   ...
   - **The `_apsk` array composer and reader.** Today `publishPublicSigningKey`
     ...
     can carry (a client whose keyfile names no enrollment publishes under
     `primary`, and cannot cite an id to send `enroll:update` with — ⚠️
     **corrected 2026-09-04:** this said `primary has no enrollment record`,
     and it does have one). And the skip-if-present is gone: it **republishes on
...
Demoted from the `## TODO` P0 table of the live plan on 2026-09-08, the day it shipped (commit `93e21ef04`). The row read:

✅ **FIXED 2026-09-08, and the warning was two populations.** `AtClientImpl` launches `PqClientBootstrap.startup()` unawaited during construction; the bootstrap's listener step calls `AtClientSecretSharing.startListening`, which read `atClient.syncService` and `atClient.notificationService`; `AtClientManager.setCurrentAtSign` assigns both only after the client is built, so a bootstrap that reached the step first got `StateError('SyncService has not yet been set')` and logged the WARNING `Could not start the envelope listener` — 83 of the functional pack's 148 clients, the same 83 in CI (job 101862322409) and in two local runs of 2026-09-07. **Fix:** `startListening` no longer needs either service at the moment it runs; `PairwiseSecretSharing.attachToServices` hooks the listener onto whichever services are set, once each, and the client's two service setters call it again, so the order no longer matters, and it is best-effort inside the setters because a client must come up whether or not its listener did. **Rejected on the way:** launching the whole PQ startup from the sync-service setter, which would have sequenced the race away, because it narrows PQ startup to clients that are given a sync service — every production client is, through the manager, but the tests that build one with `AtClientImpl.create` alone would silently lose the tail, and a caller handed nothing is the shape that does not fail loudly. **The second population:** the first fix put `EnvelopeAddressing.regexForAny` (which refuses an empty address set) inside the setter path, and 53 of 132 constructed clients threw it out of `setCurrentAtSign`, failing 54 files' `setUpAll` — clients holding no key package (legacy posture), whose listener had always failed at that same line, hidden under the sync error because that read came first. They now say so at `finer` and start nothing. Unit tests in `pairwise_secret_sharing_test.dart` pin both: a listener started before the services arrive attaches when they do and delivers (a mutant that skips only the late attach reddens it alone), and a client with no key package starts nothing and throws nothing. The pack after the fix: green on 201 tests, 148 clients constructed, `grep -c 'Could not start the envelope listener'` at 0 against 83 before.

### The post-merge fix-forward, done 2026-09-07

Removed from the `## TODO` P0 table on 2026-09-08: everything it owed is done except [#2218](https://github.com/atsign-foundation/at_client_sdk/pull/2218), a docs PR against trunk that `docs/projects/wasm/implementation-plan.md` tracks itself. The row's item read ****the post-merge fix-forward on `gkc-pq-d1-spike`****, its "Blocked on" cell read *Nothing*, and its "what is owed" cell read:

⛔ **A POINTER ROW — the analysis lives in the wasm plan, not here** (gkc, 2026-09-07). `origin/trunk` merged into the spike at `9c84011df`, bringing X4, X5 and X6 and X4a item 3. **Fixed forward the same day:** unit-green everywhere, the functional pack +201, the e2e sets +19 -2 and +51, the onboarding-CLI pack +20 -1, and every remaining live failure is the #2797 row's (it fails on the pre-merge backup too). **CI ran on the spike on 2026-09-07, the first since the merge** (runs 34157060753 and 34157062505): four reds, none a product defect in what the merge or ruling 132 built — the at_client format gate on one line of `b2_retirement_test.dart` (CI formats with Dart 3.13.3; this Mac has 3.12.2, so the gate is reproduced here with `docker run dart:3.13.3`); the storage matrix carrying no `VIRTUALENV_IMAGE` and so running the published `vip`, 54 post-quantum tests red under both backends; the e2e initializer's empty nskey stand-in keyfile deriving to `primary` beside an APKAM enrollment id, so every `setUpAll` under `authType: apkam` failed pkam (fixed by the empty-document rule in ruling 132); and the proxy pack's `otp` step exceeding its 30-second budget on `createAtClient`'s startup-tail wait (one-shot commands now skip it; run 34160884416 proved it, the proxy job green at 4 of 4 — it runs only on the Linux runner). The second dispatch, run 34160882867, was green but for two timing races: `functional_tests (beta)`'s nskey ladder, green on re-run, and `end2end_test_14`'s `bypassCache`, red on two attempts and green on the third, the known intermittent one (gkc, 2026-09-07) — so that run ended green. **Still owed:** [#2218](https://github.com/atsign-foundation/at_client_sdk/pull/2218) is unreconciled with this branch's copy of the wasm docs. Everything else — what each failure was, the conveyance investigation, the fixture helpers and the traps met — is in [`docs/projects/wasm/implementation-plan.md`](../../wasm/implementation-plan.md), the X6 row and its **Post-merge fix-forward** section.
...
### The release train reading of 2026-08-31, overtaken

The `## TODO` P0 row was rewritten on 2026-09-08 to the train's live state; this is the row it replaced, whose 2026-08-31 reading was overtaken when rc2 was cut. The row's item read **[14.18](../implementation-plan.md#1418-the-remaining-d1-initial-development-sequence) **the release train****, its "Blocked on" cell read *gkc, and the first thing owed is a DECISION rather than a publish. ✅ **Overtaken 2026-09-07:** rc1 is published and this branch declares `4.0.0-rc2` with `at_client` pinning `^4.0.0-rc2` — the "cut rc2" way out below, taken. What follows is the 2026-08-31 reading, kept as history. ⚠️ **Merged is not published — and at_auth&#39;s `4.0.0-rc1` was a version collision waiting to happen.** pub.dev holds no at_auth 4.x, while `origin/trunk` AND this branch both declare `version: 4.0.0-rc1`; trunk&#39;s at_auth is 19 commits behind the spike and still carries the two-clause `_apsk` form guard the spike replaced with the BREAKING one-clause fix. Publish rc1 from trunk and the version number is spent: the spike&#39;s BREAKING change has no heading left to land under, and `at_client` pins `at_auth: ^4.0.0-rc1`, so a later at_client carve resolves the fix-less rc1 from pub.dev. **Three ways out and they are different decisions:** re-carve at_auth from this spike, cut `4.0.0-rc2`, or publish rc1 knowingly. ⛔ **No doc anticipated this** — `git grep -P 'rc2\|re-?carve' -- docs/` returns nothing against 5 files for `rc1`. Measured 2026-08-31*, and its "what is owed" cell read:

**at_auth `4.0.0-rc1` IS published (pub.dev versions list, read 2026-09-07); the spike now carries `4.0.0-rc2`, unpublished, opened by ruling 132's BREAKING removal — so the next at_auth publish is rc2, from this spike**, then carve at_client (stacked) → at_client_flutter → at_onboarding_cli. Six of the eight positions are through by merge; what is left is publishes

### at_commons 5.17.0 and at_chops 3.6.1, published

Struck from the `## TODO` P0 table on 2026-09-08: pub.dev's versions list holds at_commons `5.17.0` and at_chops `3.6.1`, and at_server `origin/trunk` (`66598e853`) pins at_commons `^5.17.0` and handles `enroll:infons` in its verb handler, so the local override the first row mentions is gone with it. The unpublished at_commons `5.18.0` now in the tree is the release-train row's. The rows read:
...
****at_chops 3.6.1**** — **Publish it.** [PR #2181](https://github.com/atsign-foundation/at_client_sdk/pull/2181) merged to trunk on 2026-08-24 and pub.dev still tops out at `3.6.0`. Independent of at_auth and of the spike

### The revocation cascade pointer row as it stood on 2026-09-08

Shrunk in the `## TODO` P0 table on 2026-09-08 to the pointer and its blocker; the ruling-121 history it carried is kept here. The row's item read ****at_server: revocation cascades to descendants****, its "Blocked on" cell read *at_server — ✅ **[PR #2781](https://github.com/atsign-foundation/at_server/pull/2781) MERGED 2026-09-01** at `45846a7b`, and `gkc-retrofit-successor-grants-and-cap` is deleted from the remote. ⚠️ **This cell said `open as of 2026-08-31` and, earlier that same day, `unpushed with no PR`** — three states in two days, which is why nothing here quotes a sha or a commit count. Re-derive: `gh pr view 2781 --repo atsign-foundation/at_server --json state,mergedAt` ⛔ **AND MERGING #2781 WILL NOT CLOSE `UC-A5.3` c2.** The cascade climbs `parentEnrollmentId` by key, but the atServer sweeps expired records (`deleteExpiredKeys()`), so a link that has expired is GONE and the chain is severed — and each successor's ttl restarts at its own write, so under any finite key-expiry posture the earlier links expire first. Reported by the at_server session 2026-08-31 and filed as [at_server#2782](https://github.com/atsign-foundation/at_server/issues/2782). Closing the clause needs ancestry that OUTLIVES the enrollment record, so that is the real blocker, not this PR*, and its "what is owed" cell read:

⛔ **A POINTER ROW, not a second home for the design** (gkc, 2026-08-31), on the same terms as the row above. What is owed is in [ruling 129](decisions.md#129-revocation-cascades-to-descendants-and-the-roster-does-the-rest-2026-08-31); the behaviour is specified by [UC-A5.3](../acceptance.md#63-uc-a53--enrollment-revocation) c2. ⚠️ **This replaces the ruling 121 row struck 2026-08-31.** [121](decisions.md#121-a-revocation-publishes-what-it-obliges-2026-08-28) is SUPERSEDED by 129 and 130, so that row was owed work that no longer exists: its client half — a durable client-written revocation record, and a client-side subtree walk over `parentEnrollmentId` — is retired, and what took its place is the atServer's own durable revocation event log, read back as a per-namespace moment through `enroll:infons`. ⚠️ **This said "the atServer-stamped `revokedAt`" until 2026-08-31** — at_server removed that field in favour of the log, so 121's record SHAPE survives and only its writer moved, whose client work is the `UC-G2.5 c5` row above

### The primary migration row as it stood on 2026-09-08

Shrunk in the `## TODO` P0 table on 2026-09-08 to what is still owed; the record of what was done, and the atServer design it points at, are kept here. The row's item read ****at_server legacy-PKAM enrollment: the client-side half of the `primary` migration****, its "Blocked on" cell read *Nothing for the four items already done. The removal and the live re-run are blocked on a VE image built from at_server `7118d2e4`; the two remaining model questions are blocked on gkc*, and its "what is owed" cell read:

⛔ **REDESIGNED at_server-side on 2026-09-03, and the earlier design in this row is withdrawn.** This row used to describe an atServer that EAGERLY created a housekeeping enrollment beside the flat key. What landed instead MIGRATES: the credential at `privatekey:at_pkam_publickey` is moved into a real enrollment with the literal id `primary` — approved, `*:rw` plus `__manage:rw`, no expiry, no parent — and the flat key is deleted, so there is one credential and one record rather than two copies that can diverge. A bare `pkam:` is then answered as `primary`, and an `enroll:request` from such a connection is a RETROFIT of `primary`: auto-approved, carrying `primary`'s grants exactly, recording `retrofitPredecessorEnrollmentId`. ⛔ **A POINTER ROW for the atServer design, not a second home for it** — it lives in that repo's `docs/enrollment-review-findings.md`, section "The rulings", on branch `gkc-legacy-pkam-retrofit-and-review-fixes` (PR #2787, **merged 2026-09-05** — this said "draft as of 2026-09-04" until 2026-09-07). ✅ **What was owed HERE is done, 2026-09-04, except the removal below.** ① The two fixtures that enrolled a key another enrollment already holds now mint a fresh keypair per request (`tests/at_functional_test/test/enrollment_test.dart`); the atServer refuses a duplicate with `AT0032` in any status. ② `tests/at_onboarding_cli_functional_tests/test/pq_pre_enrollment_retrofit_test.dart` is rewritten: the roster holds `primary` from the moment the fixture installs the credential, so it goes one record before the retrofit and two after, the successor is selected by id rather than `.single`, and the predecessor field is pinned. ③ The eight shipping comments claiming `primary` carries `no enrollment record` are corrected — the accurate statement is that such a CLIENT cannot NAME its enrollment, which is why every one of those code paths still branches as it does. ⚠️ **Still owed here:** at_auth's client-side self-approval (`enrollment_submitter.dart`, the `selfApproves` flag, the symmetric key minted only to approve itself, and the branch that fires on a `pending` answer) is now unreachable against a current atServer, which auto-approves. gkc ruled 2026-09-01 that an atServer too old to auto-approve need not be supported, so it comes out. ⚠️ **The 2026-09-04 live runs did NOT settle whether the path is dead, and their green must not be read as saying so.** All three packs passed against an image built from at_server `d9600850`, but the submitter logs that branch at `info` under the logger `EnrollmentSubmitter`, and the CLI pack sets `AtSignLogger.root_level` to `WARNING`, so the line cannot appear whether or not it ran: the logger's name is absent from all three transcripts, which is a fact about the log level and not about the code. What would settle it: re-run that one file with the level at `info` and grep the logger name, or pin it server-side on the auto-approve. ⚠️ **Also owed, and it is a DECISION rather than a fix:** a legacy client is now fully privileged server-side while this tree still models it as holding nothing. Three places behave on the old model — namespace seeding gives it only its preference namespace, the signing-root step is skipped for it, and its `_apsk` is published directly rather than through `enroll:update`. Nothing is broken; each needs gkc to say whether the client should now act as the root it is. ⚠️ **And one acceptance consequence:** `enroll:listns` now answers a legacy connection, so the reason UC-B5.1 is recorded as undrivable from `tests/at_functional_test/test/signing_root_pull_test.dart` is false; whether the row becomes provable there is for the next catalogue pass. ✅ **The client-side half landed 2026-09-07** under [ruling 132](decisions.md#132-the-keys-name-the-enrollment-and-primary-names-the-atsigns-own-credential-2026-09-07): the client derives its enrollment from its keys, carries `primary` for an ancient keyfile, and `PkamVerbBuilder` keeps that name off the wire; the record-dependent sites ask `isAtSignCredential` rather than a null check. Of the three model questions, **seeding is ruled** (gkc, 2026-09-07): a wildcard grant seeds the preference namespace, as the atSign's own credential does, and it seeded nothing before. The signing-root request and the `_apsk` route stay on the legacy path for `primary` until gkc says otherwise. The submitter's `selfApproves` now keys on the atSign's own credential, null or `primary`, exactly as it keyed on null, so nothing about that branch moved; its removal is still the item above. Live, 2026-09-07, against `at_virtual_env:local`: the functional pack +201 over 62 files, the e2e sets +21 over 10 files and +51, the onboarding-CLI pack +21 — all green, with every keyfile-only authentication in them passing no id. Reproduce with each pack's `runLocal.sh`; the e2e sets are `./runLocal.sh 26000 test/pq -x legacy-server` and `./runLocal.sh 26000 test -x pq`.

### The CI-log row as it stood on 2026-09-08

Shrunk in the `## TODO` P0 table on 2026-09-08 to the three clusters still unread; the measurement, the four fixes with their figures, and the CI outcome on `d32f34dd9` are kept here. The row's item read ****the CI logs carry hundreds of errors that are not test failures****, its "Blocked on" cell read *Nothing*, and its "what is owed" cell read:

Asked for by gkc, 2026-09-07, reading the logs of the first CI runs since the merge: green jobs and red alike are full of SEVERE and WARNING lines nobody has debugged. Measured on run 34160882867: `functional_tests (beta)` (job 101862322409) has 1,623 SEVERE, 2,569 WARNING and 182 SHOUT lines in 175,176 — 786 `HiveAtKeyValueStore get error: HiveError: Box not found` reads plus 195 more "while syncing" (a client or its sync service still reading a store that has closed), 156 + 156 `the key package advertised by enrollment <id> does not verify` and 108 `advertised a key package that is not a signed envelope` (VerbEnrollmentDirectory, three enrollment ids), 60 from AtLookup, 35 from SyncUtil; `end2end_test_14` (job 101862392793) has 67 SEVERE in 27,915 lines, 58 of them AtLookup "the connection went away before a response arrived", 9 more Hive box reads. The proxy pack's log has none. Reproduce: `gh api /repos/atsign-foundation/at_client_sdk/actions/jobs/<job-id>/logs > job.log` (the full log; `--log-failed` truncates), then `grep -c 'SEVERE\|' job.log` and `grep -o 'SEVERE\|[0-9: .-]*\|[^\|]*\|' job.log \| awk -F'\|' '{print $3}' \| sort \| uniq -c \| sort -rn`. **Local baseline, same greps, taken 2026-09-07 for comparison** (this Mac, Dart 3.12.2, `at_virtual_env:local`, every pack green): the functional pack's two runs had 401 and 518 SEVERE, 2,317 and 550 WARNING, 182 SHOUT in ~186,000 lines; CI's green stable job had 1,219 / 2,240 / 182 in 171,480, and the memory-backend matrix 538 / 1,646 / 182. By cluster, attributed to the test running when each line was logged: (1) SHOUT is 182 in every run, local or CI — one per client construction, the `disallowLegacyEncryption` line — deterministic, a level question, not a defect. (2) the WARNING swing between two green local runs is one message: `No enrollment id; using "primary"` from `AtClientSecretSharing`, 337 vs 1,967 — a per-access warning on a hot path (`ApkamSigning.enrollmentId`), which is noise however many times it fires. (3) `advertised a key package that is not a signed envelope`: 192 local, 194 CI, all from `key_package_amendment_live_test.dart`, which publishes a malformed package on purpose — noise by design, so log it once or at a lower level under a fixture. (4) `the key package advertised by enrollment <id> does not verify`: **16 local against 322 in CI** — 232 in `key_package_amendment_live_test.dart`, 60 in `pq_retrofitted_scope_test.dart` — the one cluster that differs in kind between the environments, and the lead: the same tests pass in both, so a verifier is reading an advertisement CI's slower runner has not settled yet, or `dev_env`'s atServer serves it differently. (5) Hive `Box not found`: 137 lines in 13 files locally against 637 in 39 files in CI (`nskey_mint_lock_live_test` 82, `enrollment_pq_key_exchange_live_test` 52, `copied_keyfile_test` 47) and 0 under the memory backend — **read to its cause 2026-09-07**: the pack never stopped the clients it enrolled (`EnrolledClient` had no stop; 7 of the 62 files called any stop at all), and the `tearDownAll` that `TestUtils.isolateStorage` registers closed the storage bundles under clients whose sync services were still running; the lines fall between one file's last test and the next file's `setUpAll`, which is exactly that window. 981 SEVERE and 559 WARNING lines in CI's job are this. A fixture fault, not a store fault, fixed in (iii) below — and the product half it exposed, work parked on the network at `stop()` resuming into the closed store, is fixed under (iii) too. Checked and dismissed: `HiveAtKeyValueStore` answers such a read by trying to reopen the box (`_restartHiveBox`, logging `Hive box closed. Restarting the hive box` at `info`), which could have resurrected a closed store; that line appears 0 times in every run measured, so the reopen never ran. (6) e2e: 52 SEVERE local against 64 in CI's green job, all the `AtLookup` pair "the connection went away before a response arrived" — the same in every environment, so a product logging question rather than an environment one. The local logs live only in a session's scratchpad: re-run a pack with `./runLocal.sh > run.log 2>&1` and measure it with [`tools/log_volume.py`](../tools/log_volume.py) (`tally`, `sync`, `levels`), which reproduces every figure in this row from a local or a CI log. **The whole-log volume, measured 2026-09-07 with that script** (CI job 101862322409; the local run agrees in every proportion): 86,493 records in 175,176 lines, and half of all lines are blank because at_utils's console handler prints `message \n` through `print`. INFO is 77,029 of the records (89%); one logger, `SyncService`, is 60,285 of them; seven per-item shapes are 90% of INFO — `Pulling to local:` 35,745, `RCVD: stats notification in sync` 10,830, `NotificationServiceImpl` `Received <key>` 9,242, `Received <n> from server` 3,782, `Returning serverCommitId` 3,434, `server commit id: … pending push count` 3,037, `Inside syncComplete` 2,965. The sync volume is a product of four measured factors: (a) the atServer writes a stats notification to every monitor connection on every successful change verb (`ChangeVerbHandler.process` on at_server `origin/trunk`), not only on its 15-second timer — 1,736 distinct commit ids notified out of 1,925 commits; (b) every live client on the atSign answers each one with a full round, and the round re-fetched the commit id the notification carried with a `stats:3` verb (`_isInSync` forced a fresh read, until (iv) below), so one write cost k stats round trips and k pulls for k live clients — the same commit id is logged by up to 24 receivers within milliseconds; (c) the pack's clients were never stopped (cluster 5, fixed in (iii)), so a file's later tests inherited its earlier clients: `key_package_amendment_live_test.dart` creates 13 and is 41% of the whole log, and its last test creates one client yet logs 13,225 records; (d) 152 rounds started from an empty store and account for 78,161 of the 103,112 commit-id distance pulled — the data is 1,813 distinct keys, pulled 35,745 times, 1,204 of them `__ssenv.buzz` envelopes minted during the run with a 7-day TTL. e2e differs in kind: 9,226 distinct keys in 9,923 pulls, commit ids near 840,000 — the shared e2e atSigns hold about nine thousand live keys and every fresh client pulls them once. **Four more clusters the noise hid**: (7) `Could not start the envelope listener … Bad state: SyncService has not yet been set` — 83 of 148 clients, the same 83 in CI and in two local runs: a construction-order race for the clients holding a key package, and for the 53 or so holding none a listener that could never have started, both fixed 2026-09-08 — [the detail](#the-envelope-listener-race-fixed-2026-09-08); (8) `AtSyncQueue … used before open()` — 31 in CI, 6 locally, a sync round racing the lazy queue open, runner-dependent, unread; (9) `The connection went away before a response arrived` — 16 to 19 per functional run, 58 in the e2e job, cause unread on either side; (10) the `stats:3` round trip per notification in (b), design rather than defect. **What is owed, in the order gkc queued it on 2026-09-07**: (i) ✅ **the log-level pass, built and measured 2026-09-07**: the seven per-item shapes, the per-connection lines, the client lifecycle getters and the `primary` fallback warning are `finer`; each sync round that moved data logs one `info` line, pinned by `sync_round_summary_test.dart` (a mutant that demotes the line reddens it); the SHOUT is `info`; at_utils's console handler no longer prints the blank line. Left at `info` on purpose: the per-client start lines (the five `AtEncryptionKeysLoader` lines, `Initialising Hive persistence`, the sync queue's `opened`, `monitor started`, the PQ key-package lines) and `pkam auth success` per connection — a client start is an event, and 148 starts in a pack is the pack's shape, not the logger's. The local functional pack, green after the change (201 tests), went from 187,214 lines and 78,248 records visible at `info` to 14,669 lines and 7,307 records, 2,981 of them round summaries and none reporting a round that moved nothing; `levels` in the script had predicted 7,451 from the before-log. The CI e2e job is predicted at 13,263 → 1,448 and not re-measured; (ii) ✅ the envelope-listener race, fixed 2026-09-08 — [the detail](#the-envelope-listener-race-fixed-2026-09-08); (iii) ✅ **the fixture, 2026-09-08**: the `tearDownAll` that `TestUtils.isolateStorage` registers now stops every client still in `AtClientImpl.atClientInstanceMap` before closing the bundles, which reaches every client however a file built it, and `EnrolledClient` has a `stop()` for silencing one mid-test; `atclient_remote_secondary_test`'s last test fired three lookups it never awaited, which the stop then landed on ("failed after it had already completed"), so it awaits them. Two green runs: `Box not found` SEVERE 80 → 8 and 10, the matching sync WARNINGs 16 → 8 and 10. **The remainder, read to their cause and fixed 2026-09-08 (gkc asked):** every one was work parked on a network read when `stop()` landed, resuming into the store the stop had closed — first the in-sync check's read of the pull cursor (`_isInSync` now bails after its stats fetch; `sync_stop_race_test` asserts no local read on resume, red before the fix), then the pull's cursor persist, which sits in a `finally` the stop guard's own bail-out runs (`persistPullCursor` stands down on a stopped service; a second test parks the pull fetch), then the notification watermark, written per notification in a batch after one stop check (`handleNotificationReceipt` re-checks before each write; `notification_service_test` cuts a batch with `stop()`). Each test was red before its guard and green after; the pack went 10 → 5 → 2 → 0 `Box not found` lines, green on 201 each time, with 57 to 66 rounds now logging `abandoned: the service was stopped` at `info` instead. The family's last shape lived in the e2e and onboarding-CLI packs' logs (6 and 2 lines in green runs) and not in the functional pack's: `Monitor … Could not read the last-notification watermark, so the monitor is (re)starting without one: HiveError: Box not found`, logged for the atSign the e2e initializer had just switched the singleton manager away from. **Read and fixed 2026-09-08:** the watermark closure the monitor hands to at_lookup is called after at_lookup authenticates, and on every reconnect, so a `stop()` landing during that start reached the closed store through it; the closure now stands down when the monitor's target state is no longer `listening` (`monitor_test` parks the fake muxable where the real one authenticates, lands a `stop()`, and asserts no watermark read; red before the guard). After it: the e2e pack green on 72 with 0 such lines against 6, the onboarding-CLI pack green on 21 with 0 against 2, the functional pack green on 201 with 0 in that run and 4 in the next run the same day — the guards lower the rate, and the residual (a write already in flight when `stop()` lands) has its own P2 row; (iv) ✅ **sync, 2026-09-08**: `_isInSync` forces the commit-id fetch only for an app-sourced request (`sync()` from the app, the warm start, or the periodic safety net); a system-sourced request is the stats notification itself, whose commit id promoted the cache before it was enqueued, so the round reads the cache — `sync_round_summary_test` pushes a stats notification through the mocked monitor and asserts a pull with no `StatsVerbBuilder` sent (red before the change), with the app-triggered round's fetch as the control. Clusters 4, 8 and 9 stay unread. **CI on `d32f34dd9`:** `at_libraries` green on 15 jobs, both onboarding-CLI packs included; `at_client_sdk` red — all four functional jobs on the SPP race (the P0 row below), stable also on the ladder arm (P2), `end2end_test_14` on bypassCache (P2).

### The SPP self-enrollment race, fixed 2026-09-08

Demoted from the `## TODO` P0 table on 2026-09-08, the day its own test went green: CI run 34205926815 on `be16f4db4` (the fix commit) had all four functional jobs green, where run 34200675382 on `d32f34dd9`, the commit before, had all four red on this test. The row read:

**The mechanism, read from the atServer's own log for the stable job of run 34200675382** (CI keeps it: `gh run download 34200675382 -n ve-atserver-logs-stable`): the client's sync opened its own connection and began pulling the whole history from commit `-1`, 25 a batch, 60 ms after the client started; the test ran its raw `from:`/`cram:`/`enroll:request` dance on the client's lookup connection (approved), set the client's `enrollmentId` and `atChops` to the new enrollment, closed that connection and sent the OTP `enroll:request` with `auth: false`. Every pulled entry is a local write, every local write runs `isEnrollmentAuthorizedForOperation`, and its first run after the identity switch fetched the enrollment record over the client's lookup — which reopened the closed connection with `from:`/`pkam:` as the new enrollment, then `enroll:fetch`, and the test's request rode the same connection 3 ms later and was refused as a self-enrollment (`requested {wavi: rw}, but the enrollment being replaced holds {*: rw, __manage: rw}`). **Rate:** green in 8 of 8 functional jobs on the two CI runs before this session's commits (34157060753, 34160882867), red in 4 of 4 on `d32f34dd9`; locally 1 red in 7 runs. In yesterday's green window the pull's first batch was applied after the request had been answered; on CI the dance took 500 ms where a local run takes 140 ms and no batch lands inside it. Which commit moved CI's timing is unmeasured. **Fixed in the test:** the OTP request goes out on an `AtLookupImpl` of the test's own and the client's connection is left alone, so nothing the client does can authenticate it; the pack was green on 201 with the fix, which at the local rate proves little. 

### The CI-log row, discharged 2026-09-08

Struck from the `## TODO` P0 table on 2026-09-08: all ten clusters are read, seven were fixed the same day, and what the last three left is carried by two P2 rows — the shared-atSign test residue and the in-flight store write. The row's item read ****the CI logs carry hundreds of errors that are not test failures****, its "Blocked on" cell read *Nothing*, and its "what is owed" cell read:

Asked for by gkc, 2026-09-07. Seven of the ten clusters were read and fixed on 2026-09-08 and are recorded with their figures in [the detail file](implementation-plan.md#the-ci-log-row-as-it-stood-on-2026-09-08). **The last three were read 2026-09-08 and none is a defect in what it appeared to accuse:** **(4)** `the key package advertised by enrollment <id> does not verify` — 314 lines in CI (run 34205926815, `functional_tests (stable)`) against 20 locally, and it is **two** bad advertisements re-verified on every directory read, not hundreds of failures: `verifyAdvertisedKeyPackage` logs SEVERE per rejected member on each `listForNamespace`, with no memo. Each signature-failure line names its enrollment twice, which is why a per-id count reads double. Both bad members are left by `tests/at_functional_test/test/apsk_server_side_test.dart` on the shared `@alice🛠` and never restored — see the P2 row. ⛔ **Neither is a product defect, and this row briefly carried a P0 saying one of them was** (2026-09-08, withdrawn the same day): the second is an enrollment healed mid-run whose published key package is still signed by the authentication key a first mint drops from `_apsk`, which is the state ruling 98's premise covers and which the plan already records as closed, since nothing released writes PQ material and an enrollment now owns a signing key from birth. What the reading did produce is a wording fix, below. **The CI-versus-local gap is FILE ORDER, not the runner and not `dev_env`**: that file runs 1st of 60 in CI and 32nd of 62 locally, so in CI fifty-nine files' worth of reads follow the poisoning. Reproduce: list the files in order with `grep -o '##\[group\]✅ test/[a-z0-9_]*\.dart' job.log \| sed 's/.*test\///' \| uniq \| cat -n`. **(8)** `AtSyncQueue … used before open()` — **31 in CI on 2026-09-07, 0 in CI and 1 locally on 2026-09-08**, so this session's stop-path work all but closed it; the survivor is one `Unexpected exception in sync … used before open()` in a local run and belongs to the in-flight-write row in P2. **(9)** `The connection went away before a response arrived` — 39 in CI against 40 locally, so it is **not** runner-dependent as this row assumed; 24 of 39 are `SEVERE` from `AtLookup` raised when a client is stopped with a verb in flight, the same in-flight-write family. ⚠️ **And a figure this row's fixes were credited with is worse than recorded:** `Box not found` in CI fell from **1013** (run 34160882867) to **130** (run 34205926815) rather than to zero, and 129 of the survivors follow one `PQ startup … was stopped with 10 of 12 steps`. Locally the same run shape gives 4. The P2 in-flight-write row carries it.

### What ruling 134 left owed, both done 2026-09-08
