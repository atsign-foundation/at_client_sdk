...
| Item | What is owed | Blocked on |
| ---- | ------------ | ---------- |
| **bypassCache: the publisher's atServer answers with the old value for a minute after its client reports the update pushed** | The known intermittent (`tests/at_end2end_test/test/bypasscache_test.dart`, red on two CI attempts of run 34160882867 on 2026-09-07, red 1 in 5 local e2e runs on 2026-09-08), now with a chronology that rules out the client's sync. The test's own precondition poll fails: `the publisher stores the updated value: waited 0:01:00 for "updated_value-…" and last saw "initial_value-…"`, reading the publisher's atServer with `useRemoteAtServer`. In the red run's window, at 02:16:41.297 the publisher's fresh client logged an app-sourced `sync round … pushed 1; server commit id 101` forty milliseconds after `put(updatedValue)`, a system-sourced round five seconds later saw the same commit id, and the remote read then returned the initial value for sixty seconds; the window also holds one `AtLookup … The connection went away before a response arrived`. So either the entry pushed as commit 101 was not the update, or the atServer holds it and answers `llookup` with the old value, or the push's response was lost after the atServer applied nothing and the queue still counted it pushed — the log cannot tell these apart. What is owed: on the next red, read the atServer's own log for commit 101 and the `llookup` answers (the far side's log, not the client's), and log the pushed key in the round summary at `finer` so the client side names what commit 101 was. Not caused by the 2026-09-08 sync change: pushes are app-sourced rounds, which still fetch. | Nothing |
| **the SPP self-enrollment test loses a race with its own client's background `enroll:fetch`** | An intermittent, 1 red in 6 local functional-pack runs on 2026-09-07 and 08 (`enrollment_test.dart`, `A test to verify SPP is set and enrollment request is submitted successfully`): the atServer refused the `enroll:request` with `a self-enrollment replaces its predecessor and carries its grants: requested {wavi: rw}, but the enrollment being replaced holds {*: rw, __manage: rw}`. The test takes over the client's own connection with a raw `from:` and `cram:` and sends `enroll:request` on it; the atServer auto-approves only on a CRAM connection (`isCramConnection`) and takes the self-enrollment path when the connection carries an enrollment id. **Discriminator, read from the red run's log window for that test:** a third `pkam auth success for @alice🛠`, an app-sourced sync round that moved data, and a `LocalSecondary … Enrollment Details Map` line, none of which the green runs' windows have — so between the test's `cram:` and its `enroll:request` something re-authenticated the connection with PKAM. **Hypothesis, unmeasured:** `LocalSecondary._getEnrollmentDetails` sends `enroll:fetch` with `auth: true` over that same connection, reached from the PQ startup's `reconcileEnrollmentSnapshot` step or from a pulled entry's authorization check, and the PKAM it triggers turns the CRAM connection into an enrolled one. Not caused by the 2026-09-08 fixture change: that file already stops every client per test, and its log window is identical between the runs apart from the lines above. What is owed: prove the hypothesis (log which caller re-authenticated, or count the third auth against `startupComplete`), then give the test its own connection for the CRAM dance or have it await the client's startup tail before taking the connection over. | Nothing |
...
| **`bypasscache_test` reads a stale value, and did so before X4a** | The last red on [#2208](https://github.com/atsign-foundation/at_client_sdk/pull/2208): `tests/at_end2end_test/test/bypasscache_test.dart:201` expects `updated_value-<id>` and gets `initial_value-<id>` — a read that should bypass the cache returning the pre-update value. **Pre-existing and INTERMITTENT**: it failed in the pre-X4a run (`a9f3523aa`) and again on 2026-09-06, then PASSED on the next run of the same job at `f893dab51` — so 2 failures in 3 observed runs, a rate rather than a certainty. First recorded here as failing "in every run since", which the third run falsified. It carries no auth error, so it is unrelated both to X4a and to the enrollment-id defect fixed in `6f1c7770f` (which cleared the other 17 of that job's 18 failures). ⚠️ **The test is already known to leave server state behind**: `sharing_key_test`'s setUpAll re-asserts `autoNotify=true` and says why — *"a prior CI run's bypasscache_test may have left `autoNotify=false` persisted on the server (its `finally`-based reset is unreliable on test timeouts)"* — which makes this failure potentially self-perpetuating across runs on the shared cicd atSigns. **Owed:** establish whether the stale read is client-side cache invalidation, the TTR the test sets, or that persisted `autoNotify=false`, before changing any of them | Nothing |
...
| **the nskey ladder's state-1 assertion races the in-process holder** | Found 2026-09-07 on CI's beta functional job: 1 red in 5 runs of `tests/at_functional_test/test/nskey_rollout_ladder_live_test.dart` that day (two local, CI stable, CI beta twice). State 1 expects the never-conveyed install to MISS — `privateHalf` broadcasts an ask and returns the miss for a later retry — and the holder, in the same test process, answered before the read returned, so the read succeeded with "written by the previous build". The product's contract holds; the assertion assumes the answer cannot land inside the read's own await chain, which on a slower runner it can. Make the arm deterministic: assert the miss before the holder is primed, or hold the holder's answer until the read has returned. The CI evidence is run 34160882867, `functional_tests (beta)`, attempt 1; reproduce locally by running the pack until it lands. | Nothing |
