...
⚠️ **Rows A1, A2, A3, B2, B1, B3, B5 and D2 are built (2026-08-14) — all of
ruling 99, plus ruling 98's `_apsk` rule, its stage definitions and both of its
minting sites. Still unbuilt: B4, C1, C2, D1.**
...
| B4 | `SigningKeyMinting` becomes the heal path for enrollments that predate B3, rather than the only producer. ⚠️ **A defect for B4 to fix, read in source while building B5 and not measured end to end:** that heal path always publishes the **array**. `SigningKeyMinting._publish` sends `EnrollmentUpdateRequest(signingKeys: …)` with no branch on what the entries are, and `EnrollmentUpdater` takes the `signingKeys` branch over `apskLegacy` — so a rollout-1 client healing a pre-B3 enrollment advertises a one-entry JSON array where [98.1](decisions.md#981-the-stages) requires the bare RSA string, which is the breakage rollout 1 exists to prevent. `apskValueOf` already states the rule and `EnrollmentUpdateRequest.apskLegacy` already exists for exactly this ("an enrollment that must keep the shape every deployed consumer parses"), so the fix is for `_publish` to choose between them the way `_apskFor` does. Unreachable at rollout 2, where the array is correct, and unreachable for an enrollment created after B3, which holds its key from birth | Follows B3 |
...
| 3 | **B4** | Smaller than it reads for a *new* enrollment — after B3 it already holds its signing key, so `reconcileSigningKeys` finds nothing missing and is inert by construction — but **no longer no-hazard**: B5 read the heal path in source and found it publishes a one-entry JSON array where rollout 1 requires the bare RSA string. See the B4 row in the table above |
