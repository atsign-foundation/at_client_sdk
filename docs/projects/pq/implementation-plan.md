...
| ---- | ------------ | ---------- |
| **what `legacy` posture MEANS is unsettled, and today it does post-quantum key establishment** | ⛔ **gkc wants a longer discussion before anything changes (2026-09-08)**, prompted by reading `Prepared x-wing key package for enrollment primary` in a run whose clients are all built at `PqPosture.legacy`. **What the tree does today, read rather than assumed:** `legacy` declares `keyEstablishmentAlgorithms: [xWing]`, `sealsToKeyAlgorithms: SecretSharingAlgos.keyAlgos`, `mintLegacyMaterial: true` and `configuresPqProviders: false`. So a legacy client mints an x-wing encapsulation keypair, advertises a key package and publishes its signing key, while being unable to READ post-quantum data at all. **The defence the tree gives** is on `configuresPqProviders`: the axis *withholds the providers, not the keys*, an enrollment advertises a key package in every key-exchange mode, and a legacy client still acquires conveyed nskey privates and then declines to use them — because a key package is a RECEIVE-side capability and a rollout needs receivers reachable before senders switch on. **The question is whether that is what the name should mean.** A posture called `legacy` performing post-quantum key establishment is at best surprising, and the axes are independently settable, so the name describes a bundle rather than a property. ⚠️ **One inaccuracy found while checking:** that same dartdoc credits `reconcileKeyPackage` with minting *at every client start regardless of posture*, and it does not cover the case in the log — it returns early when the credential is the atSign's own, so `primary`'s package comes from `KeyPackageRegistration.register()` on the startup's conveyed-key-collection step instead. Whatever the discussion settles, that sentence names the wrong route. | gkc — the discussion |
