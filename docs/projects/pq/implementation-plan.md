...
| **at_server: revocation cascades to descendants** | ⛔ A pointer row, not a second home for the design (gkc, 2026-08-31): what is owed is in [ruling 129](detail/decisions.md#129-revocation-cascades-to-descendants-and-the-roster-does-the-rest-2026-08-31), the behaviour is [UC-A5.3](acceptance.md#63-uc-a53--enrollment-revocation) c2. at_server [PR #2781](https://github.com/atsign-foundation/at_server/pull/2781) merged 2026-09-01 (`45846a7b`) and does not close c2: the cascade climbs `parentEnrollmentId` by key while the atServer sweeps expired records, so an expired link severs the chain — filed as [at_server#2782](https://github.com/atsign-foundation/at_server/issues/2782), open on 2026-09-08 (`gh issue view 2782 --repo atsign-foundation/at_server`). Closing the clause needs ancestry that outlives the enrollment record. The struck ruling-121 history is in [the detail file](detail/implementation-plan.md#the-revocation-cascade-pointer-row-as-it-stood-on-2026-09-08). | at_server#2782 |
...
| **the CI logs carry hundreds of errors that are not test failures** | Asked for by gkc, 2026-09-07. Of the ten clusters the measurement attributed, seven are read and fixed — the level pass, the envelope-listener race, the fixture stop-all, the stop-path guards and the redundant stats round trip, recorded with their figures in [the detail file](detail/implementation-plan.md#the-ci-log-row-as-it-stood-on-2026-09-08). **Still unread, as measured on run 34160882867:** (4) `the key package advertised by enrollment <id> does not verify`: **16 local against 322 in CI** — 232 in `key_package_amendment_live_test.dart`, 60 in `pq_retrofitted_scope_test.dart` — the one cluster that differs in kind between the environments, and the lead: the same tests pass in both, so a verifier is reading an advertisement CI's slower runner has not settled yet, or `dev_env`'s atServer serves it differently; (8) `AtSyncQueue … used before open()` — 31 in CI, 6 locally, a sync round racing the lazy queue open, runner-dependent, unread; (9) `The connection went away before a response arrived` — 16 to 19 per functional run, 58 in the e2e job, cause unread on either side. Reproduce: `gh api /repos/atsign-foundation/at_client_sdk/actions/jobs/<job-id>/logs > job.log` (the full log; `--log-failed` truncates), then `grep -c 'SEVERE\|' job.log` and `grep -o 'SEVERE\|[0-9: .-]*\|[^\|]*\|' job.log \| awk -F'\|' '{print $3}' \| sort \| uniq -c \| sort -rn`. | Nothing |
