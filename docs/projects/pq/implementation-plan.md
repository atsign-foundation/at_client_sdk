...
| **the PQ-native activation files its signing key, and nothing tests that it does** | **Found 2026-08-31 while writing [UC-G3.1](acceptance.md#uc-g31--every-door-that-creates-an-enrollment-files-the-private-half-not-just-the-public-one).** Three doors create an enrollment holding a data signing keypair, and at_auth files the private half at three separate points. Two of the three are pinned — `packages/at_auth/test/enrollment_test.dart` and `at_self_enrollment_test.dart` both carry *"the signing key is FILED, not merely advertised"*. The third, the **activation** path in `at_auth_impl.dart`, is driven by no test in any pack: the only at_auth tests naming `advertisedSigningKey` are enrolment and self-enrolment (grep the symbol across `packages/at_auth/test` — 4 hits, all in those two files). ⚠️ **`packages/at_client/test/pq_native_onboard_test.dart` looks like coverage and is not** — it asserts the activation request CARRIES the key, which is a claim about the request object. What reaches the keyfile is whatever at_auth copies out of it, and that is the untested half. The production code is correct as read on 2026-08-31; this is a coverage hole, not a defect. **Done when** a test drives `AtOnboardingRequest.advertisedSigningKey` through `AtAuthImpl`'s onboarding path and asserts `signingKeysFor(enrollmentId)` holds it afterwards, with the no-key control beside it, and UC-G3.1 cites it. | Nothing |
