...
| ---- | ------------ | ---------- |
| **`at_libraries` CI red at `9414f8bd2` — ✅ DIAGNOSED AND BOTH FIXES LANDED; what is left is whether it recurs** | ✅ **DIAGNOSED 2026-08-28, and as of the same day BOTH the product fix and the test fix have landed. Nothing here is startable work — what is left is to watch whether the signature recurs**, which would exonerate the startup tail and mean the recreator is something else. ⛔ **Two of the three measurements this row recorded on 2026-08-28 were WRONG, and they were mine.** They are struck below rather than deleted, because how they were wrong is the reusable part.<br><br>**What actually happens.** `AtOnboardingServiceImpl.onboard` begins — first statement, `at_onboarding_service_impl.dart:276` — with `AtFileUtil.ensureWritable(File(atKeysFilePath))`, which throws `AtKeysFileOverwriteException` at `at_file_util.dart:70` when the path is occupied. The failing run's **stack trace says exactly that**, and names `at_onboarding_cli_test.dart:183` — the third onboard. The expected `AtActivateException` was never reachable: the keyfile check runs **before** `isOnboarded()` at line 314.<br><br>~~1. `AtKeysFileOverwriteException` is thrown by `FileAtKeysIo.write` — a write that failed, not a pre-flight check that fired.~~ **FALSE.** Two throwers share the type and their MESSAGES differ: `FileAtKeysIo.write` says *“Tried writing …, but failed since it already exists”* and `AtFileUtil.ensureWritable` says *“Keys file already exists at …”*. The log carries the second. I identified the thrower by grepping the exception TYPE and stopped at the first hit.<br>~~2. Reaching that write means `isOnboarded()` returned false.~~ **FALSE, and it followed from 1.** The real thrower is upstream of `isOnboarded()`, so nothing about `getServerStatus()` follows. Measurement 3 — that `isOnboarded()` reads server status rather than the keyfile — is true and now irrelevant.<br><br>⛔ **So the failure needs ONE condition, not two: the keyfile existed when the third onboard started.** The original hypothesis — an unawaited startup tail recreating the file the test deleted — was right, and the re-argument replaced it with a phantom.<br><br>**The mechanism, probe-proven 2026-08-28.** `AtClientImpl._init` fires `unawaited(_pqBootstrap!.startup())` at `at_client_impl.dart:754`, immediately after the SHOUT at line 746. That tail files nskey privates through `NskeyPrivateFiling` and so through `FileAtKeysIo.update`, and **`update` recreated a keyfile deleted between its read and its write** (past tense throughout this paragraph: it is fixed further down): `_writeValidated` takes an explicit create-if-absent branch at `file_io.dart:93`. A six-line probe — write a keyfile, call `update`, delete the file inside the mutate — returned `keyfile exists = true`, with both controls positive. The keyfile lock does not help, because the test deletes without taking it; and the tail and the test share one isolate, so every `await` is an interleaving point rather than an OS-level race.<br><br>⚠️ **The timing, and a claim of mine that was an OVER-READ.** I wrote that the tail was *demonstrably* in flight at the moment of the failure. It is not demonstrated. The second `AtClientImpl (@egcovidlab🛠)` came up at `18:12:04.634`, an `EnvelopeEnrollmentConveyance` warning landed at `04.727`, and the test failed at `04.757` — but that warning is emitted by `sweepUnanchoredEnrollments`, **step 11 of the 12** in `PqClientBootstrap._steps`, so it marks the tail nearly finished rather than running. The same ordering appears in a local green run, where the tail is settled before the delete. The CI timing is consistent with the tail being live at the delete and equally consistent with it having finished.<br><br>⛔ **Measured locally 2026-08-28, and it is a NEGATIVE result: the tail is already settled before the delete, so the fix is a no-op locally and cannot be verified there.** A probe in the test printed, for both onboards, `client runtimeType=AtClientImpl`, `pqBootstrap null? false`, `tail already settled? true`, `waited 0ms`. The middle two lines are what make the zero readable: `await null` and a finished tail both wait 0 ms, and the first probe could not tell them apart.<br><br>**What still puts the tail in the frame is ELIMINATION, not timing.** Between the delete at `at_onboarding_cli_test.dart:169` and the `ensureWritable` at `at_onboarding_service_impl.dart:276` the test runs only a handful of statements — but several are `await`s, so the event loop turns, and the unawaited startup tail is the only other thing in the isolate that writes that keyfile.<br><br>**The rate, measured rather than recalled.** 21 `at_libraries` runs on this branch, 6 red, ⚠️ **every one of them manually dispatched** — the workflow's `push:` trigger is scoped to `trunk` and tags and its `pull_request:` trigger to `trunk`, so nothing on this branch runs unless somebody asks it to, and the run count is a record of when it was asked rather than of every head. It is also why pushing here fires nothing: and **exactly one carries this signature**; every log was read to its cleanup lines. Of the other five: one compile break (four files failing to load) and three `pq_native_onboard_test.dart` failures. ⚠️ **This row said “green for five consecutive runs”; it was TWELVE.**<br><br>✅ **RULED 2026-08-28 (gkc), TWICE, and the second ruling reverses the first — both are recorded because the reversal is the interesting part.** ⛔ **The first ruling was: leave the product alone and fix the test**, with *“do not make `FileAtKeysIo.update` refuse a keyfile that vanished mid-flight”* recorded here as a rejected proposal. ✅ **gkc then re-opened it and it is BUILT.** What changed in between is the measurement below: the tail is already settled locally, so the test-side wait may be guarding the wrong thing, while a refusal in `update` guards whatever the recreator turns out to be.<br><br>**What `update` does now.** It passes `requireExisting: true` to the shared writer, which throws `AtKeysSourceAbsentException` rather than creating. `flush` is unchanged and still creates — that is its job, and the distinction is the whole design: `update`'s own `read` already throws when the file is absent at the *start*, so completing by writing a file absent at the *end* contradicted the call it began. Pinned by two tests in `packages/at_auth/test/at_keys_update_test.dart` and **three mutations, each reddening one assertion and quoting it**: drop the flag and the refusal is not thrown; throw but write anyway and the not-resurrected assertion fails naming its own reason; flip the writer to refuse outright and only the `flush` arm reddens, naming `FileAtKeysIo.flush`. ⚠️ **Blast radius, enumerated:** seven production `update` call sites, all in at_client; four carry no local guard (`pq_signing_root.dart:1297`, `key_package_minting.dart:166`, `signing_key_minting.dart:339` and `:367`) — but every one of them was already exposed to this exact exception from `update`'s own `read`, so this adds an instant to an existing failure mode rather than a new one. Verified: at_auth 357 tests, at_client 1636, both exit 0; `dart analyze --fatal-warnings` exit 0 in at_auth, at_client, at_onboarding_cli and at_client_flutter; at_client's format gate exit 0.<br><br>✅ **The test fix landed 2026-08-28**: `quiesceStartupTail` in `tests/at_onboarding_cli_functional_tests/test/at_onboarding_cli_test.dart`, awaiting the client's `pqBootstrap.startupComplete` before the delete. `AtClient.ensureReachable` is deliberately not used — it waits for one namespace's advertisement, a single startup step, where what has to be quiet is every step that touches the keyfile — so the experimental `pqBootstrap` is reached with an `ignore` comment carrying that reason. Verified: the package's `dart analyze --fatal-warnings test` is back to its exact baseline (exit 0, 15 info, no new warning), and `runLocal.sh` against CI's `dev_env` image is **19 tests, exit 0**, with the third onboard now printing the `AtActivateException` it was always asserting for.<br><br>⛔ **What the fix does NOT prove, and this is the part to carry forward.** It closes the window if the tail is the recreator; it is a no-op locally, where the tail is already settled. **A green run says nothing here** — the signature occurred once in 21 runs, so any single green was always the likely outcome. The fix is justified by elimination and by costing nothing, not by a differential. If the signature recurs, the tail is exonerated and the recreator is something else entirely.<br><br>✅ **Five runs dispatched at `d9a51c0b8` on 2026-08-28 and all five were GREEN** (`33177216996`, `33177219481`, `33177222214`, `33177224869`, `33177227138`), re-derived from `gh run list` rather than read off the watcher that polled them. ⛔ **Read that for what it is worth, which is little about the signature.** At the observed rate of 1 in 21, five consecutive greens is the outcome roughly **78%** of the time even if nothing had changed — it excludes a *high* rate and says almost nothing about this one. Bounding a 1-in-21 rate to the point where a red is more likely than not takes on the order of 60 runs. What the five did buy is a regression check over the whole workflow, which is what a change to the keyfile write path warranted. | Nothing. ⛔ Not pickable work — both fixes are in and the five runs are read. This row is a candidate for `## PARKED` |
...
| [a client that exits during its startup tail abandons seeding](#a-client-that-exits-during-its-startup-tail-abandons-seeding) | ✅ **Reproduced 2026-08-27** — `seeding_tail_runs_live_test.dart` and `seeding_tail_abandoned_live_test.dart`, a two-file differential. ✅ **FIXED 2026-08-27** — the abandonment logs at `warning` naming what it skipped, and `AtClient.ensureReachable(namespace)` is the supported way to wait for reachability, on the interface rather than on `@experimental` `pqBootstrap` (gkc's ruling). <br><br>⛔ **The LOOP RATE question was taken up 2026-08-28 and it splits in two: answered for the path this row names, real for one it does not.** The premise — that a short-lived client relaunched inside the lock's cooldown is starved — was checked against the tree rather than measured as posed.<br>**Seeding is already immune.** `PublishedNskeyKeyRing.mintAndPublish` passes `ownLockIsNotContention: true`, and `MintLock._holder` is `atClient.enrollmentId ?? 'primary'` — an identity, not an instance — so it survives a restart. A relaunched client meets its own token, `_isOwnLock` says so, and it proceeds.<br>⛔ **The SIGNING ROOT is not.** `PqSigningRoot`'s mint (`pq_signing_root.dart:337`) passes no flag, so `MintLock._take` returns false without ever reading whose lock it is. A client that takes `pqSigningRootMintLockKey`, exits before publishing and restarts inside the root lock's ttl loses to its own token, and keeps losing for the rest of it with nothing in flight anywhere. The documented consolation — pull the private from another enrollment — has nobody to ask when this is the atSign's first or only enrollment.<br><br>✅ **Proven, not read**: `packages/at_client/test/mint_lock_self_contention_test.dart` — three arms over one fake atServer where the **flag is the only thing that varies**. Refused by default and it never even asks whose lock it is; proceeds when the caller opts in, on the identical refusal; and still loses to a *different* enrollment when it does opt in, which is the control that keeps the interlock honest. A mutation removing the flag check reddens the first arm alone, quoting its own reason.<br><br>✅ **Three misleading messages fixed in the same commit, because each one asserts the half that is false.** `MintLock`'s loser log said *“Another enrollment holds …”*, `PqSigningRoot`'s said *“another of this atSign's enrollments holds the mint lock”*, and `pq_signing_root_test.dart`'s `reason:` said *“another of this atSign's enrollments is minting; this one waits to be given the root”*. Where the caller did not opt in **nothing read the lock's value**, so none of them could know — and in the self-contention case they send a reader looking for a process that does not exist. `MintLock.withLock`'s dartdoc now says so too.<br><br>✅ **RULED 2026-08-28 (gkc): shorten the root lock's ttl**, on the grounds that *“there will never be a normal situation where two minters are both trying to mint the root signing key at the same time”* — so the cooldown is being paid for a race that does not occur, while the starvation it causes is real. **`signingRootMintLockTtl = Duration(seconds: 15)`** now sits beside `mintLockTtl`, which stays at two minutes for the nskey lock because that path opts in and never waits it out.<br>**The floor was measured, not guessed:** the ttl is also the winner's own budget — it carries the matching `MintLease` and abandons rather than publishing once spent — and a full root mint is **25ms** with the network mocked (`pq_signing_root_test.dart`, three runs: 25/24/24), so fifteen seconds leaves room for several round trips on a slow link and a far slower CPU. An overrun costs a retry at the next start, not damage.<br>⚠️ **The network term in that floor is estimated, not measured** — the 25ms is CPU with the wire mocked, and a real mint also pays the take, the publish and the reads before them as separate round trips. **gkc pointed out 2026-08-28 that [a sequential, abort-on-failure `batch`](#p1--must-do-before-d1-closes) attacks exactly that term**: collapsing those round trips into one would leave the floor dominated by the measured 25ms, and the ttl — and so the starvation window — could go far below fifteen seconds. It is the better fix for a second reason: *take the lock, and publish only if that succeeded* IS the interlock, and as a batch it becomes one atomic step instead of two round trips with a client-side lease check between them.<br>⚠️ **The residual, stated rather than buried:** shortening the ttl shortens the anti-double-mint window by the same amount. Two enrollments that both start while no root is published, more than fifteen seconds apart, could now both mint where before they could not — which needs a mint to take longer than the lease, i.e. a broken link rather than a slow one.<br>⛔ **The alternative was NOT taken:** opting the root path into `ownLockIsNotContention`, which would need `_mintUnderLock` to re-read inside the lock the way `_mintUnlessPublished` does. It remains the way to make the window zero rather than short.<br><br>⚠️ **What WAS owed, before the ruling above.** Opting the root path in would need `_mintUnderLock` to re-read inside the lock the way `_mintUnlessPublished` does, since `ownLockIsNotContention` is only safe for an idempotent critical section. The alternatives are a shorter ttl for the root lock, or releasing it on a clean exit — which the nskey side deliberately does not do, and [the lock's own dartdoc](../../../packages/at_client/lib/src/crypto/nskey/mint_lock.dart) records why deleting cost something real<br><br>⚠️ **What is left is the LOOP RATE only**: whether a short-lived client relaunched repeatedly could be starved by the lock's cooldown. Both of failure mode 2's mechanisms were **measured 2026-08-27** (`nskey_mint_lock_live_test.dart`); do not rebuild them. A short-lived client at a seeding posture takes the mint interlock and dies before publishing, so the atSign has no namespace key and no peer can seal to it — it sends post-quantum and cannot receive. Confirmed live in both directions on 2026-08-26. ⛔ **Nothing tells the caller**, and the only symptom is at the FAR end, where a different atSign reports the wrong party as unseeded | Nothing |
...
| **the add conveys and signs without an assertion over either** | **The mint and the `add` landed 2026-08-28**; what is left is two clauses no assertion reaches. **UC-G2.6 c4** — only the *newly minted* private is conveyed: `NskeySeeding._addMissing` conveys exactly the kids absent from the generation as it stood, and nothing asserts that it sends neither more nor fewer. **UC-G2.6 c6** — the added document is re-signed by the **adding** enrollment, which need not be the one that minted the generation: `_prepareMint` signs through this client's own signer, so it holds, and no test reads the signer off the republished envelope. Both want the same fixture — a generation minted by one enrollment and added to by another — which no unit test builds today, and which the live pack can. ⛔ **UC-G2.11's ladder clauses (c1, c2) are a separate gap**: they are about two installs of one atSign reading each other across the add, which is a live shape rather than a unit one | Nothing |
