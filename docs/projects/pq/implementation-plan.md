...
here and now, and it is G4.** ⚠️ This read "**TWO** … they are G3 and G4" when
it was written on 2026-08-25; G3 was diagnosed, ruled, fixed and pack-proven the
same day. A cold read that morning got as far as the list and then could not say
what to do, because the ranked head was blocked on something a session cannot
do. That is the failure this list exists to prevent, so it is stated rather than
left to be re-derived:

| gate | startable in this repo, by a session? |
| ---- | ------------------------------------- |
| G0, G2, G3, G5 | **No** — discharged |
...
| G8 | **No** — built and measured the day it was raised |

⚠️ **`[RECOMMENDED]` moved twice on 2026-08-25, G0 → G3 → G4** — first when
at_server merged the G0 fix, then when G3 was diagnosed, ruled, fixed and proven
by two full functional pack runs the same day. **G4 is now the only gate a
session can start on.** This table's first row read "G0 `[RECOMMENDED]` — **No.** Diagnosed,
...
| **a receiver drops a pqActive notification because its own nskey request goes unanswered** | ⛔ **DIAGNOSED 2026-08-25 at `38c51f67e`. It is a client defect — not the atServer, not the monitor, and not this machine's Docker.** ⚠️ **This row read "a pqActive notification vanished in one functional pack run", called it "ONE red against two greens … a rate observation, not a diagnosis, and deliberately not chased", and concluded that "the **pqActive sender's** notification specifically vanished, twice, with the atServer reporting both delivered". All three are falsified: it is not one run, nothing vanished, and the notification reached the receiving client, which then threw it away.**<br><br>**The rate: 3 red of 5**, five successive local runs of the functional pack, one machine, same code, same image (`at_virtual_env:g0fixed`). Always the same test — `pq_posture_grid_test.dart`, *"a notification crosses atSigns and its value decrypts, for every sender posture"*. ⚠️ **The durations are bimodal and identify a red on their own**: reds 459 / 460 / 462 s, greens 313 / 313 s. The ~147 s gap is the test's own 60 s + 90 s waits for its two sends, so a pack run near 7½ minutes has already failed. **Same arm as the earlier red**: the only non-docs commit between `ecf1082de` and `38c51f67e` is `chore(at_auth): dart format`, and stripping whitespace from all 18 files gives byte-identical hashes on both sides (positive control: this plan file does differ).<br><br>**What happens.** bob's atServer writes the pqActive notification to a **superset** of the sockets that received `s-legacy` and `s-pqready` — 9 connections against 6 in run 1 — so nothing is lost in transit or in the monitor. at_client receives it, cannot decrypt it, and **parks** it (`NotificationServiceImpl._park`, `packages/at_client/lib/src/service/notification_service_impl.dart`): *"Parked notification …s-pqactive…: waiting for the nskey private for @bob🛠:&lt;ns&gt; generation &lt;kid&gt;"*. Two minutes later `_evictParkedOverBounds` drops it, correctly at `warning`.<br><br>**Why the key never comes, and this predicts all five runs.** The grid's **r-pqActive** cell asks for that namespace's nskey private. In all 3 reds it **never files an answer**; in both greens it files one 0.1–1.5 s after asking. **r-legacy** files one in **all five** runs and asks in **none**. So a conveyance reaches one enrollment of `@bob🛠` and not another — the `(owner, id)` shape, live. Run 1, in full: r-pqActive (`cafcdfe3`) asked at 14:45:04.413; r-legacy (`5a73eb44`) filed generation `97bc40714f0a09ab` at 14:45:05.239; the notification parked at 14:45:06.378 and was dropped at 14:47:37.716.<br><br>⛔ **Two red herrings, recorded so nobody re-derives them.** (a) `WARNING … Not delivering notification … to enrollment 3c79f30a… — not authorized for that key` looks like the cause and is the grid **working**: `3c79f30a` is `r-unready`, the deliberately restricted cell. (b) Grepping a log for `s-pqActive` matches enrollment **device names**, not notifications, and the wire key is lowercased by the test (`slug(name).toLowerCase()`), so it is `s-pqactive`. A positive control drawn from `s-legacy` is all-lowercase and never exercises the casing branch — both greps returned a confident zero for the wrong reason.<br><br>⚠️ **CI being green on this test is not a contradiction.** Run `32854365838`'s own `acceptance-report.json` records this exact test as `success` on the stable leg, against `atsigncompany/virtualenv:dev_env`. The mechanism is a client-side race, so a different machine and image can sit on the other side of it — and that is also the cheapest next differential: the same five runs against `dev_env` locally would separate "image" from "machine timing". | Nobody yet. **What is owed:** read the nskey conveyance path and settle whether a holder conveys once per **requesting enrollment** or broadcasts once for the atSign; then either address the reply to the requester's enrollment id, or have the requester re-ask on a timer instead of waiting out `parkTtl`. ⚠️ Separately and **not** what these reds measure: `_park` registers for *future* filings and never re-checks whether the generation is already held, so a key filed between the decrypt failure and the park would also be missed. **Re-derive the rate**, never quote it: five runs of `runLocal.sh` with `VIRTUALENV_IMAGE=at_virtual_env:g0fixed`, then per run compare who asked against who filed — `grep -E -e "Asked other enrollments for the nskey" -e "Filed the nskey private" <run-log>` and read the `PqClientBootstrap (@bob🛠, <id>)` prefix on each hit, against the `##GRID## up:` lines that map every cell to its `runningAs` id |
...
cd packages/at_client         && dart test --concurrency=1                 # 1559
...
cd packages/at_onboarding_cli && dart analyze lib test                     # exit 0, 207 info
...
VIRTUALENV_IMAGE=<a-ref-you-named> bash tests/at_functional_test/runLocal.sh              # 183 pass, 2 skipped
#   ⚠️ 183, not 181: the no-commit work added two live tests. ⛔ And this row is
#   the RED one — it passed twice at 28d053ce3 and failed once at ecf1082de on
#   the same code. Run it twice before believing either colour, and read the
#   failing test's name rather than the count.
...
# ✅ ALL of the above were re-measured together on 2026-08-25 at `ecf1082de`,
# against `at_virtual_env:g0fixed` — a G0-FIXED atServer, unlike every earlier
# measurement in this file. ⚠️ The block previously read "ALL SIXTEEN … at
# `bef991985`, against at_server `a37e3e3b`"; three rows were added since, so
# do not quote a count of rows either.
#
# ⛔ ONE OF THEM WAS RED, and averaging it away would be the whole problem: the
# FUNCTIONAL pack failed once, on `pq_posture_grid_test.dart`, having passed
# twice on the same code earlier the same day. Every other figure here is exit 0.
# ⚠️ This read "That is 1 of 3 and it is written up in its own `## TODO` row with
# everything already ruled out". The rate is now 3 of 5 — five successive runs at
# `38c51f67e` on this same image, the code being identical bar a whitespace-only
# format commit — and the cause is DIAGNOSED rather than ruled out: the receiving
# client parks the notification for an nskey private that is conveyed to a
# sibling enrollment of the same atSign and never to the one that asked. The
# candidates that row listed were genuinely excluded; the cause was not among
# them. See the `## TODO` row for the evidence and the re-derivation.
#
