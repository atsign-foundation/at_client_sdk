...
3. ~~**The wire half, client side — none of it exists.**~~ ⚠️ **STOP — this
   ...
   `:56`. **Only the in-use signing set and mint-on-demand (steps 17–18) are
   genuinely still owed from this item.** The original text is kept below
...
| 17 | The in-use signing set on `AtClientPreference`. **Spec: [`decisions.md` 91.3](decisions.md#913-the-rulings) ruling 16** — app-settable on the preference, defaulted by `ReleasePosture`, and SDK releases move that default. ⚠️ Ruling 16 does not say what the 3.x and 4.0 defaults ARE, whether the set is a `List` or a `Set`, or whether it is final-at-construction like `disallowLegacyEncryption`; those are open. Today the preference carries only the single deprecated `signingAlgoType = SigningAlgoType.rsa2048` (`at_client_preference.dart:206`) |
