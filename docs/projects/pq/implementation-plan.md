...
|---|---|---|---|
| 1 | **14.22 rows 5, 6, 7** — the signing root becomes an ordinary, rotatable signing key | [14.22](#1422-making-the-signing-root-rotatable--decisions-101) | **Rows 1, 2, 3 and 4 landed 2026-08-15**, plus a mint-race fix an adversarial review turned up. Row 1 was the reader, the only D1 item with a deadline of its own — due before the GA minor. **Row 5 is the next action**: a root link gains `kid`, `_checkRootLink` selects by it and falls back to trying each advertised root, and signing selects the active root. Row 3 already built the "which key signs" half, so row 5 is the link's half of the same question |
