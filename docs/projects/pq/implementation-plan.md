...
| **the add conveys and signs without an assertion over either** | **The mint and the `add` landed 2026-08-28**; what is left is two clauses no assertion reaches. **UC-G2.6 c4** — only the *newly minted* private is conveyed: `NskeySeeding._addMissing` conveys exactly the kids absent from the generation as it stood, and nothing asserts that it sends neither more nor fewer. **UC-G2.6 c6** — the added document is re-signed by the **adding** enrollment, which need not be the one that minted the generation: `_prepareMint` signs through this client's own signer, so it holds, and no test reads the signer off the republished envelope. Both want the same fixture — a generation minted by one enrollment and added to by another — which no unit test builds today, and which the live pack can. ⚠️ **They are not the same KIND of assertion, though, and c4 is the weaker one live** (gkc ruled 2026-08-28 to keep all four in the live file anyway). c6 is a property of the published document — fetch the record, read `SignedEnvelope.signerEnrollmentId`, compare. **c4 is an ABSENCE**: *neither more nor fewer*, i.e. no second conveyance for a kid that was already there. An absence is a claim about the enumeration that produced it, so the live arm must **prove its enumeration non-empty before asserting the zero** — it has to find A's original conveyance and B's new one, and only then may "no second conveyance for the original kid" mean anything. A call count over `_convey` in a unit fixture measures the same thing directly and was the alternative considered. ✅ **Both clauses VERIFIED IN PRODUCTION 2026-08-28, so what is owed really is only the assertion** — this row asserted that and it is now checked rather than inherited. **c4:** `NskeySeeding._addMissing` takes `before` as the set of kids in the generation as it stood, then skips any key whose kid is in it, so only newly minted privates are conveyed. **c6:** `add()` prepares through `_prepareMint(..., retaining: current.keys)`, which signs with `_signer = AtClientEnvelopeSigner(_atClient)` — the **adding** client's own.<br><br>✅ **The live fixture is specified, and the two facts it depends on are measured.** (1) `keyEstablishmentAlgorithms` is threadable per live client — `test_utils.dart:23` says it must be threaded and `key_package_amendment_live_test.dart` does it. (2) Run-unique namespaces are the pack's own idiom (`'seed${DateTime.now().microsecondsSinceEpoch}'`). **The shape:** on `firstAtSign` with a run-unique namespace — *not* a dedicated atSign, because the pack has only `firstAtSign` and `secondAtSign` and `signing_root_pull_two_enrollments_test.dart` already claims the second for rooting, so isolation comes from the namespace instead. Enrollment **A** at `keyEstablishmentAlgorithms: [xWing]` seeds a one-entry generation; enrollment **B** at `[xWing, mlKem1024]` starts, finds its algorithm missing and adds it. Then c4 is *the xWing private was not re-conveyed*, c6 is *the republished envelope's signer is B*, and the ladder reads across the add. ⚠️ **Both enrollments need run-unique `(appName, deviceName)`** — an approved pair collides on the second run — and the file must be **re-run before it is believed**.<br><br>✅ **THE FIXTURE'S SHAPE IS SETTLED 2026-08-28, after five live runs, and the blocker was not what any of the earlier guesses said.** In order, each found by running rather than reading:<br>1. A `pqReady` client **seeds from its own unawaited startup tail**, so an explicit `seed()` and a *“nothing is published yet”* precondition are both wrong.<br>2. `enrolAndAuthenticate` defaults to `signingAlgo: rsa2048`, so a `pqReady` client **self-retrofits** and signs under a different enrollment id than the helper returns. **Answers the question this row used to ask**: the enrollments do NOT need to start at legacy for the retrofit's sake — pass `mldsa65`.<br>3. ⛔ **And the real blocker: the mint lock's COOLDOWN.** The minter's tail takes `_nskeylock.<ns>` at the production `mintLockTtl` (**two minutes**) and nothing releases it but expiry, so the adder — a *different* enrollment, which `ownLockIsNotContention` therefore does not help — loses the election and logs *“Did not add ml-kem-1024 … this time; the next start asks again”*. That is **UC-G2.6 c3 working as specified**, not a defect.<br><br>**So the fixture must follow `nskey_rotation_live_test.dart`'s pattern**, which documents this exact cooldown: enrol both at **`PqPosture.legacy`** so no tail seeds or takes the lock, then drive the work explicitly through rings built with `lockTtl: shortLockTtl`. ⚠️ **Drive `NskeySeeding.seedNamespace`, not `ring.add()`** — c4 is about which privates `_addMissing` conveys, and calling the ring directly skips it entirely. ⚠️ The cost of `legacy` is that the wired tail is no longer what runs, which is the same trade the rotation test makes and should be said out loud in the file.<br><br>~~ **OPEN QUESTION, raised by gkc 2026-08-28 ANSWERED ABOVE: they must, but for the mint lock rather than the retrofit.~~** `enrolAndAuthenticate` defaults `signingAlgo: SigningAlgoType.rsa2048`, so a `pqReady` client built on it self-retrofits at startup and comes up on a **different enrollment id** from the one the helper returned — which is what made the first draft of this test fail on its own control. Passing `signingAlgo: mldsa65` would make each enrollment PQ-native from the start, no retrofit, one id. The clauses are about the **add**, not about retrofit, so the retrofit looks like incidental complexity the fixture inherited from a default rather than chose. ⚠️ Against that: an enrollment that never retrofits may not exercise the same conveyance and signing paths a real deployment's does. Settle it before writing the assertions, not after.<br><br>⛔ **UC-G2.11's ladder clauses (c1, c2) are a separate gap**: they are about two installs of one atSign reading each other across the add, which is a live shape rather than a unit one | Nothing |
