...
| **at_commons 5.17.0** | **Publish it, and note who is waiting.** It carries the `enroll:infons` verb token added 2026-08-31 for [UC-G2.5 c5](acceptance.md#175-uc-g25--an-nskey-rotation-mints-fresh-material-and-carries-nothing-forward). ⛔ **at_server cannot delete its local syntax override until this publishes** — it inserts `infons` into at_commons' pattern rather than copying it, and carries a tripwire test that reddens the day a published at_commons defines the token, so the removal does not depend on anyone remembering. Its pins are caret (`^5.15.0`), so this needs a lock refresh and a floor raise rather than a constraint change. ⚠️ **5.16.0 is already published** — checked on pub.dev, not inferred from the in-tree CHANGELOG heading, which is how the version was nearly got wrong | gkc — publishing is his act, as row `at_chops 3.6.1` already said. ⚠️ **This cell said `Nothing` until 2026-08-31**, so two P0 rows described the same act with different blockers and a session could read this one as pickable |
