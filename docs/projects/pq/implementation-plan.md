...
| **the P0 band has a tie its own rule cannot break** | **A ruling from gkc, and it is his rule to change.** `## TODO` says "pick one of the P0s" and, separately, "**Nothing ranks within a bucket**". A cold read found several P0 rows sharing `Blocked on: Nothing` and reported plainly that the stated rule does **not** select between them; it picked by document order, "a convention the document explicitly disclaims". ⚠️ **This named a count and two rows, and both were stale within the hour** — another row landed above them the same evening. ⛔ **Do not write the number here: count the bucket.** So two cold sessions pick differently and neither is wrong. ⛔ **Not to be fixed by re-introducing a `[RECOMMENDED]` marker** — that is the one fact with two homes the rule was written to kill | gkc's ruling |
