...
|**`ApkamSigning` breaks NoPorts, and the break is already in the tree**|gkc, 2026-09-11: the mixin is heavily used by NoPorts and almost nowhere else. Measured against **published at_client 3.14.0**, which is what that consumer compiles against: `publicSigningKey` went `String` to `Future<String>` (`6c8bd8919`, 2026-08-13) and `privateSigningKey` was removed (`c7e06efd0`, 2026-08-03) — both in the post-quantum work, neither in the deprecation pass. `enrollmentId`, `publicSigningKeyUri` and `publishPublicSigningKey()` still compile, so the break is those two members and not the mixin. Decide between restoring a synchronous accessor beside the future, and coordinating the change with NoPorts before at_client publishes. ⚠️ Whatever is decided, the objective gkc states is that an application reaches post-quantum **without editing a line**, so this is a release-train question rather than a tidy-up. The instrument that measured it is in [the deprecation plan](../deprecations/plan.md): compile a published package's own example app against the tree.|gkc's ruling|
