...
| **`at_end2end_test` has not been run since at_auth's surface changed** (2026-08-04). `at_auth` gained `EnrollmentKeyExchangeMode`, `apkamSymmetricKeyResolver` and `approvedWithMintedKey`, and `AtEnrollmentResponse`/`EnrollmentRequestDecision` grew fields. The e2e pack is a **separate package**, so a break there is invisible to at_client's own `dart analyze`/`dart test` and to the functional pack — all of which are green. Run it before drawing any conclusion from that | `at_end2end_test` |
| **`NskeyPrivateFiling.start` is an arrival hook nothing calls.** Found 2026-08-04 while building the root-private equivalent. `file()` exists and works, but the `receivedSecrets` subscription that would drive it has no production caller — so an nskey private conveyed to an enrollment reaches its secret store and never its `AtKeys`, and is lost on restart. The root private deliberately avoided the same shape: `PqSigningRoot.filePendingPrivate` is a store check run at client start, with no lifecycle to own. The nskey path wants the same treatment | `at_client` |
