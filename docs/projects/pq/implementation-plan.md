...
| **UC-A2.1 is not met, though SS-2 reads complete.** It claims *nothing in the conveyance path is RSA-wrapped*. The key package and the secret conveyance landed; the **reversed approval direction did not** — the approver still does not encapsulate `apkamSymmetricKey` to the enrollee's key package, so that wrap remains RSA. It was an SS-2 deliverable and is the thing A2.1 actually tests; a stale blocker is how it stayed invisible | **SS-2** residual |
