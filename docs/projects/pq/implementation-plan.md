...
unqualified until 2026-08-28, when a cold read counted four ✅s in this table and
...
| **the legacy bypass window has no fixed end** | **A decision from gkc**, then either a change or a written acceptance. Found while correcting UC-B1.1 c3 and UC-B2.2 c1 on 2026-08-27, and read from at_server `origin/trunk` rather than a sibling checkout: `_capEnrollmentExpiry` writes a **full** grace period from now on **every** self-enrollment, mins only against the record's own remaining lifetime, and never against the cap a previous sibling wrote. **So each sibling clone that retrofits extends the window in which the un-upgraded legacy credential still authenticates**, and a deployment with laggard devices keeps it open as long as they keep arriving. ⚠️ **The behaviour is deliberate and its reason is good** — a deadline fixed by the first sibling's upgrade would strand every laggard whose next run fell outside it, and the dartdoc says so. What was wrong was the catalogue, which described a fixed deadline; that is corrected. What is not settled is whether an unbounded bypass window is acceptable, or wants a ceiling. ⛔ **Not a client change** — the cap is the atServer's | gkc's decision |
...
| **no rail resolves a cross-file `#anchor`** | **A rail, and it would guard a property that currently HOLDS rather than fix a break.** Found by a cold read 2026-08-28, whose manual pass was the first time this doc set's cross-file anchors had ever been resolved — every link in both live docs resolved. `docs_structure_test.dart` checks that no *linked* heading slug is duplicated, that the ledger index and its bodies correspond, and that the catalogue's counts agree; it never opens a link's target. The edit-time hook catches a bad anchor in the file being written, so the hole is **a heading renamed in one file and linked from another** — which happened on 2026-08-27 (`Two clauses pinned…` → `Clauses pinned…`) and was caught only because the same session moved the pointer. ⚠️ Scripted doc edits bypass the hook entirely | Nothing |
| **the P0 band has a tie its own rule cannot break** | **A ruling from gkc, and it is his rule to change.** `## TODO` says "pick one of the P0s" and, separately, "**Nothing ranks within a bucket**". A cold read on 2026-08-28 found two P0 rows both `Blocked on: Nothing` — the clause burn-down and the seeding-tail loop rate — and reported plainly that the stated rule does **not** select between them; it picked by document order, "a convention the document explicitly disclaims". So two cold sessions pick differently and neither is wrong. ⛔ **Not to be fixed by re-introducing a `[RECOMMENDED]` marker** — that is the one fact with two homes the rule was written to kill | gkc's ruling |
...
| Draft | What it would assert |
| ----- | -------------------- |
| **a reader accepts more entries than it understands** | A build that implements one algorithm must *use* the entry it knows in a two-entry advertisement, never refuse the document. This is what makes an ADD safe to publish before any reader is upgraded, and it is the load-bearing half of the whole design |
| **an ADD moves nothing peers already address** | The existing entry keeps its `kid` and stays `active`; anything already sealed or signed against it still opens or verifies. Proven for key packages; unproven for `_apsk`; impossible for nskeys |
| **a sender picks from the intersection under its own fixed order** | No negotiation on the wire: the sender walks its own strongest-first list across the advertiser's **authenticated** offer. An attacker can neither add a weak entry nor strip a strong one |
| **no shared entry is a refusal, never a guess** | Sealing or signing under something the far side did not offer produces a record it cannot use, and the failure would surface on their side naming nobody |
| **a retired entry stops being OFFERED but keeps opening history** | The removal half of agility. An entry withdrawn for new operations must still verify and decrypt what it produced, or an upgrade destroys data |
...
missed; and two were the self-copy, an AtCollection behaviour these rows do not
