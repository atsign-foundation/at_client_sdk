...
1. **`UC-G1.15` threw `FormatException: Unexpected character` at `pq_posture_grid_test.dart:809`**, jsonDecoding `JEZc2KNLnoYj08i7INnsqN4DdNaipfnNWmM3B6n9Jlc=` — 44 base64 characters, i.e. a 32-byte value, so the `get` returned a **symmetric key rather than the envelope JSON**. This is NOT the conveyance race the 2026-08-24 fix addressed and it is not a delivery flake: the read succeeded and returned the wrong thing. Diagnose before re-running — a `get` that yields an undecrypted or wrong record without throwing is a claim about at_client, not about the harness.
