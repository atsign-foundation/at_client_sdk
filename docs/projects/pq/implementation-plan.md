...
⚠️ **Rows A1, A2 and A3 are built (2026-08-14) — all of ruling 99. Everything
from B1 down, which is ruling 98's stages, is not.** The tree still has the
pre-98 stages, so a green rail below A is not evidence that any of it landed.
...
**The functional pack has not run since A1.** It was 163/163 at `cb3848b4d`
and A2 (`edc9dc8be`) changed at_auth validation only — but "only at_auth
validation" is a claim about scope, not a measurement, and A2 changed the code
path every keyfile read goes through. Run `./runLocal.sh` from
`tests/at_functional_test` before the B rows start touching the wire, so a
later red has one row to blame rather than two.
