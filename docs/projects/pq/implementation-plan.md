...
| **the at_lookup `signingAlgoType` default — the successor's half is built, the carve PR is owed** | **The code landed on this branch, and the carved at_auth PR is open as [#2198](https://github.com/atsign-foundation/at_client_sdk/pull/2198) — rebased onto trunk after [#2195](https://github.com/atsign-foundation/at_client_sdk/pull/2195) squash-merged, and MERGEABLE. What is left is its review.** Rowed in P2 on 2026-09-01 out of [#2161](https://github.com/atsign-foundation/at_client_sdk/issues/2161)'s deferral — *“This rides the next at_lookup version whenever one is opened for another reason”*, and at_lookup reached `3.7.0-rc1` without it — then made P0 by gkc the same day.<br><br>**What landed.** ① `authenticatorForChops` takes `signingAlgo` **and** `hashingAlgo` as required arguments. It is the one authenticator with no keystore behind its signer, so the caller is the only party that knows what the AtChops holds. ② `AtKeys.authenticationAlgorithmFor` **refuses** an algorithm this build cannot sign with instead of returning null — see the correction below. ③ `RemoteSecondary` passes `hashingAlgo` (it set the ladder's from the preference and let the authenticator take `sha256`, so the two read one field differently) and loses a `?? rsa2048` guard that could not fire. `signingAlgorithmForEnrollment` is deliberately unchanged and still reports both nulls; its many callers depend on that.<br><br>⚠️ **A correction, because this row asserted the opposite when it was written.** It said `authenticatorFor`'s two `?? SigningAlgoType.rsa2048` sites were both correct — *“a legacy keyfile records no algorithm and is RSA-2048”*. That holds for the branch that takes its signer from `authenticationFor`, which **throws** rather than reporting null for typed material it cannot read. It was false for the **injected-chops** branch: there the signer is the caller's own, and `authenticationAlgorithmFor` collapsed *“no typed material”* and *“typed material this build cannot sign with”* into one null, so `rsa2048` was a guess about a key nothing here can read — while a caller **without** an injected signer was refused for the same keyfile. gkc caught it; the refusal now sits one level up so both callers are held to the same terms. Latent rather than live: reaching it needs a keyfile written by a newer client.<br><br>**What the carve is, and why it lands alone safely.** A branch off trunk carrying only the at_auth half. Measured at `origin/trunk`: `authenticatorForChops` has exactly **two** call sites there, both in `packages/at_auth/test/at_authenticator_test.dart`, and at_client on trunk installs no authenticator at all — zero references under `packages/at_client`, with the pathspec proven non-empty first. The three live test packages hold none either. So nothing outside at_auth compiles against it on trunk.<br><br>⛔ **Two things this row is NOT, so they are not re-opened.** `AtLookupImpl.signingAlgoType`'s own `rsa2048` default needs nothing: it is `@Deprecated`, it dies with the credential ladder in the next major, and requiring it or making it nullable is breaking, so neither could land in 3.x. And amending #2161's *“rides the next at_lookup version”* sentence is **gkc's call, declined once already** — he chose not to comment on that issue on 2026-09-01. | [#2198](https://github.com/atsign-foundation/at_client_sdk/pull/2198) — review and merge. The code is built and on this branch; the PR is rebased onto trunk and MERGEABLE, so nothing here is startable |
