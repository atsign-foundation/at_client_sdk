...
| **a pqActive notification vanished in one functional pack run** | ⚠️ **ONE red against two greens on the same code, 2026-08-25 — a rate observation, not a diagnosis, and deliberately not chased** (gkc). `pq_posture_grid_test.dart`, *"a notification crosses atSigns and its value decrypts, for every sender posture"*, failed once at `ecf1082de` against `at_virtual_env:g0fixed`; the same pack ran **183 pass, exit 0** twice earlier the same day on the same code. The red run also took **5:33** against a normal **3:00**.<br><br>**What the failure says**, and it is unusually specific: *"s-pqActive's notification did not reach the receiver on EITHER of two sends, 150s in total, and both notifies reported delivered"* — and the test lists the 23 notifications its monitor DID see, including the `s-legacy` and `s-pqready` sends, then says *"It IS receiving, so this is not monitor readiness"*. So the monitor worked and the **pqActive sender's** notification specifically vanished, twice, with the atServer reporting both delivered.<br><br>⛔ **Already ruled out — do not re-derive:** it is **not** the pqActive/monitor phenomenon this plan records elsewhere, which fails with `AT0014` at the same rate on both arms — this run contains **zero** `AT0014`. It is not the day's `AtRpc` change, which nothing in this pack exercises (`git grep -l AtRpc -- tests/at_functional_test` is empty). It is not the day's no-commit work: the only file in the pack that sets the flag is that work's own live test, which writes run-unique keys with remote routing.<br><br>**The cheapest discriminating step, unrun:** the pack twice with `no_commit_live_test.dart` removed — it is the only thing added to the pack that day which writes records as the same atSign, so a differential rules it in or out quickly. After that, run `pq_posture_grid_test.dart` alone in a loop with the atServer's own log copied out before teardown (`docker cp test-virtualenv-1:/apps/logs <dir>`, and read the **rotated** files too) | Nobody yet. ⛔ **Do not call it a flake and re-run** — record any further occurrence with its numerator, denominator and commit, as this row does. If it recurs it needs an entry of its own before D1 closes, on the same grounds as the intermittent that became G3 |
...
# twice on the same code earlier the same day. That is 1 of 3 and it is written
# up in its own `## TODO` row with everything already ruled out. Every other
# figure here is exit 0.
#
