...
| **`at_activate approve` never terminates when the enrolment uses pq key exchange, which is the default** | **Found 2026-08-29 and reproduced from a shell prompt with no test involved.** The approver prints `Approving enrollmentId …` and never returns; the enrollee is approved on the atServer and then fails with `No conveyed apkamSymmetricKey arrived for key package <kpid> within 0:00:30. The enrollment is approved but cannot decrypt anything`. So the enrolment is left approved and unusable — the exact hazard the CLI's own `--key-exchange` help describes, on the default route rather than on a misconfiguration. <br><br>**The differential, interleaved so enrolment depth cannot account for it** — one atSign, one passcode, one master keyfile, `--key-exchange` the only thing varying, run legacy/pq/legacy/pq: **2 of 2 `legacy` invocations exited 0 in 4s and 5s; 2 of 2 `pq` invocations were still running at the 90s kill.** The legacy arms print `Server response: … EnrollmentStatus.approved`; the pq arms never print it at all. <br><br>⛔ **It is the default path.** `AtClientPreference.posture` defaults to `PqPosture.pqReady` (`at_client_preference.dart:179`), and `pqReady.keyExchangeMode` is `pq`, as is `pqActive`'s. Only the `legacy` posture avoids it, so two of the three postures and every caller that names none take the hanging path. <br><br>**Where it blocks.** The single await between the two print points is `atClient.enrollmentService!.approve(decision)` (`auth_cli.dart:898`). Inside it, the server-side approval at `enrollment_service_impl.dart:107` **succeeds** — which is why the enrolment ends up approved — so the block is in the post-approval work at `:117-123`: the record re-read, or `_conveyance.conveySecretsTo`. ⚠️ **Run the CLI with `--debug` to see any of this.** `auth_cli.dart:183` sets `AtSignLogger.root_level = 'shout'`; `-v` gives `info` and `--debug` gives `finest`. Without it the child logs almost nothing, and a silenced child reads exactly like a stalled one — which is how this defect stayed unlocated through several runs. <br><br>**Measured at `finest`, bounded at 420s and killed**: **188 log lines in total, and nothing at all after the 15-second mark.** No polling, no retries, no timer output — an alive, wholly idle process. So it is not a bounded wait quietly expiring. By marker, it prints `Approving enrollmentId` and `Prepared x-wing key package`, and **never** prints `Stored secret envelope` or `Server response`. Its last act on the wire is a remote `scan` for `.<kpid>.__ssenv.e2etest` returning `data:[]`. That brackets it to `EnvelopeEnrollmentConveyance.conveySecretsTo` between `verifyAdvertisedKeyPackage` (`envelope_enrollment_conveyance.dart:60`) and `shareSecretWith` (`:98`). <br><br>**Ruled out in source, so do not re-check them**: `waitForSecret` always ends (`completer.future.timeout(timeout)`); `requestSecret` has **no callers in `lib`** and is dead code; the only 5-minute `conveyanceWait` use (`nskey_seeding.dart:432`) is `unawaited`. <br><br>⛔ **ROOT CAUSE, found 2026-08-29 with trace markers across that bracket (markers proven to fire by a unit-test control first).** The approve stops after sealing the envelope and before signing it. The chain is `shareSecretWith` → `sendEnvelope` → `wrapAndSignAndJsonEncode` → `wrapAndSign` → `await signingKeys` → **`await signingKeyMintSettled(client)`** (`signing_key_mint_barrier.dart:28`). That barrier is a `Completer` registered by `PqClientBootstrap`'s **constructor** (`pq_client_bootstrap.dart:223`) and completed only by its `startup()`; its own dartdoc says it must complete on every path *because everything that signs waits on it*. **Nothing awaits it with a timeout.** In the hung approve, at `finest`, there are **zero** `PqClientBootstrap` and `SigningKeyMinting` log lines — the startup that must settle the barrier produced no output at all, so the signer waits on a Completer nothing will ever complete. That is also exactly why the differential splits the way it does: a `legacy` approve conveys nothing, so it never signs an envelope, never touches the barrier, and exits in 4s. <br><br>**Narrowed further by deduction from the step order.** `PqClientBootstrap` runs twelve ordered steps; step 4 is `mintInUseSigningKeys`, which settles the barrier in its own `finally` on **every** path — success, failure and gated-off alike — and `startup()`'s outer `finally` settles it again as a backstop. So if step 4 had run, signing could not block. It blocks; therefore **`startup()` is stuck in step 1, 2 or 3** — `hydrateHeldSecrets`, `collectConveyedKeys` or `startEnvelopeListener` — all of which precede the mint. The last wire act observed is a remote `__ssenv` scan returning empty, which is what `collectConveyedKeys` does. <br><br>⛔ **So this is a deadlock CLASS, not one bad step: any pre-mint step that signs an envelope, or waits on anything that needs signing, cannot complete — because signing waits on the barrier only a later step settles.** The existing dartdoc anticipates the mirror hazard ("waiting on `startupComplete` instead would deadlock — the steps after the mint sign envelopes themselves") and the pre-mint direction is the one it does not cover. ⚠️ **Still open**: which of the three, and why it settles for the first two approves of a run but not the third. The fix is likely a timeout or a not-running default on the barrier rather than anything in the conveyance — but that is a design call, and the constructor-registers/startup-completes split is where to make it. <br><br>**Reproduce**: bring up the onboarding-CLI virtualenv with an onboarded atSign and an spp, then `at_activate enroll --key-exchange pq -p buzz -d probe -n e2etest:rw` in one process and `at_activate approve -i <id> -k <master>` in another. The legacy control is the same pair with `--key-exchange legacy`. <br><br>**Consequence for the pack**: this is what makes `pq_native_enroll_test.dart`'s retrofit case time out. That test's earlier green rested on two CLI commands sharing one AtClient, so it never ran the command it names. ⚠️ **The guard is what exposed this, not what caused it** — `AtClientImpl.refuseChangedStoragePath` (2026-08-29) started refusing the client reuse the test depended on, and the reuse is why it had been green. The hang itself is older: the conveyance block it stalls in was extracted 2026-08-10, and `pq` became the default key-exchange mode on 2026-08-26 when the default posture became `pqReady`. ⛔ **Do not look for an at_client_sdk commit `c763407`** — that sha is at_server's, the at_persistence override the last green run used, and it was cited as this repo's head throughout the investigation before a `cat-file` caught it. **The at_persistence bump is exonerated by direct evidence**: the session transcript shows the onboarding pack green at `+19: All` **after** the override moved to `ddb04b26` (15:13) and **before** the guard existed (15:27), so the guard is the only change between the last green and the first red. ⚠️ **Reverting the guard does not make the test pass**, though — measured: with the guard neutered the pack is still 18 pass 1 fail, and the one failure is this hang. The guard and the subprocess fixture each independently stop the client sharing that was hiding it | Nothing. It reproduces by hand in under two minutes |
