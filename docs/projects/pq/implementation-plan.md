...
[Ruling 116](decisions.md) records proposals that were made and **rejected on
evidence** — nine of them, each one a fix somebody argued for and that turned
out to be wrong. It moved out of this plan on 2026-08-23 because a rejection is
a decision, and because nothing in the codebase or in `git log` can record it:
a proposal that was rejected was never built, so no commit contains it.
