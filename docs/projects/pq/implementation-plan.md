...
| **at_server legacy-PKAM enrollment: the client-side half of the `primary` migration** | ✅ The fixtures, the rewritten retrofit test, the corrected comments, the client-side half under [ruling 132](detail/decisions.md#132-the-keys-name-the-enrollment-and-primary-names-the-atsigns-own-credential-2026-09-07) and the seeding ruling are done (2026-09-04 to 2026-09-07); the record is in [the detail file](detail/implementation-plan.md#the-primary-migration-row-as-it-stood-on-2026-09-08). **Still owed:** ① remove at_auth's client-side self-approval (`enrollment_submitter.dart`: the `selfApproves` flag, the symmetric key minted only to approve itself, and the branch that fires on a `pending` answer) — unreachable against an atServer that auto-approves, and gkc ruled 2026-09-01 that an atServer too old to auto-approve need not be supported. Prove the path dead first: the 2026-09-04 green runs could not show it, since the branch logs at `info` and the CLI pack runs at `warning` — re-run that one file at `info` and grep `EnrollmentSubmitter`, or pin the auto-approve server-side. ② Two model questions for gkc: whether `primary`'s signing-root request and its `_apsk` route stay on the legacy path now that a legacy client is fully privileged server-side. ③ UC-B5.1's recorded reason for being undrivable from `signing_root_pull_test.dart` is false now that `enroll:listns` answers a legacy connection — for the next catalogue pass. The `blocked on a VE image built from at_server 7118d2e4` this row carried is discharged: `at_virtual_env:local` was built 2026-09-05, after that commit, and the onboarding-CLI pack's retrofit test passed against it on 2026-09-07. | Nothing for ① and ③; gkc for ② |
| **the CI logs carry hundreds of errors that are not test failures** | Asked for by gkc, 2026-09-07. Seven of the ten clusters were read and fixed on 2026-09-08 and are recorded with their figures in [the detail file](detail/implementation-plan.md#the-ci-log-row-as-it-stood-on-2026-09-08). **The last three were read 2026-09-08 and none is a defect in what it appeared to accuse:** **(4)** `the key package advertised by enrollment <id> does not verify` — 314 lines in CI (run 34205926815, `functional_tests (stable)`) against 20 locally, and it is **two** bad advertisements re-verified on every directory read, not hundreds of failures: `verifyAdvertisedKeyPackage` logs SEVERE per rejected member on each `listForNamespace`, with no memo. Each signature-failure line names its enrollment twice, which is why a per-id count reads double. Both bad members are left by `tests/at_functional_test/test/apsk_server_side_test.dart` on the shared `@alice🛠` and never restored — see the two rows below. **The CI-versus-local gap is FILE ORDER, not the runner and not `dev_env`**: that file runs 1st of 60 in CI and 32nd of 62 locally, so in CI fifty-nine files' worth of reads follow the poisoning. Reproduce: list the files in order with `grep -o '##\[group\]✅ test/[a-z0-9_]*\.dart' job.log \| sed 's/.*test\///' \| uniq \| cat -n`. **(8)** `AtSyncQueue … used before open()` — **31 in CI on 2026-09-07, 0 in CI and 1 locally on 2026-09-08**, so this session's stop-path work all but closed it; the survivor is one `Unexpected exception in sync … used before open()` in a local run and belongs to the in-flight-write row in P2. **(9)** `The connection went away before a response arrived` — 39 in CI against 40 locally, so it is **not** runner-dependent as this row assumed; 24 of 39 are `SEVERE` from `AtLookup` raised when a client is stopped with a verb in flight, the same in-flight-write family. ⚠️ **And a figure this row's fixes were credited with is worse than recorded:** `Box not found` in CI fell from **1013** (run 34160882867) to **130** (run 34205926815) rather than to zero, and 129 of the survivors follow one `PQ startup … was stopped with 10 of 12 steps`. Locally the same run shape gives 4. The P2 in-flight-write row carries it. | Nothing |
| **a first mint drops the authentication key from `_apsk`, and everything it signed stops verifying** | **Found 2026-09-08 by reading CI-log cluster 4 to its cause, and it is a product defect, not test noise.** An enrollment that authenticates post-quantum and holds no typed signing material signs envelopes with its APKAM **authentication** key — `ApkamSigning.signingKeys` falls back to it, and `apskEntries` advertises it on exactly the same condition, which the source states as one rule that cannot drift. When that enrollment mints its first signing key, `SigningKeyMinting.reconcileSigningKeys` publishes `[...minted, ...keeping]` with `retiring: superseded`, and **both `keeping` and `superseded` are built from typed held keys only** — the authentication key is in neither, so it simply disappears from the advertisement. Everything it signed then verifies against nothing, which is the outcome the retired-key mechanism exists to prevent: the code says retired keys "stay advertised as retired, so what they signed still verifies", and reasons about exactly this hazard for the rsa2048 case while leaving the ML-DSA-authentication case open. **Evidence:** `apsk_server_side_test.dart`'s `a healed enrollment advertises its signing key in the bare form` heals such an enrollment; for the rest of the run every peer rejects its key package with `Signature verification failed using public key for @alice🛠 enrollment <id>` — 155 times in CI run 34205926815, and in the local pack too. The test passes because it asserts only the FORM of the advertisement. **Blast radius, unmeasured:** the key package itself recovers at the next client start, because `reconcileKeyPackage` re-signs and re-advertises it; anything else the authentication key signed and left published does not. Enumerate those signers before choosing a fix. **What is owed:** a ruling on the fix — carry the authentication key into the advertisement as retired at the first mint, or re-sign what it signed in the same step — then the fix and a test that pins verification across a first mint rather than the advertisement's shape. | Nothing |
...
| Item | What is owed | Blocked on |
| ---- | ------------ | ---------- |
| **`apsk_server_side_test.dart` poisons the shared atSign for the rest of the run** | Found 2026-09-08 reading CI-log cluster 4. Two records are left on `@alice🛠` and never restored, and every later `listForNamespace` re-verifies both and logs SEVERE: ⓵ the cross-enrollment-overwrite test's own positive control writes the literal `this enrollment may write its own` over the ATTACKER enrollment's `_apsk` — base64 decoding that string fails at the space after `this`, which is the `FormatException: Invalid character (at character 5)` in the logs, 155 times in CI run 34205926815; ⓶ the healing test leaves an enrollment whose package no longer verifies, which is the product defect in P0, not this row. ⓵ is test hygiene and the pack's own rule covers it: a test that destroys shared server-side state gets its own atSign. Either give the file one, or capture and restore the `_apsk` it overwrites. ⚠️ Fixing ⓵ removes roughly half the cluster and none of the defect — do not read a quieter log as the defect being gone. | Nothing |
| **bypassCache: the publisher's atServer answers with the old value for a minute after its client reports the update pushed** | The known intermittent (`tests/at_end2end_test/test/bypasscache_test.dart`, red on two CI attempts of run 34160882867 on 2026-09-07, red 1 in 5 local e2e runs on 2026-09-08), now with a chronology that rules out the client's sync. The test's own precondition poll fails: `the publisher stores the updated value: waited 0:01:00 for "updated_value-…" and last saw "initial_value-…"`, reading the publisher's atServer with `useRemoteAtServer`. In the red run's window, at 02:16:41.297 the publisher's fresh client logged an app-sourced `sync round … pushed 1; server commit id 101` forty milliseconds after `put(updatedValue)`, a system-sourced round five seconds later saw the same commit id, and the remote read then returned the initial value for sixty seconds; the window also holds one `AtLookup … The connection went away before a response arrived`. So either the entry pushed as commit 101 was not the update, or the atServer holds it and answers `llookup` with the old value, or the push's response was lost after the atServer applied nothing and the queue still counted it pushed — the log cannot tell these apart. What is owed: on the next red, read the atServer's own log for commit 101 and the `llookup` answers (the far side's log, not the client's), and log the pushed key in the round summary at `finer` so the client side names what commit 101 was. Not caused by the 2026-09-08 sync change: pushes are app-sourced rounds, which still fetch. Red again on 2026-09-08 in runs 34200675382 and 34205926815, both in `end2end_test_14` only, same message — red on 4 of that job's last 5 attempts. **The discriminator, read 2026-09-08:** the two e2e jobs run the same Dart (3.13.3) and the same file, and differ in the atSign pair — `end2end_test_14` moves `config14.yaml` into place (publisher `@ce2e1`, reader `@ce2e4`) and `end2end_tests` moves `config23.yaml` (publisher `@ce2e2`, reader `@ce2e3`) — and the test has never failed in the second, green on both of today's runs. So the atServer answering with the stale value is `@ce2e1`'s, and that host is the far side to read. The e2e pack runs against the shared e2e atServers, whose logs this repo's CI does not collect, so the owed far-side read needs access to those hosts or a reproduction against the functional virtualenv, whose logs CI does keep. | Nothing |
