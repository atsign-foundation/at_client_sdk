...
| **`PqStartupGates` is public API that only tests use** | Nothing in any `packages/*/lib` names a custom gate set — production gets all-on or inert, both derived from the posture. A patch to fence the field `@visibleForTesting` and move the posture-to-gates rule behind an `@internal` resolver on `AtClientPreference` is ready at `~/dev/atsign/repos/tmp/pq-startup-gates-internal.patch`, based on `957010e9f`, gated on that base (at_client 1858, the functional pack +200, the format gate clean). Two calls are open: `@internal` appears in no `lib/` in this tree, so this would be its first use; and the resolver's name has no precedent here. | gkc's view on `@internal` |
