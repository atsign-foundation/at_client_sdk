...
**The release train's live gate**: gkc published at_chops 3.6.0 and at_commons
5.16.0 on 2026-08-21, so the first two positions are released. **at_lookup
3.7.0-rc1 is not on pub.dev**, and nothing after it can publish until it is.
...
**So what remains of this entry is arms 2 and 3**, plus the clause level of the
ledger. ⚠️ This read "arms 2–4 … arms 3 and 4 need the EE at a named
`at_server` ref", and both halves changed on 2026-08-24: **arm 4 is cancelled**
by gkc (the hosted fleet will run the version a release requires), and **neither
arm 2 nor arm 3 needs the EE** — the belief that a transition could not run in
the VE generalised the matrix's rule against re-minting in a *cell* to an
advance, and 24 retrofits across five live runs on one virtualenv refute it.
...
1. **Arm 2 replaces the 4×4** with one in-process grid over sender posture ×
   receiver posture — self and cross-atSign puts, gets and notifications, with
   per-cell expected outcomes, carrying the signed-envelope exchange too.
2. **Arm 3 is a separate ladder**, legacy → pqReady → pqActive on one
   enrollment, asserting the `.atKeys` shape per rung and re-reading
   pre-advance data.
3. **`tests/pq_matrix/` is deleted** — the scenario package, both programme
   arms and the `##PQM##` line protocol. One standalone released-peer test
   survives to keep UC-G1.14 proven; nothing else in the tree compares this
   tree's legacy posture against a released at_client.
4. **Provisioning is 2 atSigns × 7 enrollments across 5 namespaces**, live
   proven 2026-08-24: 14 clients in one process, each with its own Hive store.
...
`success`, measured 2026-08-22. Since then, re-derived 2026-08-23:

```bash
git rev-list --count 64480808d..HEAD                              # 26
git diff --name-only 64480808d..HEAD | grep -vc '^docs/'          # 45
```
...
|---------------------------------|---------------------------------------------------------------------|----------------------------------------------------------------------------------|
| **app enrollments cannot be PQ-native** | ⛔ **RULED A PRODUCT DEFECT BY gkc, 2026-08-24.** An app enrolling with an OTP always gets an **RSA-2048** APKAM authentication keypair, and there is no way to ask for anything else: `AtEnrollmentRequest` declares no `signingAlgo` field (its siblings `AtSelfEnrollmentRequest` and `FirstEnrollmentRequest` both do), `EnrollmentSubmitter._handleAtEnrollmentRequest` mints with `AtChopsUtil.generateAtPkamKeyPair`, which takes a key size and nothing else, and that method never sets `EnrollVerbBuilder.signingAlgo` — so the field is **absent on the wire**, which the atServer reads as `rsa2048`. **Consequence on a fully pqActive atSign:** every app install creates a valid RSA-authenticating enrollment, which the client then retrofits away at construction, leaving the RSA parent **capped rather than deleted** to `min(now + grace, expiry)` — 720h by default. So an operator who has moved everything to pqActive still grows a rolling 30-day RSA surface they cannot turn off, and nothing reports it. ⚠️ **This is a policy exposure, not a new cryptographic one**: harvest-now-break-later does not reach a capped enrollment, because the cap expires long before RSA-2048 does — exploiting it needs an adversary who can already break RSA-2048 today, and such an adversary reaches every other RSA credential too. **Owed:** give `AtEnrollmentRequest` a `signingAlgo`, thread it to the minted keypair, the `_apsk` advertisement and the verb builder. ✅ **The atServer needs NO change** (scoped 2026-08-24). The OTP path and the self-enrollment path are the *same code*: `enroll_verb_handler.dart:354-355` copies `signingAlgo` onto the enrollment record **above every branch on authentication type**, so all three request flavours fall through it. Read in `at_server` on branch `gkc-2678-protocol-enhancements` @ `7554e237`, which **is** an ancestor of the local `origin/trunk` (`96243830`) and differs from it only in three `.github/workflows/` files — but the local trunk ref was written 2026-08-22 and was not re-fetched. ⚠️ **Absent means null, not `rsa2048`**: the record stores null and the fallback is applied at *read* time (`apkam_signature_verifier.dart:52`), and PKAM is **record-authoritative** — the enrollment record's algorithm overrides whatever the `pkam:` wire token claims. ⚠️ **`enroll:request` does not validate `apkamPublicKey` against the declared `signingAlgo`** (only `enroll:update` does, via a self-signature), so a client that declares one algorithm and mints another is not caught at request time — it surfaces at first PKAM. ⚠️ **A SECOND gap in the same request type:** `AtEnrollmentRequest` also cannot supply `advertisedSigningKey`, which its two siblings both forward — so even once `signingAlgo` exists, the OTP path cannot reproduce what PQ-native onboarding produces (an ML-DSA authentication key beside an RSA-2048 signing key the enrollment owns from birth), and an mldsa65 OTP enrollment would advertise an `_apsk` array naming `mldsa65`. ⛔ **Separate and pre-existing:** every other atServer implementation would record an ML-DSA enrollment and then never authenticate it; that gap is independent of this fix and lives on an unmerged branch. **Sequencing ruled by gkc 2026-08-24: the acceptance suite is built FIRST, against today's behaviour**, so G2 does not block behind this | ✅ **The default is RULED (gkc, 2026-08-24): there is none — `signingAlgo` is REQUIRED on `AtEnrollmentRequest`**, so the compiler enumerates every construction site rather than letting one silently inherit a value it never chose. Default it **once**, at whichever boundary genuinely has no basis to choose, with a dartdoc saying why it is safe there. at_auth is mid-carve at 4.0.0-rc1 and already taking a major, so the break costs nothing extra. ⚠️ Nothing in the scoping was RUN — every finding is read from source, so a live probe of an OTP `enroll:request` carrying `signingAlgo:mldsa65` is the first build step, not the last |
| doc-set reduction, phases 3–5 | ⛔ **RULED BY gkc 2026-08-23, AFTER D1 — do not start it while D1 is open.** The end state is five files: `roadmap.md` (stale, needs a pass), `design.md`, `acceptance.md`, `decisions.md` (seriously shrunk) and this plan, which from now records **only what is still owed**. Phases 1 and 2 landed 2026-08-23 — the rejections and measurements became [rulings 116 and 117](decisions.md), and this plan went 1,878 → 1,075 lines. **What remains, and phase 3 MUST precede phase 4:** (3) trim the **117** ruling bodies in `detail/decisions.md` and inline them into `decisions.md` — they average **98 lines each**, and only **4 of 116** rulings are dead, so this is an editorial pass over live content rather than a purge of obsolete ones; (4) delete `detail/` and repoint or remove the **250** links into it (113 from this file, 63 acceptance, 62 design, 9 roadmap, 2 decisions, 1 seal-spec), rewriting `docs_structure_test.dart`, which enforces index↔body correspondence both ways and names `detail` 28 times — the rail changes in the SAME commit or CI goes red; (5) substitute explanations for the code that cites `detail/` paths, which is a standing rule violation as well as a broken link: `tests/at_functional_test/test/pq_rollout_matrix_test.dart` and, worse, a **dartdoc** in `tests/pq_matrix/current/lib/envelope_exchange.dart`, plus two in `tests/pq_matrix/README.md`. `cross_cutting_test.dart` reads `detail/decisions.md` as a build input and is fine. ⚠️ **Phase 3 is where this goes wrong silently** — a ruling trimmed too far reads complete, and 112 of 116 rulings are still in force. Re-derive the size: `for f in docs/projects/pq/*.md docs/projects/pq/detail/*.md; do echo "$(grep -c '' $f) $f"; done` | Nothing but D1 closing. `detail/` is **19,141 lines against 7,231 live**, so this is most of the reduction |
...
https://pub.dev/api/packages/<pkg> | python3 -c "import
sys,json;print(json.load(sys.stdin)['latest']['version'])"`.
