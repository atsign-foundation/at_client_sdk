...
⚠️ **The SS-4 prerequisite holds for `B-1c` onward** — `B-1a`/`B-1b` proceed against a test fixture that
supplies the nskey private directly (see the chunk table below). B-1 **as a whole still requires SS-4**; the
dependency is not dropped, only deferred past the first two chunks.
...
**PR chunks (ordered — B-1 is the only XL project; it lands as five sequential PRs, `B-1a`…`B-1e`, each
one PR, each with its own green acceptance scenario). `B-1` stays the project id; the chunk ids are its PR
breakdown, not new projects:**
...
The **B3 capability marker** spans the last two chunks: per-destination scheme selection lands with `B-1d`
(the first chunk with a non-self destination), `providerId` on the notification frame with `B-1e`.
...
carries the scenarios listed against it in the chunk table — no chunk merges without its own green scenario.
