...
| ---- | ------------ | ---------- |
| **the post-merge fix-forward on `gkc-pq-d1-spike`** | ⛔ **A POINTER ROW — the analysis lives in the wasm plan, not here** (gkc, 2026-09-07). `origin/trunk` merged into the spike at `9c84011df`, bringing X4, X5 and X6 and X4a item 3. Unit-green everywhere; the functional pack is `+195 -6`; **the e2e and onboarding-CLI packs have NOT been run against it**, which a storage/lifecycle change wants. What is owed, why each failure fails, the conveyance investigation, the fixture helpers and the traps met are all in [`docs/projects/wasm/implementation-plan.md`](../wasm/implementation-plan.md) — the X6 row and its **Post-merge fix-forward** section. Read that before touching this; it exists so none of it is re-derived. | Nothing |
