...
1. **`UC-G1.15` threw `FormatException` at `pq_posture_grid_test.dart:809`**, jsonDecoding a 44-character base64 string — 32 bytes, a symmetric key, where the record holds a **4,656-character envelope**. **`get` returned it without throwing.** Instrumented and re-run 2026-08-24; what is established:
   ...
   - **The structural suspect is re-entrancy, not the wire.** Response correlation looked correct in the log (value ciphertext `32bO…` returned first, the 0x02 pqSeal `AgRg…` second, in send order), so replies are not crossing sockets. But `SymmetricAesGcmProvider._resolveFromConveyance` (`symmetric_aes_gcm_provider.dart:267`) calls `context.atClient.get(conveyance)` **re-entrantly, from inside the outer `get`'s own decrypt**, and discards the result — the CK is collected as a cache side effect. An outer `AtValue` that ends up holding the inner read's value and metadata is what that shape would produce.
