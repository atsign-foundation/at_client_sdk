...
| **The acceptance burn-down under-reports by more than half, and hides one real gap.** Audited 2026-08-03. (a) `catalogue_test.dart` runs in at_client's unit suite and cannot see `tests/at_functional_test` or `tests/at_end2end_test`, where roughly half the rows are actually proven — so a live-proven scenario can never turn its own row green. (b) `blockers.dart` is stale: 3 rows still read `blocked: SS-2` (complete) and 7 `blocked: SS-4` (half done); its own dartdoc describes the fix — delete the constant and let the analyzer point at what it unblocked. Reads **4 of 43** against a branch where the live packs prove ~**8–9 rows plus 2 invariants** | `at_client` tests |
...
| **`B-1e` does not work, found 2026-08-04 by the new two-client harness** ([decisions.md 26](decisions.md#26-uc-a44-does-not-work-the-ck-fetch-deadlocks-inside-notification-dispatch-2026-08-04)). Both halves are implemented and unit-covered, and the nskey notification still never reaches a subscriber: the CK fetch in `SymmetricAesGcmProvider._resolveFromLocalConveyance` never returns when it runs inside the monitor's dispatch path. Probed to the exact await — `START` with no `DONE` — with regex, throw and non-dispatch all eliminated by measurement. UC-A3.4 / UC-A4.4 are **not met** and must not be claimed. The *legacy* receive path is now live-covered. Superseded note, kept because it is why this row read as done: | The send half stamps `appMetadata.providerId` at both notify entry points (`notify_request_transformer.dart`, `notification_service_impl.dart`) and the receive half routes by it through `CryptoRuntime.decryptForNotification` (`notification_response_transformer.dart`). So UC-A3.4 / UC-A4.4 are implemented and unit-covered; what neither can get is **live** coverage of the receive half, because `AtClientManager` is a singleton whose `setCurrentAtSign` stops the previous atSign's monitor *and* unsets its `notificationService` — no test can hold a subscription on one atSign while another sends. B-1e is therefore a **harness** item (two concurrent clients), not a data-path one | **B-1e** |
