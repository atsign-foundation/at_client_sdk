...
| **at_server retrofit on a `pkamLegacy` connection — tests owed here once it lands** | **gkc is extending at_server to auto-approve a retrofit on a `pkamLegacy` connection** (2026-09-01, worked through in a separate session). What is owed *here* is the tests that verify that path once it lands, and the client code that comes OUT when it does. ⚠️ **Nothing is broken today, and this row is not a gap report** — it is a mechanism moving from the client to the atServer. A pre-enrollment atSign retrofits today by **park-then-self-approve**: the request lands `pending` because neither auto-approve branch matches `pkamLegacy` (`enroll_verb_handler.dart:372` is the CRAM first enrollment, `:406` needs an `AuthType.apkam` connection with an approved `parentEnrollmentId`), and the client then approves its own request over the same connection — `enrollment_submitter.dart:550` and `:611` — which the atServer allows because a connection carrying no enrollment id has full access. That arrived in `ddb4a9342` (2026-08-30, commit 7) and is **absent from `origin/trunk`**; before it, a pre-enrollment atSign was excluded from the retrofit entirely, returning early with no enrollment id to move. It is proven by `tests/at_onboarding_cli_functional_tests/test/pq_pre_enrollment_retrofit_test.dart` — in the pack whose runner deliberately skips `pkamLoad` — which installs the flat `at_pkam_publickey` **by hand** rather than calling `AtOnboardingServiceImpl.onboard()`, because that sends `enroll:request` on the CRAM connection and would create the very first enrollment whose absence defines the state under test. ⛔ **Two things that follow, one in each repo:** ① at_server's own `tests/at_functional_test/test/enroll_verb_test.dart:283` — named *“enroll request on APKAM authenticated connection and verify enroll:list”* while authenticating with **no enrollment id** — asserts `status == 'pending'` at `:297`. That assertion REVERSES when the retrofit auto-approves, and the name misdescribes the connection even today. It is the only functional coverage of the current behaviour. ② Here, the client's self-approve comes **out**: `selfApproves`, the symmetric key it mints solely to approve itself, and the `:611` branch. ⛔ **There is no old-server fallback to preserve** — gkc ruled 2026-09-01 that we do not need to support an atServer too old to auto-approve, and the existing `enrollStatus != approved` block already denies the record it just created and throws, which is that ruling in force. A `pkamLegacy` retrofit against an atServer lacking the path would simply join it. ⚠️ **This row first said that branch must SURVIVE as an old-server fallback, which was wrong** and conflated the two causes of `pending`: the APKAM-against-an-old-server case is the one that deny-and-throws, and it never routed through the self-approve at all. ③ **Our functional pack must widen for the housekeeping enrollment H.** gkc ruled 2026-09-01 that a legacy authentication EAGERLY creates H — a deterministic-id enrollment holding `{'*': 'rw', '__manage': 'rw'}` at `approved` — and that `enroll:list` shows it unfiltered. `tests/at_onboarding_cli_functional_tests/test/pq_pre_enrollment_retrofit_test.dart` breaks in FIVE places, because its `enrollmentsOf()` helper authenticates with the FLAT credential and so CREATES H by the act of reading: the precondition `isEmpty`, `expect(roster, hasLength(1))`, `roster.values.single` and `roster.keys.single` (both THROW rather than fail on two entries), and the control test's two `isEmpty` assertions on the legacy-posture atSign. The control keeps its force once widened — what it proves is that a legacy-posture atSign grows no SUCCESSOR, and H is not a successor. ✅ **H's id is CONFIRMED as the literal `primary`** (gkc, 2026-09-01), giving the enrollment key `primary.new.enrollments.__manage<atSign>` — safe to pin. It is not a new constant: `primary` is ALREADY this tree's stand-in when no enrollment id exists, at `mint_lock.dart:208`, `apkam_signing.dart:66` and `:67`, and named in `pq_signing_root.dart:918`. Nothing branches on behaviour — `apkam_signing.dart:67` only logs a warning — so making the sentinel a REAL value does not wake a dormant check. ④ **A SHIPPING DARTDOC BECOMES FALSE, and it has FOUR homes.** `packages/at_client/lib/src/mixins/apkam_signing.dart:82` says this is the only writer for an `_apsk` no `enroll:request` can carry "— a client with no enrollment publishes under `primary`, which has no enrollment record for the atServer to compose one from". Once H exists there IS such a record. The BEHAVIOUR still holds and only the REASON goes false, which is the worse case: a reader trips on the justification, not the claim. The same sentence is restated at `detail/implementation-plan.md:2794`, `:6886` and `:7175`, so a fix that edits only the dartdoc leaves three copies lying. ⑤ **A gap closes for free, and it is ours.** `publicSigningKeyUri` means a legacy client already publishes at `public:_apsk.primary.a.__e@<atSign>` today, and the per-enrollment regex captures `primary` as its EnId — so once H exists that key becomes H's per-enrollment data and a revoke or expiry parks it at `.r.`/`.d.` like any other enrollment's. Today nothing ever retires it: a legacy client's published signing key outlives the credential indefinitely. ⚠️ **The e2e retrofit pack will not notice the change** — `tests/at_end2end_test/test/pq/retrofit_e2e_test.dart:80` starts every retrofit from an OTP enrollment approved by the owner-keys client, so it exercises `:406` and never `pkamLegacy`. ⚠️ **PR #2195's *Why* paragraph is wrong in two ways and gkc has NOT asked for it to be corrected** (2026-09-01): it names only the APKAM auto-approve and never the CRAM path, and it says *“there is no second device to approve it from”*, which inverts the fact — no second device is NEEDED, because the connection is the owner's and already fully privileged. The sentence came from at_auth's own class dartdoc at `enrollment_submitter.dart:28` | at_server — gkc, in a separate session as of 2026-09-01. The tests owed here cannot run against a real atServer until it lands and the local VE image is rebuilt. ⛔ **No sha, no branch name and no PR number here** — none had been stated when this row was written; re-derive from gkc or from the at_server repo |
...
| **the at_lookup `signingAlgoType` default, and the one that replaced it** | **Deferred out of [#2161](https://github.com/atsign-foundation/at_client_sdk/issues/2161) on 2026-08-18, and the deferral's premise is now spent.** The issue closed noting that `AtLookupImpl.signingAlgoType` still initialises to `rsa2048`, so *“any construction site that forgets silently authenticates with the wrong routine”*, and deferred the fix because at_lookup was then in-tree and published 3.6.1: *“This rides the next at_lookup version whenever one is opened for another reason.”* at_lookup **is** `3.7.0-rc1` now, opened for the consolidation, and the change did not ride it — `at_lookup_impl.dart` still declares `SigningAlgoType signingAlgoType = SigningAlgoType.rsa2048`.<br><br>**That field is no longer the thing to fix.** It is `@Deprecated` and dies with the credential ladder in the next major; requiring it at construction or making it nullable is breaking, so neither can land in 3.x. **The defect moved into the successor instead.** `authenticatorForChops()` — named by that very deprecation message as the replacement — declares `SigningAlgoType signingAlgo = SigningAlgoType.rsa2048`, and its dartdoc states the default rather than questioning it. A caller holding an ML-DSA `AtChops` that omits the named argument signs the PKAM challenge with the RSA routine: the #2161 failure exactly, one layer up.<br><br>**What it should do.** ① Make `signingAlgo` **required** on `authenticatorForChops` and let the compiler enumerate the sites — the standing *“a cross-cutting change that must reach every call site”* move, and free here: at_auth is `4.0.0-rc1` in-tree, pub.dev's latest is **3.3.0**, and the published 3.3.0 archive carries no `at_authenticator.dart` and no `authenticatorForChops` at all, so there is no holder outside this tree. Four call sites, of which both production ones already pass it explicitly (`remote_secondary.dart`, `notification_service_impl.dart`), so only the two in `packages/at_auth/test/at_authenticator_test.dart` change — and naming `rsa2048` there turns their `contains('signingAlgo:rsa2048')` assertion into a pin instead of a restatement of the default it inherited. ② Drop `remote_secondary.dart`'s own `_signingAlgoType ?? SigningAlgoType.rsa2048`: a third written home for the constant, and a guard that cannot fire — `_signingAlgoType` is assigned from `signingAlgoType ?? preference.signingAlgoType` four lines before the constructor calls `_installAuthenticator`, and the only other call is from the `atChops` setter, which needs a constructed object. ③ Amend #2161's *“rides the next at_lookup version”* sentence, which is false whichever way the rest is decided.<br><br>⛔ **Three other `rsa2048` sites on this path are correct and must not be swept in with it**, each decided by key material rather than by a caller forgetting: `authenticatorFor`'s `keys.authenticationAlgorithmFor(...) ?? SigningAlgoType.rsa2048` (twice — a legacy keyfile records no algorithm and *is* RSA-2048); `authenticatorForPrivateKey`'s constant (a bare private key is RSA by construction); and `signingAlgoOf`'s tail in `resolved_signing_algo.dart`, the documented legacy fallback for untyped material.<br><br>**The ruling this follows is already in the tree, twice.** `auth_cli.dart` and `at_onboarding_service.dart` each had an `rsa2048` fallback removed for this reason, and each carries the reasoning in place: *“Two defaults for one fact agree only until one of them moves.”* Both stopped agreeing when the shipped default posture became `pqReady` — the enrolment minted an RSA APKAM keypair while `authenticate()` declared ML-DSA-65 for the same enrollment, and at_chops refused the pair by key size.<br><br>⚠️ **Two neighbouring rows are this shape at other sites and are deliberately not merged with it**: the test-helper row (`enrolAndAuthenticate`'s `signingAlgo` default, in test code) and the key-package row (`enrollment_key_package.dart`, data signing rather than PKAM authentication). Three sites, three decisions. | Nothing |
