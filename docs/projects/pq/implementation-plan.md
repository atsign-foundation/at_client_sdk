...
**The one thing commit 7 must get right.** `preventDuplicateEnrollRequest` runs
...
None is a correctness gap, and each of the branch's four parent-dependent
mechanisms — `verifyNoEscalation`, `parentEnrollmentId`, the inherited key-expiry
posture and the cap — would need a no-parent answer first. Optional, and not what
commit 7 waits on.
...
- **at_auth** — `_handleSelfEnrollmentRequest` mints a symmetric key, wraps it to
  the atSign's own encryption public key on the **request**, and then approves
  over the same connection through the ordinary `EnrollmentApprover`. Wrapping to
  the atSign's own key is what keeps the record's copy recoverable, so nothing is
  stranded by a key minted in a process that then forgets it. ⚠️ **The
  discriminator is the SESSION's enrollment id, not the connection's**, because
  `pending` has a second cause — an APKAM self-enrolment against an atServer too
  old to auto-approve — and that one keeps its deny-and-throw.
...
all. Re-derive rather than quoting these — the commands are in
