...
| **Parity across every atServer implementation for the enrollment key-exchange relaxation.** The Dart atServer now lets an `enroll:request` advertising a key package omit `encryptedAPKAMSymmetricKey`; every other implementation still rejects it, so a `pq` enrollee meets a mandatory-field error there. Deliberately not done in the same sweep | **SS-2** |
