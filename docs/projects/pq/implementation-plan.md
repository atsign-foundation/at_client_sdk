...
| ---- | ------------ | ---------- |
| **the developer-facing rotation hooks have no acceptance clauses** | **Raised by gkc 2026-08-28, and the gap is threefold.** The two rotation policy closures shipped on 2026-08-28 as public — `@experimental`, but public — API on `CryptoConfig`: `CkRotationPolicy`/`CkRotationContext` with the default `rotateCkAfterOneWeek`, and `NskeyRotationPolicy`/`NskeyRotationContext` with the default `neverRotateNskey`. [Ruling 122](detail/decisions.md#122-rotation-cadence-the-nskey-lever-fires-on-cause-the-ck-lever-asks-a-policy-2026-08-28) states the DESIGN. **Nothing states the CONTRACT.**<br>1. **No clauses.** Measured 2026-08-28 with a positive control (`nskey` matches 155 times in the same file, so it is searchable): `acceptance.md` contains **zero** occurrences of `RotationPolicy`, `rotation policy`, `rotateCkAfterOneWeek`, `neverRotateNskey` or `CkRotationContext`.<br>2. **No doc names the test AS A PROOF.** `packages/at_client/test/rotation_policy_test.dart` holds six tests, and until this row was written on 2026-08-28 it was named in no file under `docs/projects/pq` at all. This row is now the only place that names it — which is a pointer, not a citation: no clause cites it, so the burn-down still counts none of what it proves.<br>3. ⛔ **The rail that exists for exactly this could not see it, and half of that is now fixed.** `docs_structure_test.dart`'s *“each `pq_*_test.dart` is named somewhere under docs/projects/pq”* enumerated **only** files under `tests/` **and only** those matching `pq_*_test.dart`, so everything in `packages/at_client/test/` was invisible on both counts. ✅ **The root was widened on 2026-08-28** to `tests/` plus every package's own `test/` tree, and a second rail was added beside it — *“every test a citation names is named in the doc set”* — which is the sharper of the two, since a cited test is PQ-relevant by construction whatever it is called. ⚠️ **The filter half stands**: both `mint_lock_self_contention_test.dart` and `rotation_policy_test.dart` are named `pq_*` by neither convention nor citation, so neither rail reaches them and this row is still the only thing naming them.<br><br>**What the use cases must carry.** Every clause below is already true and already covered, so these land as PROVEN citations rather than new gaps — but open each production path before writing its clause, because a clause can be false rather than untested. **The CK lever:** asked *before* the already-current key is returned; handed destination, namespace, ck kid, `cutAt` and `now`; returning true cuts a fresh key and conveys it, and the superseded conveyance record is **retained** so an enrollment that joins later can still read what was written before it; the default is seven days and the boundary is **inclusive**; age is measured against the `now` it is handed, never the wall clock. **The nskey lever:** asked at **two** points — once per authorised namespace at every client start, and before a content key is conveyed where the destination is this client's own atSign — because neither reaches every application alone; the default is never, at any age; returning true mints fresh material, retains the previous private and conveys the new one to every authorised enrollment; and a sender cannot replace a **peer's** namespace key. ⚠️ The last two are the clauses most likely to be re-derived wrongly, so they are the ones that most need stating.<br><br>✅ **The measurement this row asked for was taken on 2026-08-28, and it changed the answer.** Pointing the rail at `packages/at_client/test/` wholesale would go red on **70 of 142** files — at_collections, sync, legacy encryption, verb builders — which is the long tail this row predicted, and it is not worth naming. Two narrower widenings were built instead and between them go red on **8**, every one real: widening the `pq_*` root to package test trees added 6 files of which 5 were already named (**1** red), and requiring every file a `provenIn` citation names to appear in the doc set covered 81 files of which 74 were already named (**7** red). All 8 are now named in [section 14](acceptance.md#14-test-harness--implverify-mapping). The seven included one file cited by **six separate rows** and two in `packages/at_auth`, a package no version of this rail had ever looked at. Re-derive rather than trusting these figures: `dart test test/acceptance/docs_structure_test.dart --concurrency=1` | Nothing |
...
| **The PQ data-signing-key programme — EIGHT commits, seven done, design settled 2026-08-30** | ⛔ **Design settled with gkc 2026-08-30. Supersedes the former rows for the mint barrier and for `sendEnrollRequest`'s stranded key package.** The design is in `untracked/pq-data-signing-key-states.md` and the sequence in `untracked/pq-commit-sequencing-plan.md` — ⚠️ **both gitignored — but the write-up is DONE**: each of the seven landed commits carried its own amendment, and both untracked files are now partly stale, so the tracked docs win.<br><br>✅ **1, 2, 3, 5, 6, 8 and 9 are DONE** — `3308eddcb`, `56f2e5d39`, `b8e4d5291`; re-derive with `git log --oneline`. ⛔ **4 is DROPPED** ([ruling 127](detail/decisions.md#127-a-client-with-no-enrollment-id-still-mints-and-publishes-its-own-signing-key-2026-08-30)). **Eight commits. SEVEN ARE DONE; only 7 remains, and it needs an atServer change.** 1 at_auth files the advertised signing key's private half on the ordinary enrollment path. 2 at_onboarding_cli mints and advertises one per algorithm in `dataSigningKeyAlgorithms` — **the algorithm the enrollment will keep**, rsa2048 at pqReady and mldsa65 at pqActive, not rsa2048 unconditionally — and the same for `pq_native_onboard`. 3 `reconcileSigningKeys` stops miscounting a legacy enrollment's rsa2048, **scoped to rsa2048** or it collapses the auth/signing split at pqActive. ⛔ 4 **dropped**: a null-id client publishing its own `_apsk.primary` is a mechanism `signing_key_minting_test.dart` specifies, and 7 — which was to replace it — is refused by the atServer. See [ruling 127](detail/decisions.md#127-a-client-with-no-enrollment-id-still-mints-and-publishes-its-own-signing-key-2026-08-30) for the residual it leaves open. ✅ 5 **the mint barrier is deleted**. ⚠️ **2 and 3 did NOT fully carry it** — it still guarded one state, an enrolment authenticating post-quantum that holds no signing key, and `serialiseApskWrite` does not cover that because the lock serialises `_apsk` **writers** and this is a **reader**. Deleted anyway (gkc): the state has no holder outside this tree, and the deadlock is measured. [Ruling 126](detail/decisions.md#126-the-mint-barrier-is-deleted-legacy-authentication-and-data-signing-are-one-keypair-2026-08-30) is amended in place, because its own safety sentence was what was false. ✅ 6 `AtClientPreference` refuses an empty signing set with non-rsa2048 auth, and any explicit authentication axis weaker than its posture. [Ruling 113](detail/decisions.md#113-pqposture-three-postures-and-the-rollout-they-drive-2026-08-18) amended. ⚠️ **The blast radius was TWELVE sites across four packages, not the seven the design named** — the doc keyed its sweep on the literal `authenticationKeyAlgorithm:`, while rule B keys on the RESOLVED value, so a site naming only an empty `dataSigningKeyAlgorithms:` against a pq posture trips it while appearing nowhere in that list. One such site was an assertion rather than a fixture, so the rule **deleted a pinned property** (that an explicit empty set beats the posture) rather than breaking a test. ⛔ **And the doc's prescribed fix for the pinning fixtures — "rebase on pqReady" — would have created fresh rule-D violations**; they take a non-empty signing set instead, keeping the posture. The retired justification string was duplicated in **two** packages, not one. 7 a pre-enrollment atSign retrofits at a PQ posture. ✅ 8 a client with `!configuresPqProviders` neither approves a pq request nor sweeps. **The discriminator is the ABSENCE of a wrapped symmetric key**, not the advertised key package — a package rides every mode, and the code already computed the right signal for its own reasons, so refusing after it costs no extra round trip. A legacy request is still approved normally, which is the whole of what such a client is for. The sweep is refused in `EnrollmentServiceImpl` **and** gated in the startup: the gate is the startup's answer, the throw is every other caller's. ⚠️ **`MockAtClient.getPreferences()` is a CONCRETE override**, so `when(...)` cannot stub a posture — the mock and its builder took a `posture` argument for this. ⚠️ **The refusal reddened 28 functional tests, and the FIXTURES were wrong, not the rule** — their approvers named `PqPosture.legacy` to get two of its *other* axes (no retrofit, no namespace seeding) and silently also got "no post-quantum providers", which was never the intent and only became load-bearing when something asked. 15 approver constructions now name `legacyPlusPqProviders`, which is legacy in every axis except that one. The grid's legacy RECEIVER cell is deliberately left alone. ✅ 9 a privileged approver without the root private signs a chain link when it holds a data signing key, and conveys **nothing** when it holds neither — that third arm is the one that must not guess, because `signingKeys` falls back to the APKAM key and that key is dropped rather than retired. [Ruling 67](detail/decisions.md#67-workstream-bi-the-sweep-anchors-to-the-root-2026-08-10) amended: its "the every-start pull heals possession" does not hold, since the pull and the sweep are both startup steps, so nothing re-attempts the link for an enrolment approved while its approver was unpossessed.<br><br>⛔ **1 and 2 are a pair** — advertising without filing is worse than today. 1 is separable only because it is a no-op alone. ⛔ **7 is REFUSED BY THE atSERVER, measured 2026-08-30 against `origin/trunk`** — the spec it needs is [below](#the-atserver-change-commit-7-needs). `enroll_verb_handler.dart:406` gates the self-enrolment auto-approve on `authType == AuthType.apkam`, and a pre-enrollment client authenticates with the flat `at_pkam_publickey`, which sets `pkamLegacy` and a null enrollment id — so it cannot enter that branch at all, and :410-414 refuses a null id outright. 7 needs an at_server change or a different mechanism; it is not a client-only item.<br><br>**Two live tests moved with 1-3, both re-fixtured rather than weakened**: `self_enrollment_retrofit_live_test.dart` pinned the retrofit’s key package as RS256 at pqActive, which was the rsa2048 hardcode; `apsk_server_side_test.dart` healed a legacy-shaped enrolment, which is exactly the mint 3 removes — its enrolment now authenticates post-quantum, which is the only shape the heal path still serves.<br><br>✅ **Rulings 67, 113 and 126 are all amended in place**, each in the commit that changed it, and each marked AMENDED in the ledger index.<br><br>**Commit 7 is the largest and least specified** and may deserve its own session.<br><br>**What actually proves 5, and what does not.** ⛔ **The CLI pack is NOT a differential for it.** `tests/at_onboarding_cli_functional_tests` ran green — 19 tests, 1m34s, all five files, the approve path exercised — but the 45-second bound added on 2026-08-29 means it would have passed before the deletion too, just slower. It establishes **no regression**, not that the deadlock is gone; and the barrier's own warning is absent because the code that emits it was deleted, which is a control drawn from the property under test. **The proof is the unit guard**: `pq_client_bootstrap_test.dart`'s *"a signer answers while a startup step is still parked"* parks the sweep so the startup is genuinely mid-flight and has not reached the mint, then asserts a signer returns. Mutation-proven — making `signingKeys` await anything reddens exactly that test, quoting its own reason, with the other sixteen green.<br><br>✅ **The e2e pack IS run**: 62 passed at base port 26000, including commit 6's `long_lived_atsign_guard_test.dart` change. ⛔ **Its approvers need NO posture change** — measured, not assumed: the three retrofit files pass with their approvers at `PqPosture.legacy`, because those enrolments carry their own wrapped key and the refusal keys on the absence of one. Moving them breaks `TestPreferences`' one-posture-per-atSign memoisation, which is what a first attempt did. | ⛔ **gkc, and an at_server change.** Only commit 7 remains and it is NOT pickable: [its spec](#the-atserver-change-commit-7-needs) question 6 is unresolved — nothing caps the legacy root credential after a pre-enrollment retrofit — and the change lands in another repo |
| **the `_apsk`-mismatch refusals have no test coverage** | Measured 2026-08-30 with a working positive control: none of the three refusal messages appears in any `*_test.dart`. A throwaway probe that day confirmed both flavours refuse correctly — control stamped true, probe false, and the warning named the key mismatch rather than an incidental cause — and was deleted. ⚠️ **A conveyed link is bound to the EXACT `_apsk` string**: five whole-string comparisons in `pq_signing_chain.dart`. That is why the mint must be a no-op at first start, or it breaks the link the approver just conveyed. A real test should land with the chain work. | Nothing |
| **`apkamPublicKey` names the whole `_apsk` value, not a key** | Both link flavours read the child's entire `_apsk` record value and put it verbatim into a payload field spelled `apkamPublicKey`. Under the specified behaviour that value is always a **data signing** key, so the name is actively misleading. ⚠️ **It is a field name inside a SIGNED payload** — the preimage is `at-root-link:` plus the JSON — so renaming it changes what verifies and is a wire break, not a rename. | Nothing. It rides the chain work |
| **"withdraw" means two contradictory things, and ruling 126 rests on one of them** | ⛔ **Agreed with gkc 2026-08-30.** The word is used in two senses that contradict each other. **Retained**: `withdrawnSigningKeys`, `withdrawnSigningKeysFor`, `apskEntries`'s `withdrawn:` parameter, and ruling 126 at 12939 — an entry that STAYS advertised with status `retired`, so what it signed still verifies. ✅ **Removed — all four sites are gone as of the barrier deletion, 2026-08-30.** They were `signing_key_mint_barrier.dart` line 15, `apkam_signing.dart` line 176, `pq_client_bootstrap.dart` line 287 — and a **fourth this row missed**, `pq_client_bootstrap.dart` line 129 in the step-list dartdoc, which the "two of the three go with the deletion anyway" arithmetic left behind — *"publishing a minted key withdraws the authentication key from the advertisement"*, meaning it stops being named at all. ⚠️ `apkam_signing.dart` carries both senses **thirteen lines apart**: line 272 uses the removed sense inside the dartdoc of the getter whose name is the retained one. <br><br>**Why it matters beyond tidiness**: ruling 126 at 12935 rests the barrier deletion on *"It never **withdraws** a key anything could have signed with"*. In the retained sense that is trivially true; in the removed sense it is the actual, contestable claim. The word carries the argument and is ambiguous exactly there. <br><br>**What remains owed is the RETAINED sense only**: `withdrawnSigningKeys`, `withdrawnSigningKeysFor`, `apskEntries`'s `withdrawn:` parameter and their dartdocs still say "withdraw" where they mean **retired** — an entry that stays advertised. No site now uses the removed sense, so the ambiguity that carried ruling 126's argument is gone; what is left is a rename for clarity, not a correctness item. | Nothing |
| **acceptance clauses for the data-signing-key design just settled** | ⛔ **Raised by gkc 2026-08-30, while the nine-commit programme was being built.** The design settled that day changed what the tree is *supposed* to do in eleven places, and [`acceptance.md`](acceptance.md) states none of them. A behaviour with no clause is a behaviour the burn-down cannot count and the next reader cannot check, so this is owed **beside** the code, not after it.<br><br>**What needs clauses, each already decided and most already built:** (1) an enrolment that advertises a data signing key **files its private half** — all three creation paths; (2) the algorithm minted is **the one the enrollment keeps** — rsa2048 at pqReady, ML-DSA-65 at pqActive — so the first start&#39;s reconciliation is a **no-op** and `_apsk` is not rewritten; (3) a conveyed link is bound to the **exact `_apsk` string**, so a republish at first start would break the link the approver just conveyed; (4) a legacy enrollment&#39;s authentication keypair signs data **in memory only** and is never filed as signing material; (5) `reconcileSigningKeys` treats **rsa2048** as held when the enrollment has no typed signing material and its authentication keypair is rsa2048 — **scoped to rsa2048**, with a control proving pqActive still mints ML-DSA and retires rsa2048; (6) no mint at all when there is no enrollment id; (7) **no signer waits on a mint** — the barrier is gone; (8) `AtClientPreference` **refuses** an empty signing set with non-rsa2048 authentication, and **refuses** any axis weaker than its posture; (9) a **pre-enrollment** atSign retrofits at a PQ posture rather than minting into `_apsk.primary`; (10) a client with `!configuresPqProviders` **neither approves a pq request nor sweeps**, and throws **before** the atServer call so the enrollment stays pending; (11) a fully privileged approver holding **no root private but a data signing key** conveys a **chain** link — the three-arm rule.<br><br>⚠️ **Open each production path before writing its clause.** A clause can be **false** rather than untested, and every instrument reports both the same way; the fix for a test failing against a false clause is to weaken the assertion, which enshrines the wrong behaviour as the spec. Three of six clauses examined closely on 2026-08-27 asserted the opposite of the tree.<br><br>⚠️ **The `_apsk`-mismatch refusal paths still have no test coverage** — that is its own row above, and its clause belongs in this sweep.<br><br>**Done when** every item above is a THEN clause in the catalogue with a `provenIn` citation the ledger accepts, and the burn-down denominator has moved to match. Re-derive the meter rather than quoting it: `dart test test/acceptance/docs_structure_test.dart --concurrency=1` and the acceptance suite&#39;s own `BURN-DOWN` line<br><br>⛔ **gkc, 2026-08-30: the live proofs go in `tests/at_onboarding_cli_functional_tests`** unless there is an argument against, and the catalogue already agrees — [section 14](acceptance.md#14-test-harness--implverify-mapping) records that **no `provenIn` citation reaches either CLI pack**, while naming it *the only place a client is built from a `PqPosture`*. Enrolment creation, the first start&#39;s no-op, approval and conveyance are all end-to-end acts, and that pack is where an atSign is actually enrolled. ⚠️ **It cannot run beside the main functional pack — they share ports.**<br><br>**One carve-out argued for, not assumed:** item (8), `AtClientPreference`&#39;s two coherence refusals, is a **constructor throwing before any I/O**. A live arm would spawn a CLI to watch an `ArgumentError` and would prove less than a unit test does, more slowly — there is no wire, no atServer and no second process in the mechanism. It stays unit, and is recorded here as a decision rather than an omission. Items (6) and (9) are **parked** pending their own rulings: (6) contradicts a mechanism `signing_key_minting_test.dart` currently specifies, and (9) is refused by the atServer as it stands. | Nothing. The clauses can be written against the design; each lands with, or straight after, the commit that builds it |

### P1 — must do before D1 closes

| Item | What is owed | Blocked on |
| ---- | ------------ | ---------- |
| **the at_persistence_secondary_server override must come out** | The workspace root carries a `dependency_overrides` entry pointing `at_persistence_secondary_server` at at_server's `gkc-multiple-stores` branch, which fixes [ruling 125](detail/decisions.md#125-a-hive-stores-identity-is-its-storage-path-not-its-atsign-2026-08-28) — `hiveStoragePath` was silently ignored for the second AtClient of an atSign in one process, so co-located clients shared one keystore and one sync queue. **Three things are owed, in order:** that branch merges to at_server trunk; at_persistence_secondary_server publishes (pub.dev tops out at **5.2.1** while trunk declares an unpublished 5.3.0); then the override is deleted here and the constraint raised past whatever version carries the fix. ⚠️ **`pubspec.lock` is gitignored in this repo**, so the committed override pins the BRANCH rather than a sha — every machine and every CI run resolves its head, and a force-push there changes what this tree compiles against. ⚠️ **at_client's own half is already on this branch and does not depend on the publish**: `AtSyncQueue` takes a storage path, and at_client tests close `HiveInstances.closeAll()` as well as the global `Hive.close()` — count them with `git grep -l 'HiveInstances.closeAll' -- packages/at_client/test \| wc -l` rather than from a figure here, which said **four** and was already five. Those stop compiling the moment the override is removed without the published fix behind it <br><br>⚠️ **The fix exposed three tests that were green for the wrong reason, all now corrected — do not "simplify" them back.** `pq_advance_ladder_test` gave every rung its own storage path, so its central claim — *nothing an install wrote stops being readable when it upgrades* — was only ever asserting that one store reads its own writes; it now uses **one** directory across rungs, as an install upgrading in place does. `pq_posture_grid_test`'s *"each with its own store"* checked that a DIRECTORY existed, which was true under the bug because the encryption-secret `<sha>.hash` lands at the path a client names even when its box opens elsewhere; it now asserts a `.hive` file. And `side_by_side_storage_test`'s own sync-queue arm read an in-memory replay taken at open, so it passed with the queue back on the global instance until the second client was built AFTER the first's write. ⚠️ `sync_multiple_client_test` drives two clients of ONE atSign at two paths and was therefore driving one store — it needed no change and still passes, which is the evidence that real sync works | at_server `gkc-multiple-stores` merging, then a publish |
...
### The atServer change commit 7 needs

**Not a ruling — a proposal, and the open questions are marked.** Written
2026-08-30 from `origin/trunk`, named because a sibling checkout is a claim
about whatever branch it is on.

**The blocker, measured.** A pre-enrollment atSign authenticates with the flat
`at_pkam_publickey`. `pkam_verb_handler.dart:72-76` takes the no-enrollment-id
branch, sets `AuthType.pkamLegacy` and leaves `enrollmentId` null on the
connection. `enroll_verb_handler.dart:406` gates the self-enrolment
auto-approve on `authType == AuthType.apkam`, and `:410-414` refuses a null
parent id outright — so such a connection cannot enter that branch at all. It
falls to the `else` at `:327-331`, gets `preventDuplicateEnrollRequest`, and
lands **pending**, waiting for an approval nobody can give.

⛔ **The apkam branch cannot simply be widened, because four of its mechanisms
are built on having a parent enrollment**, and a pre-enrollment atSign has
none: `verifyNoEscalation(parent.namespaces, …)` (`:430`), the
`parentEnrollmentId` that makes revocation cascade (`:436`), the key-expiry
posture inherited from the parent (`:439-462`), and the cap applied to the
parent afterwards (`:488`).

**The closer analogue is the CRAM branch** (`:371-397`): no parent, grants
`{__manage: rw, *: rw}`, auto-approved, `_publishApskSigningKey` called the
same way. And the authorisation argument is the same shape — a `pkamLegacy`
connection has proved possession of the atSign's **own** root PKAM private,
which is strictly stronger than any enrollment's credential. Auto-approving a
self-enrolment from it is at least as safe as auto-approving one from CRAM,
which the atServer already does.

**What the spec must settle:**

| # | Question | Proposed | Confidence |
| - | -------- | -------- | ---------- |
| 1 | May a `pkamLegacy` connection self-enrol, auto-approved? | Yes, on the argument above | high |
| 2 | What grants? | `{__manage: rw, *: rw}`, as CRAM gives — the atSign's owner is fully privileged by definition, and there is no parent to bound it | high |
| 3 | Does it overwrite `at_pkam_publickey`, as the CRAM branch does at `:387-389`? | **No.** That field IS the legacy credential; overwriting it would lock out every sibling clone of the keyfile mid-migration | high |
| 4 | Is `preventDuplicateEnrollRequest` skipped, as for apkam? | Yes — same reasoning: it is one app re-enrolling itself, and sibling clones share `(appName, deviceName)` | medium |
| 5 | A new `AuthType`, or reuse `pkamLegacy`? | Reuse; the discriminator is `authType == pkamLegacy && enrollmentId == null`, which is exactly the state | medium |
| 6 | ⛔ **What caps the legacy root credential afterwards?** | **UNRESOLVED — this is the crux** | — |

⚠️ **Question 6 is the one to answer first, and nothing in the tree answers
it.** The apkam branch caps the parent so the legacy credential retires a grace
period after the last clone upgrades. Here there is no parent record to cap, so
on the obvious implementation the flat root credential stays valid **forever**
alongside the new enrollment — a permanent, unexpiring, fully privileged
credential that the retrofit was partly meant to retire. The options are to
accept it (the atSign's owner holds that key anyway), to cap
`at_pkam_publickey` itself by some new mechanism, or to say a pre-enrollment
retrofit is not a security boundary move at all and document it as such. That
is a decision, not a derivation.

**Client-side, nothing is owed until this lands.** `_settleEnrollmentIdentity`
returns at its `enrollmentId == null` guard, which is
[ruling 127](detail/decisions.md#127-a-client-with-no-enrollment-id-still-mints-and-publishes-its-own-signing-key-2026-08-30)'s
recorded state: such a client publishes its own `_apsk.primary` and keeps
working.
