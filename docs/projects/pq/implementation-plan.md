...
| 3 | **The private selector stops guessing.** `_activePrivate` returns the active private whose public half corresponds to an advertised entry, rather than `.firstOrNull` over the root slots. ⚠️ **Row 4 changed what this row is FOR, and the change is a demotion — do not overstate it when building.** Row 4 made every write path leave exactly one active private: `store` refuses a second when it has no correspondence verdict, retires every other when it has one, and `_storeFreshPair` is reached only after the held private is retired or republished. So `.firstOrNull` is not currently answering by iteration order — there is nothing to iterate. This row makes the selector independent of that invariant rather than fixing a live defect, and a test for it has to construct two actives by hand because no path produces them | ⚠️ Four callers ask "do I hold the root" through it — `privateHalf`, `store`'s already-held guard, `requestPrivateIfAbsent`'s cheapest check and `hydrateStore`. With two generations that question has an answer *per entry*, and `.firstOrNull` answers it by iteration order |
...
- ⚠️ **`design.md:290` and `acceptance.md:1115` were assigned to row 1 and the
  assignment was wrong** — re-read 2026-08-15 when row 1 landed without
  touching them. Both sentences ("it never rotates, so it stays immutable
  create-once"; "The root never rotates, so a split …") describe the record's
  **immutability**, which row 1 and row 2 leave exactly as it was. They move to
  **row 6**, beside the two lines already assigned there, and they are correct
  until it lands: this block's own rule is that a description of what is built
...
|---|---|---|---|
| 1 | **14.22 rows 3, 5, 6, 7** — the signing root becomes an ordinary, rotatable signing key | [14.22](#1422-making-the-signing-root-rotatable--decisions-101) | **Rows 1, 2 and 4 landed 2026-08-15.** Row 1 was the reader, the only D1 item with a deadline of its own — due before the GA minor. Row 4 was pulled ahead of row 3 by gkc's ruling. **Row 3 is the next action**: `_activePrivate` stops answering "do I hold the root" with `.firstOrNull` over the slots — row 4 made two active privates producible, so the selector now has a real case to get wrong |
