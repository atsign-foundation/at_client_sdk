...
**G5. Close 14.19 item 36** — clauses of UC-A2.5/UC-A2.6 that the catalogue
asserts and no live row proves: A2.5's superseded-kpid envelope and the peer
negotiating its own preferred key, and A2.6's revoked-E4 state gate. ⚠️ This
read "three clauses … it was found by hand", and the finding-by-hand half is
now false: the ledger's clause level computes them, so `tools/acceptance_ledger.sh`
re-derives the list rather than a reader re-reading the catalogue. **The work
is still owed** — computing a gap is not closing it. Start with A2.6's state
gate, which is the cheapest: `key_package_amendment_live_test.dart` has no
revoked arm at all (`grep -ci revoke` returns 0 there, against 53 for
`enroll`), and revoking E4 then re-sending `enroll:update` is one more arm in a
file that already holds the accepted and refused ones.
