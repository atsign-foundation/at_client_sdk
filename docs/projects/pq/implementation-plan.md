...
| **at_onboarding_cli has no local functional harness** | Moved into this list 2026-08-27 — it was reachable only from `MEMORY.md`. [`tests/at_onboarding_cli_functional_tests`](../at-lookup-consolidation/plan.md#blocks-the-major-partly--at_onboarding_cli-had-no-local-functional-harness) has **no `runLocal.sh`**, and its compose file defaults to the published `atsigncompany/virtualenv:vip` rather than the local PQ-capable build everything else here is verified against. **So nothing in at_onboarding_cli can be functionally verified locally at all**, and `tests/at_functional_test` does not exercise the CLI — its authenticator install is **unit-green only** (54 tests). ⚠️ **Not purely a post-D1 concern**: at_onboarding_cli is the *last* package in the release-train publish order, so this is the one package in that order with no local live check. The cheap thing is a `runLocal.sh` matching at_functional_test's and defaulting to `at_virtual_env:local` — before any migration of its six construction sites, not after | Nothing |
