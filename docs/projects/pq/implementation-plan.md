...
|---|---|---|---|
| 1 | **14.22 row 7** — the differential test that proves D1's boundary | [14.22](#1422-making-the-signing-root-rotatable--decisions-101) | **Rows 1-6 have all landed (2026-08-15)**, plus a mint-race fix an adversarial review turned up. Row 6 made the record mutable behind `_rootlock@<atSign>` and generalised `NskeyMintLock` into `MintLock`. **Row 7 is the next action, and it is the last of the set**: a keyfile and a record each carrying two root entries, one active and one retired, with a root link signed under the **retired** one still verifying. ⚠️ Row 5 already built the tolerant verifiers and row 4 the tolerant heals, so check what is left before scoping it — this may be mostly a test rather than a build, which is precisely the claim it exists to settle |
