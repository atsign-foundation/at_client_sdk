...
| **the UC-G2.9 architecture guard is unwritten** | Ruled 2026-08-31: c1 and c2's absences get a **source-shaped** guard in `architecture_guard_test.dart` — `verifyEnvelope` takes no accept parameter, and `AtClientPreference`'s algorithm-typed field set is exactly the six that exist — cited from the row with `provenIn` carrying **no** `clauses:` list. ⛔ **Not a behavioural tripwire**: step 3's lever must ship permissive-unless-narrowed or it would refuse every stored envelope on its release day, so an assertion that the weak algorithm is accepted would stay GREEN through the exact event it exists to catch | Nothing |
| **three B-cluster clauses are one arm short each** | Found by the 2026-08-31 triage and each closable cheaply. **UC-B1.2** c1 — `retrofit_e2e_test.dart` establishes six of its seven arms live and never asserts the successor's APKAM **algorithm**; B1.1 and B1.3 both assert it, B1.2 does not, so a `selfRetrofit` that ignored `signingAlgo` and minted RSA passes unchanged. One `expect`, plus a second citation to `signing_root_pull_two_enrollments_test.dart` for the public/private correspondence arm, which is proven there and cited by no row. **UC-B0.1** c1 — five arms are proven live against the pinned pre-PQ image, which CI already runs as `legacy_server_tests` at `-t legacy-server`; what nothing exercises is the parenthetical about the atServer's immutable write being present even there. One `expect` in a file that already holds an authenticated owner client. **UC-B5.1** c1 — the headline is proven and merely unpinned, and the nskey half is proven twice over and uncited (`nskey_self_heal_test.dart`, `nskey_self_heal_live_test.dart`). Two arms genuinely owed: the **no-push absence**, which belongs in `architecture_guard_test.dart` since it is a claim about which code path exists; and *"persists until one answers"*, which is ⚠️ **narrower in production than the clause says** — the request is an envelope with a seven-day `envelopeTtl`, and what actually makes it persist is the every-start re-broadcast. ⛔ Also: `b5_edge_cases_test.dart`'s scenario comment still describes the **struck** push-primary text, so it documents a clause that no longer exists | Nothing |
