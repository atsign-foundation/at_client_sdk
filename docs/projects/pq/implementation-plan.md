...
| **Delete the signing-key mint barrier, and stop a legacy enrollment minting a second data signing keypair** | ⛔ **Ruled by gkc 2026-08-30, [ruling 126](detail/decisions.md#126-the-mint-barrier-is-deleted-legacy-authentication-and-data-signing-are-one-keypair-2026-08-30), which supersedes [ruling 114](detail/decisions.md#114-a-signer-waits-for-its-own-mint-the-mint-alone-does-not-superseded). The ruling carries the reasoning; this row is the work.** <br><br>**1. Delete the barrier.** `packages/at_client/lib/src/signing/signing_key_mint_barrier.dart` entire; `ApkamSigning.awaitsSigningKeyMint` and its overrides in `SigningKeyMinting` and `apkam_signing_keys_test.dart`; the wait at the head of `ApkamSigning.signingKeys`; `PqClientBootstrap._mintSettled`/`mintSettled`, its `registerSigningKeyMintBarrier` call and the `finally` that settles it; `packages/at_client/test/signing_key_mint_barrier_test.dart`; and the `signingKeyMintWait` bound plus its CHANGELOG entry, which described a mechanism that is now going. <br><br>**2. A legacy enrollment never mints a data signing keypair.** In `SigningKeyMinting.reconcileSigningKeys`, exclude from `missing` any algorithm the enrollment's **authentication** keypair already satisfies, **and only when `heldSigningKeys` is empty**. `authenticationSigningKey` is the data signing keypair on a legacy enrollment, and `AtKeys.signingKeysFor` cannot see it because it reads typed material only. ⚠️ **The scope is the whole correctness of this change** — `authenticationSigningKey.algorithm` is `signingAlgoOf`, the recorded resolution, so on a retrofitted enrollment it reports mldsa65; unscoped, the exclusion stops `pqActive` minting its ML-DSA signing key while still retiring rsa2048, and nothing goes red. File nothing: skipping the mint is enough, and the ruling records why adopting was rejected. **Land both halves in one commit** — rule 1 alone is unsafe, which is why the ruling says the second is why the first is safe. <br><br>**Sweep in the same commit** (unscoped, tracked and untracked): `signingKeyMintWait`, `awaitSigningKeyMint`, `registerSigningKeyMintBarrier`, `signingKeyMintSettled`, `awaitsSigningKeyMint`, `mintSettled`, `signing_key_mint_barrier` — re-derive the file list rather than trusting a count (`git grep -l --untracked -E '<the seven terms>'`), and note `detail/implementation-plan.md` carries none of them yet cites ruling 114 as live in two places. Includes `acceptance.md`'s row for `pq_client_bootstrap_test.dart` (it names *“when `mintSettled` settles”* as something that file proves) and **two** unreleased CHANGELOG entries under `3.15.0-rc1`, not one: the `signingKeyMintWait` bound, and the larger entry announcing the barrier mechanism itself (*“a signer can no longer sign with the authentication key in the window …”*). Shipping an rc that announces a mechanism which does not exist is the cost of missing the second. Also correct `_mintInUseSigningKeys`' dartdoc, which claims it *“runs before every step that signs — … and the sweep”* and does not: the sweeps are steps 2 and 3, the mint is step 4. <br><br>**Tests**: `pq_client_bootstrap_test.dart` loses its `mintSettled` cases and needs one asserting no step waits on any other; a new case must pin that an enrollment holding no typed signing material, whose authentication keypair supplies the wanted algorithm, mints nothing and leaves `_apsk` byte-identical. ⚠️ **A legacy enrollment at `pqReady` does not reach `reconcileSigningKeys`** — it retrofits at `_init` first — so construct one of the two states that do: a swallowed `_settleEnrollmentIdentity` failure, or `authenticationKeyAlgorithm` overridden to rsa2048 under `dataSigningKeyAlgorithms: {rsa2048}`. **The control that must stay green is a retrofitted enrollment on `pqActive`**, which still mints mldsa65 and retires rather than drops rsa2048 — that control is what catches an unscoped exclusion. <br><br>**Proof it is fixed**: `at_activate approve` exits under its two-minute bound in the `tests/at_onboarding_cli_functional_tests` pack. It was killed at that bound in eight separate runs. ⛔ **Do NOT run `at_activate approve` from a shell as the proof** — standalone it completes in 4-9s at every commit tested, fixed or not, because the sweep only signs when a peer has left a pending request and standalone there is none. That false green was written into this row once already and corrected in `275af363c`. ⚠️ That pack cannot run beside the main functional pack; they share ports. | Nothing |
| **An enrollment created by `sendEnrollRequest` has no data signing keypair, so its own first mint strands its key package** | ⛔ **Raised and evidenced 2026-08-30 while settling [ruling 126](detail/decisions.md#126-the-mint-barrier-is-deleted-legacy-authentication-and-data-signing-are-one-keypair-2026-08-30); gkc set the requester aside to settle the approver first and it was not returned to. Ruling 126 does NOT fix this.** <br><br>**The mechanism, from source.** `AtOnboardingServiceImpl.sendEnrollRequest` builds `AtEnrollmentRequest.pq(...)` and never sets `advertisedSigningKey`, where `selfRetrofit` does (`self_retrofit.dart`, the `_freshRsaSigningKey()` it passes as `advertisedSigningKey`). `EnrollmentSubmitter._apskFor` therefore takes `pub = advertisedSigningKey?.publicKey ?? apkamPublicKey` and advertises the **authentication** keypair. The enrollment's key package rides `enroll:request` as `EnrollParams.metadata`, signed at enrollment time by that same authentication keypair — correctly, since it is what `_apsk` names. Then the enrollment's first client start mints a data signing keypair for the in-use set, publishes it, and `apskEntries` drops the authentication keypair (it is added only `if (entries.isEmpty)`). The key package, durable in the enrollment record, verifies against nothing from that moment. <br><br>**Measured 2026-08-29**, one `pq_native_enroll_test` run against `@curtly`, enrollment `e20285f6` submitted as mldsa65: `Minted rsa2048 signing key(s) for e20285f6` at `22:01:32.119730`, then **seven** SEVERE records from `VerbEnrollmentDirectory` beginning 48ms later — *“the key package advertised by enrollment e20285f6 does not verify against its `_apsk` … the envelope is signed under \"ML-DSA-65\" and the published `_apsk` advertises \"rsa2048\" — no algorithm in common … not sealing to it”*. The consequence is that no peer will seal to that enrollment at all, so it receives no conveyed material. <br><br>**What is NOT established**, and must not be assumed: whether any non-test caller reaches this. The test uses the shipped API rather than a hand-built request, so the API does produce the state — but which postures and app flows actually hit it has not been enumerated. <br><br>**Two candidate fixes, for the ruling.** (1) `sendEnrollRequest` mints and files an `advertisedSigningKey` the way `selfRetrofit` does, so every enrollment satisfies gkc's invariant — *each enrollment has an authentication keypair and at least one data signing keypair* — from birth. (2) Retain the authentication keypair in `_apsk` as withdrawn. ⚠️ Ruling 126 rejects (2) **for legacy enrollments** on the grounds that it forces the JSON array where a bare rsa2048 value would do — that objection does **not** transfer here, because an enrollment whose authentication keypair is mldsa65 already advertises the array. The rejection must not be cited against (2) in this row without re-deriving it. | A ruling on which of the two fixes is right, and on whether a non-test caller reaches this at all |
