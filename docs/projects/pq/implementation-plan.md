...
| **`pq_native_enroll_test`'s retrofit case hangs in the approve it runs as a child, and a standalone `at_activate approve` does not** | ⚠️ **This row claimed something false until 2026-08-29 and the correction is the point.** It said `at_activate approve` *never terminates* under `--key-exchange pq` and that this **reproduced from a shell prompt with no test involved**. It does not. Re-measured against virtualenvs where no approve had previously been killed, a standalone `at_activate enroll --key-exchange pq` followed by `at_activate approve` completes with exit 0 in **4 to 9 seconds at every commit tested** — the last-known-good `53608de6d`, the legacy-posture `3406e4a6e`, the guard `187924c4a`, and head. Every earlier hand reproduction that hung ran on a virtualenv the pack had just left with an approve killed mid-conveyance. The interleaved key-exchange differential (2 of 2 `pq` hanging against 2 of 2 `legacy` exiting under 5s) was internally valid but ran on that polluted state, so **`pq` is a necessary condition, not a sufficient one**. <br><br>**What is actually established.** Bisected on a fresh virtualenv per point, prompted by gkc's insistence that something changed: at `53608de6d` the pack is **+19 All**; at `187924c4a` — the storage-path guard, with the fixture untouched — it fails at **`setUpAll`**, `Expected: <0> Actual: <1>` on the `spp` command, which is the guard refusing a client reuse. So the guard is what turned the pack red, and it does so before any test runs. `ae8c6f342` (each CLI command in its own process) removes those refusals and leaves the pack at 18/1, with the retrofit case's approve hanging. <br><br>**The hang itself, as observed.** Traced with markers proven to fire by a unit-test control: the child stops between sealing the conveyance envelope and signing it, in `shareSecretWith` → `sendEnvelope` → `wrapAndSignAndJsonEncode` → `wrapAndSign` → `await signingKeys` → `await signingKeyMintSettled(client)`. That barrier is a `Completer` registered by `PqClientBootstrap`'s constructor and settled by `mintInUseSigningKeys` (step 4 of 12) or by `startup()`'s `finally`; **nothing awaits it with a timeout**. Since it blocks, step 4 never ran, so `startup()` is stuck in step 1, 2 or 3. At `finest` the hung child logs 188 lines and nothing at all after the 15-second mark. ⚠️ **Run the CLI with `--debug`** — `auth_cli.dart:183` sets the log level to `shout`, so a silenced child reads exactly like a stalled one. <br><br>**What is owed**: explain why that approve hangs as a child of the test process while the identical command standalone does not. Candidates not yet separated: concurrent clients of the same atSign held by the test process, and state left by a previously killed approve. ⛔ **Do not restate the shipped-CLI framing** — four hypotheses about this hang have already been disproven, and the fifth was the claim that it reproduced standalone. | Nothing. Both arms reproduce in about three minutes each |
