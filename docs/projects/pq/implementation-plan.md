...
| Item | What is owed | Blocked on |
| ---- | ------------ | ---------- |
| **at_server now revokes a non-root retrofit predecessor at the successor's first authentication** | at_server [#2797](https://github.com/atsign-foundation/at_server/pull/2797) replaces the 30-day cap with immediate revocation: at the successor's first `pkam:` the predecessor's approval children are re-parented onto the successor and the predecessor is revoked with a `superseded` history event; every other holder of its keyfile is refused AT0027 on its next command; the stamp is renamed `predecessorSettledAt` (decoder reads both at rest, wire emits the new name only); the predecessor no longer gets an `expiresAt`; `apkamSelfEnrollmentGraceHours` is gone. This tree pins the cap: `tests/at_end2end_test/test/pq/retrofit_cap_value_e2e_test.dart` (UC-B1.1) and `retrofit_retirement_e2e_test.dart` (UC-B2.1/B2.2), and `at_client_impl.dart`'s comment "the atServer caps a parent enrollment on success". Adapt those, grep the tree for `predecessorCapArmedAt` read off the wire, and amend `acceptance.md`/`design.md` where they state the cap as the contract. **#2797 merged to at_server trunk as `57212824` on 2026-09-05; `dev_env` rebuilt 17:19Z, so this is now pickable.** The functional pack's `tests/at_functional_test/test/self_enrollment_retrofit_live_test.dart` is also affected — three of its cases already fail against the new server with `AT0027 ... revoked`, because they authenticate the predecessor after retrofit, which the new server refuses. Adapt it alongside the e2e cap tests. ⚠️ **Confirmed pre-merge on 2026-09-07 by a two-arm differential** — the same three cases fail with the same messages on `gkc-pq-d1-spike-backup-premergeback-20260907` and on the merged spike, against one `at_virtual_env:local` whose binary carries `predecessorSettledAt` and no `apkamSelfEnrollmentGraceHours` — so they are this row's, not the merge's. One more site to adapt: `self_retrofit.dart`'s dartdoc still says the legacy enrollment "keeps authenticating until the atServer's expiry cap retires it". ✅ The functional pack's three cases were adapted 2026-09-07 (own keyfiles per arm; the published key read before the successor authenticates). Still red under this row, all three failing on the pre-merge backup too: `retrofit_cap_value_e2e_test` (the stamp), `retrofit_retirement_e2e_test` (expects the cap's exception, gets `AT0027 … revoked`), and the CLI pack's `pq_native_enroll_test` "a legacy enrolment that retrofits at start can still run a verb" — `at_activate list` re-authenticates as the revoked legacy id after the client retrofitted itself at start, which the 30-day cap used to hide. | Nothing |
