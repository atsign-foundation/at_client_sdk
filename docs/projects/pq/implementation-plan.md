...
| Item | What is owed | Blocked on |
| ---- | ------------ | ---------- |
| **ruling 134 leaves two things owed in the tree** | **Ruled 2026-09-08**, [134](detail/decisions.md#134-a-posture-move-replaces-the-enrollment-so-the-authentication-key-is-never-retained-2026-09-08): a posture move replaces the enrollment, `primary` is legacy-only, and the authentication key is never retained. Two consequences are unbuilt. ⓵ **[Ruling 127](detail/decisions.md#127-a-client-with-no-enrollment-id-still-mints-and-publishes-its-own-signing-key-2026-08-30) is superseded and its test still pins the opposite.** `signing_key_minting_test.dart`'s *a client with no enrollment* group asserts that such a client mints ML-DSA, publishes the record itself and files under `primary` — a state 134 says cannot exist, because reaching a PQ posture retrofits first. ⛔ **A test is the specification of the mechanism it guards, so read its `reason:` lines before touching it**: rewrite it to pin that a credential with no enrollment id retrofits rather than minting, and check what else in that file rests on the minting path. ⓶ **The pinned-axis case is knowingly unfixed and nothing says so in code**: `authenticationKeyAlgorithm: rsa2048` beside `dataSigningKeyAlgorithms: {mldsa65}` takes no retrofit, mints ML-DSA and drops the RSA key that signed everything. Either refuse the combination at construction, where the preference already refuses others, or document it where a caller setting both axes will read it. | Nothing |
