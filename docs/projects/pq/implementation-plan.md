...
**Start here, and it is startable now:** build **arm 1**, the 3-cell stage arm,
in `tests/at_functional_test`. `TestUtils.getPreference(atSign, posture:)`
already takes a posture, so the arm is 3 clients and their assertions, and it
executes UC-C1.2 for the first time — `disallowLegacyEncryption` has no setter
and no constructor argument, so a posture is the only way to reach it, and it
has 0 hits under `tests/`. Its one real prerequisite is that this pack has no
`dart_test.yaml` and no `pq` tag, so `dart test --tags pq` selects nothing
there. ✅ The prerequisite that read "`PqPosture.pqActive` breaks the monitor"
...
The ledger is the second piece and needs `manifest.dart` moved from
`packages/at_client/test/` to `lib/` before any pack can import it.
