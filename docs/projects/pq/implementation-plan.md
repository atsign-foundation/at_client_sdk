...
| **The acceptance burn-down understates by ~5x, and the figure is now re-derived rather than estimated (2026-08-04).** It reads **4 of 43** green, 39 skipped — confirmed by running it. Counting the skips by blocker constant: `ss2` **3**, `ss4` **7**, `b1`+`b1CrossAtSign` **12** — all **22** blocked on work that is now **complete**. Genuinely still blocked: `r1`+`r1CrossAtSign` **6**, `b2` **4**, `rfSrv`+`rf2b` **6** (off the GA path), `on1`+`on1CrossAtSign` **2**. So the honest reading is ~**26 of 43** claimable against this branch, not 4. Two independent causes remain: (a) `catalogue_test.dart` runs in at_client's unit suite and cannot see `tests/at_functional_test` or `tests/at_end2end_test`, where much of the proof lives — a live-proven scenario can never turn its own row green; (b) `blockers.dart` still defines `ss2`, `ss4` and `b1`, whose projects are done. Its own dartdoc describes the fix: delete the constant and let the analyzer point at what it unblocked |
