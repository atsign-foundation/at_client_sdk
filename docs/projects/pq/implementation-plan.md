...
| arm 2's flake rate is not re-bounded | ⚠️ `pq_posture_grid_test.dart` was **4 of 5** green before 2026-08-24's fix — three isolated runs and one full pack green, one full pack red on two cells. Both causes were the test's, not the product's: the envelope test re-read a record sealed to the receiver's namespace key once per receiver, dragging the nskey **conveyance race** into a claim about verifiers; and the notification cell asserted delivery absolutely against a monitor that drops at a measurable rate (14.34's family). Both are fixed — the envelope reads once by the holder and every posture's verifier runs against those bytes, and the notification re-sends once so a drop is absorbed while two independent drops still fail. ⚠️ **The rate is now 1 of 2, and the second run failed in a way neither fix addresses.** A clean `runLocal.sh` on 2026-08-24 (fresh virtualenv, `at_virtual_env:local`, nothing else touching it) gave **179/181 — two cells red in this file, and only this file**:

1. **`UC-G1.15` threw `FormatException` at `pq_posture_grid_test.dart:809`**, jsonDecoding a 44-character base64 string — 32 bytes, a symmetric key, where the record holds a **4,656-character envelope**. **`get` returned it without throwing.** Instrumented and re-run 2026-08-24; what is established:
   - The failing cell is **`s-pqActive`, which is on the nskey path** (`sharedKeyEnc: null`, `appMetadata.providerId = at/symmetric/AES/GCM`, with `ckKid`/`ns`). ⚠️ Not the shared-AES-key path — `s-legacy` and `s-pqReady` carry `sharedKeyEnc` and `providerId: legacy`, and both read back at full length in every run, failing or not.
   - **It is context-dependent, and does not reproduce in isolation.** Measured rates, same virtualenv, interleaved arms: the grid's envelope test **alone 0 of 9**; preceded by the whole of `key_package_amendment_live_test.dart` **2 of 9**; preceded by `secret_sharing_delivery_test.dart` **0 of 3**; preceded by either single test of the amendment file **0 of 3** each.
   - ⚠️ **That does NOT attribute it to the amendment file.** The arms differ in total preceding workload as well as in content, 2-of-9 against 0-of-3 is underpowered, and no single test from that file reproduced it. Read it as a load- or timing-dependent race whose rate rises with preceding work.
   - **The value returned is the CONTENT KEY, and the record is stamped with the conveyance provider.** In both failing runs the value record's `appMetadata.providerId` is **`at/nskey/XWING/AES/GCM`** with `recipientKind: nskey` and an `nskeyKid`; in every passing run the same write produced **`at/symmetric/AES/GCM`**. `NskeyProvider.decrypt` (`crypto/nskey/nskey_provider.dart:263`) ends `return ck.toBase64()` **by contract** — it is the decrypt for `__ck` *conveyance* records, whose ciphertext genuinely is a sealed content key, and `NskeyProvider.encrypt` confirms it by doing `ContentKey.fromBase64(plaintext)`. So **any ordinary data record carrying that providerId decrypts to a content key, silently.** The atServer is not involved: the wire holds the full 6,723-character ciphertext in failing and passing runs alike.
   - ⛔ **Refuted, so do not re-derive it**: metadata aliasing between the value key and the conveyance key. `ckConveyanceKey` (`crypto/nskey/nskey_records.dart:171`) allocates a fresh `AtKey` with its own `Metadata()`, so the conveyance write cannot overwrite the value record's stamp that way.
   - ⚠️ **This entry said "It is the WRITE, not the read", and that was wrong — withdrawn the same day it was written.** The reasoning was: `GetResponseTransformer` sets `atValue.metadata` from `decodedResponse['metaData']` before decrypting, and `prepareMetadata` fills `appMetadata` from that wire map (`at_client_util.dart:113`), so the observed stamp must be the atServer's stored copy of **the value record**. Both halves of the mechanism are true; the conclusion is not, because it assumes the response corresponds to the key that was asked for. **That was never checked** — `decodedResponse['key']` carries the resolved key and no probe printed it.
   - **The put-path sweep argues against a write bug.** `git grep -P "cryptoProviderId\s*="` over `packages/at_client/lib` returns exactly one put that requests an nskey provider — `ck_manager.dart:203` — and it does so on a key derived by `ckConveyanceKey`, which allocates a fresh `AtKey` and `Metadata`. `PutRequestTransformer` always assigns a fresh `AppMetadata` from `CryptoRuntime.providerIdFor`, and `CryptoConfig.defaultProviderId` is a `final` field on a `const` config, so it cannot change mid-run. **No path stamps a value record with the conveyance provider.**
   - **So the leading hypothesis is now that the `get` RESOLVED THE CONVEYANCE RECORD** rather than the value record — which would explain every observation at once: conveyance metadata, a content key as the value, no exception, and nothing wrong on the write side. **Cheapest next probe: print `decodedResponse['key']` (the resolved key on the lookup response) on the failure path**, beside the key that was requested. One reproduction settles it.
   - ⛔ **`AtSignLogger.root_level` will NOT give you the wire traffic, whatever you set it to.** Measured 2026-08-24 while trying to capture `AtLookup`'s frames: `AtSignLogger`'s constructor is `logger = logging.Logger.detached(name)` followed by `level = _root_level` (`packages/at_utils/lib/src/logging/atsignlogger.dart`). **Detached** means the logger has no parent and never inherits from `Logger.root`, so the `Logger.root.level = …` that `root_level`'s setter performs reaches nothing; and the constructor **snapshots** the static level, so raising it later cannot move a logger that already exists. `AtLookup`'s logger is built during setup at `info` and stays there, which is why its `logger.info('SENDING: …')` monitor line appears while `logger.finer('SENDING: $command')` (`at_lookup_impl.dart:1270`) and `RECEIVED` (`connection/outbound_message_listener.dart:196`) never do. The tell is **`FINEST` lines present and `FINER` lines at zero** — impossible for one logger, ordinary for two with independently pinned levels. Measured over 10 runs: 0 `FINER` lines in every one, against 6 `FINEST` as the positive control. **To capture wire traffic, raise the level before the client whose traffic you want is constructed** — and note `TestUtils.initAtClient` assigns `root_level` as its first statement, so it must be after the last such call.
   - ✅ **REPRODUCED 2026-08-24 with a verified wire log** (run 3 of 3, after the level fix; 12,000+ `FINER` lines per run, so the rig was checked before the result was counted). The captured probe:
     ```
     WIRE   provider=at/symmetric/AES/GCM   wireLen=6723
     CLIENT provider=at/nskey/XWING/AES/GCM valueLen=44
     CLIENT additional={recipientKind: nskey, ckKid: 76a93b6bb86f0293, …}
     ```
     **The atServer's record is correct.** The client's `AtValue` carries the *conveyance* record's value (a 44-char content key) AND its metadata. The `ckKid` in that metadata is the one whose `__ck` record was looked up 7 ms after the value lookup — `lookup:all:env…s-pqactive` at `30.277517`, `lookup:all:76a93b6bb86f0293.__ck…` at `30.284289`.
   - ⛔ **The provider layer is EXCLUDED**, by `packages/at_client/test/nskey_reentrant_conveyance_test.dart`. `SymmetricAesGcmProvider._resolveFromConveyance` (`symmetric_aes_gcm_provider.dart:267`) calls `context.atClient.get(conveyance)` re-entrantly from inside the outer value's own decrypt and discards the result, taking the CK as a cache side effect — a path that had **no unit coverage at all**, because every other test of this provider seeds the cache first. Driven directly, the outer decrypt returns the value's plaintext. The test asserts the nested read happened, and mutating `_fromBytes` to `ck.toBase64()` reddens it with the live failure's exact shape.
   - **`AtClientImpl.get` and `GetResponseTransformer` read clean**: a fresh `Tuple` and a fresh `AtValue` per call, no caching, no in-flight coalescing, and the transformer holds only the client.
   - ⚠️ **An earlier reading of the wire log — "two lookups pipelined on one connection" — was NOT supported and is withdrawn.** `CryptoRuntime._context()` passes `_atClient`, and `getRemoteSecondary()` returns that client's own instance, so an outer read and its nested conveyance read share one `AtLookupImpl` and one `requestResponseMutex` — they are serialised, and the overlap that reading described cannot happen. The log identifies no `AtLookup` instance (every one logs under the same name) and **133 connections were live**, so adjacent `SENDING:` lines are not evidence that one client issued both. What does survive is the `ckKid` match: the content key in the failing metadata is the one whose `__ck` record was read.
   - **Still unexplained**: how the outer `AtValue` came to hold the conveyance record's value *and* its metadata when the atServer held the value record correctly. The unit harness cannot reach it — `buildRemoteBackedMockClient` stubs `AtClient.get` outright, so `GetResponseTransformer` never runs under test. Closing that fixture gap is the next concrete step.
   - ⛔ **Ruled out, with evidence — do not re-derive**: a write-path stamp (the wire says `at/symmetric/AES/GCM`); metadata aliasing via `ckConveyanceKey` (fresh `AtKey` and `Metadata`); mutex desync after a timeout (`_process` holds `requestResponseMutex` across send-and-read, and there were **0** timeouts or `Exception in sending` lines before the failure); response mis-correlation across sockets; and in-flight coalescing in `AtClientImpl.get` (there is none).
   - **A 926-line slice of the reproduction's wire log is kept at `untracked/pq_wrong_value_read_2026-08-24.log`** — the window from the failing cell's write through the probe's verdict, with a header giving the capture command, the anchors and the caveat about attributing adjacent `SENDING:` lines. ⚠️ `untracked/` is gitignored, so it is machine-local: a fresh clone does not have it and the recipe above is what re-creates it. The full 68 MB log is gone. Rate with the working rig: **1 of 3**.
   - ⚠️ **It is not reproducible on demand, and the rates are not comparable.** 0 of 12 with a failure-path-only probe; 2 of 9 with an earlier build that printed on every read; 1 occurrence in the clean pack with no probe at all. Three different rigs, so those are three numbers and not a trend — and the virtualenv had accumulated many runs' state by the later batches. Bring up a fresh virtualenv before trying to reproduce.
2. **The `s-pqActive` notification did not arrive on either of two sends**, 150s total, both reported delivered, while `s-legacy` and `s-pqready` did arrive and the monitor saw 19 events. ⚠️ Do **not** read this as posture-dependent: [14.34](#1434-an-unexplained-intermittent-in-self_enrollment_retrofit_live_testdart) measured pqActive at 16/18 against a control's 18/20, both failing at the same rate with `AT0014`. One run cannot distinguish that background rate from a posture effect.

Owed: diagnose (1), which is a defect rather than a rate; then run the pack a few more times and record the rate for (2), or accept it as unbounded and say so | Nothing. `cd tests/at_functional_test && VIRTUALENV_IMAGE=at_virtual_env:local ./runLocal.sh` |
