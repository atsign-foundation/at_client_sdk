...
**Client-side, what commit 7 now is.** `_settleEnrollmentIdentity` returns at its
`enrollmentId == null` guard, which is
...
recorded state. Making it retrofit means: dropping that guard, sourcing
`appName`/`deviceName` (there is no enrollment record to read them from, and the
device name must be per-device), requesting full grants, and self-approving —
at_auth's `_handleSelfEnrollmentRequest` currently denies and throws when the
response comes back `pending`, which is the one place that has to change.
