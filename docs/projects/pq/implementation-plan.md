...
|---|---|---|---|
| 1 | **14.22 rows 3–7** — the signing root becomes an ordinary, rotatable signing key | [14.22](#1422-making-the-signing-root-rotatable--decisions-101) | **Rows 1 and 2 landed 2026-08-15** (`19a31d51b`, and row 2 with this table's own correction). Row 1 was the reader, the only D1 item with a deadline of its own — due before the GA minor. **Row 3 is the next action**: `_activePrivate` stops answering "do I hold the root" with `.firstOrNull` over the slots |
