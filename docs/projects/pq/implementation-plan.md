...
| **The PQ startup deadlocks itself: step 2 signs, and only step 4 releases signers** | ⛔ **Root cause, established 2026-08-29 with trace markers proven to fire by unit-test controls, and observed end to end in one log.** `PqClientBootstrap`'s **constructor** registers a `Completer` — the signing-key mint barrier — and from that moment everything in the process that signs an envelope must await it. Only the startup completes it: at step **4** (`mintInUseSigningKeys`) and, as a backstop, in `startup()`'s outer `finally`. The startup runs 12 ordered steps. <br><br>**The circle.** Step **2** is `collectConveyedKeys` → `sweepOnce`. When that sweep finds an inbound *request* envelope from another enrollment it answers it — `_handleRequestPayload` → `shareSecretWith` → `sendEnvelope` → seal → **sign** — and that sign awaits the barrier, which only step 4 completes, which cannot start until step 2 returns. One isolate, one future, no tie-break. Captured by stack trace at the seal, and every sweep-signs event in the log is followed by the barrier wait elapsing: `987 SWEEP SIGNS → 1069, 1106 BARRIER` then `1158 SWEEP SIGNS → 1236, 1274 BARRIER`. <br><br>**Why it looked state-dependent for a whole day.** The sweep signs only if a peer has left a **pending request** addressed to this client's key package. Standalone there is none, so the sweep returns and nothing signs — which is why a hand-run `at_activate approve` completes in 4-9s at every commit tested, and why an earlier version of this row wrongly called it a shipped-CLI hang reproducible from a shell prompt. In the pack the test process's own clients issue those requests, so the approve child's sweep finds one. <br><br>**The design anticipated the two neighbouring cases and not this one.** `SigningKeyMinting` overrides `awaitsSigningKeyMint => false` because the mint cannot wait for itself; and the barrier's dartdoc says waiting on `startupComplete` instead *"would deadlock — the steps after the mint sign envelopes themselves"*. A step **before** the mint that signs needs neither escape, so nothing catches it. <br><br>**Landed**: `awaitSigningKeyMint` bounds the wait at 45s, returns the elapsed bound, and the signer warns and signs with what the keyfile holds (4 tests, 3 controls, mutation-proven). That converts the deadlock into a delay plus a warning — the file is now **+2 All** — but it does **not** remove the circle. <br><br>**What is owed**: break the circularity itself. Either the sweep must not answer requests before the mint has settled, or answering must not require the barrier, or the mint must precede `collectConveyedKeys` in the step order. That is a design call about which invariant matters more — the barrier exists so a signer never signs with a key the advertisement no longer names. ⚠️ Note that until it is broken, a client in this state still fails to answer the peer request that triggered it, and whatever step 2 would have collected stays uncollected. | A ruling on which of the three ways to break the circle is right |
