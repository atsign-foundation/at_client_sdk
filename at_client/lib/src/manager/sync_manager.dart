...
class SyncManager {
  ...
  Future<void> sync(Function onDone,
    ...
    _regex = regex;
    await syncOnce(onDone, _onError, regex: _regex, isStream: isStream);
    return;
  }
  ...
  void _handleError(var syncManager, Exception e, Function onDone,
      Function onError) {
  ...
  for (var unCommittedEntryList in uncommittedEntryBatch) {
    ...
    for (var entry in batchResponse) {
      try {
        ...
        if (commitId == -1) {
           _logger.severe(
              'update/delete for key ${commitEntry
                  .atKey} failed. Error code ${responseObject
                  .errorCode} error message ${responseObject.errorMessage}');
        }
        ...
        _logger.severe(
            'exception while updating commit entry for entry:$entry ${e
                .toString()}');
      }
    }
  }
...
Future<void> _pullToLocal(VerbBuilder builder, serverCommitEntry,
    CommitOp operation) async {
...
List<dynamic> _getUnCommittedEntryBatch(List<CommitEntry> uncommittedEntries) {
  var unCommittedEntryBatch = [];
  var batchSize = preference.syncBatchSize,
      i = 0;
