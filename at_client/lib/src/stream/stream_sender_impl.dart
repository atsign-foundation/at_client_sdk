...
class StreamSenderImpl extends StreamSender {
  final _logger = AtSignLogger('StreamSenderImpl');

  final chunkSize = 1024;

  StreamSenderImpl(String currentAtSign) : super(currentAtSign);
  ...
  @override
  Future<String> send(AtStreamRequest atStreamRequest, Function onDone,
      Function onError) async {
    final streamId = Uuid().v4();
    var atStreamResponse = AtStreamResponse(streamId);
    try {
      final remoteSecondary = RemoteSecondary(currentAtSign, preference);
      ...
      await _checkConnectivity(remoteSecondary);
...
//          }
  ...
  Future<void> _startStream(RemoteSecondary remoteSecondary, File file,
      String receiverAtSign, int startByte) async {
    var readBytes = 0;
    ...
    try {
      ...
      while (readBytes < length) {
        ...
        final encryptedBytes =
            AESCodec(encryptionKey).encoder.convert(actualBytes);
        remoteSecondary.atLookUp.connection!.getSocket().add(encryptedBytes);
        readBytes += chunkSize;
      }
    } finally {
...
//    }
  ...
  Future<void> _checkConnectivity(RemoteSecondary remoteSecondary) async {
