...
class SyncService {
  ...
  static const changeThreshold = 5, idleTime = 30;

  static const serverCommitIdRegex = ''; //#TODO
  ...
  int _change = 0, _serverCommitId = -1, _lastSyncedCommitId = -1;

  final _logger = AtSignLogger('SyncService');

  SyncService(this._atClient) {
    _localSecondary = _atClient.getLocalSecondary()!;
    _preference = _atClient.getPreferences();
    _atSign = _atClient.getCurrentAtSign()!;
  }
  ...
  void start() {
    ...
    // this ensures sync is run on a dedicated connection
    _remoteSecondary = RemoteSecondary(_atSign, _preference!);
    NotificationServiceImpl(_atClient)
        .subscribe(regex: serverCommitIdRegex)
        .listen((notification) {
      _serverCommitId =
          1; //#dummy impl - setting server commit id = 1 . Listen gives AtNotification. how to get server commit from AtNotification?
      if (_serverCommitId != _lastSyncedCommitId) {
        _change++;
      }
    });
  ...
  void _checkForChange() {
    ...
    if (_change >= changeThreshold ||
        (_lastSyncedTime != null &&
            (DateTime.now().millisecond - _lastSyncedTime!.millisecond) >
                idleTime)) {
      _logger.finer('triggering sync for changes: $_change');
      _sync();
    }
  }

  void _sync({String? regex}) async {
    if (_syncInProgress) {
    ...
    try {
      ...
      if (serverCommitId > lastSyncedCommitId) {
        ...
        await _pullChanges(syncObject.lastSyncedCommitId!, regex: regex);
      }
  ...
  Future<void> _pullChanges(int lastSyncedCommitId, {String? regex}) async {
    _logger.info('Stream sync process started');
    // stream sync set to true by default
    await _remoteSecondary.sync(lastSyncedCommitId,
        syncCallBack: _syncLocal, regex: regex, isStream: true);
    return;
  }
  ...
  Future<void> _pullToLocal(
    ...
    _lastSyncedTime = DateTime.now();
    _lastSyncedCommitId = serverCommitEntry['commitId'];
    if (serverCommitEntry['commitId'] == _serverCommitId) {
      _change--;
    }
  }
