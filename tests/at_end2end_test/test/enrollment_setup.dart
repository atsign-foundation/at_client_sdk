...
void main() {
  ...
  setUpAll(() async {
    for (var atSign in atSignList) {
      await TestSuiteInitializer.getInstance().testInitializer(atSign, namespace, 'pkam', enableInitialSync: false);
    }
  });

  for (var currentAtSign in atSignList) {
    test('A test to submit and approve an enrollment for $currentAtSign', () async {
      // Set SPP at the start of enrollment tests to pass as OTP.
      await TestSuiteInitializer.getInstance()
          .testInitializer(currentAtSign, namespace, 'pkam', enableInitialSync: false);
      // Set SPP into the Remote Secondary
      var atResponse = await AtClientManager.getInstance().atClient.setSPP('ABC123');
      expect(atResponse.response, 'ok');

      // Submit an enrollment request with at_auth package
      AtEnrollmentBase atEnrollmentBase = AtEnrollmentBase.create(currentAtSign);
      ...
      EnrollmentRequest enrollmentRequest = EnrollmentRequest(
          appName: 'wavi-$random', deviceName: 'iphone', otp: 'ABC123', namespaces: {TestConstants.namespace: 'rw'});
      AtEnrollmentResponse? atEnrollmentResponse = await atEnrollmentBase.submit(enrollmentRequest, atLookUp);
      ...
      String? enrollmentFetchResponse = await AtClientManager.getInstance()
          .atClient
          .getRemoteSecondary()
          ?.executeCommand('enroll:fetch:{"enrollmentId":"${atEnrollmentResponse.enrollmentId}"}\n', auth: true);
      enrollmentFetchResponse = enrollmentFetchResponse?.replaceAll('data:', '');
      Enrollment enrollment = Enrollment.fromJSON(jsonDecode(enrollmentFetchResponse!));

      // Approve enrollment
      AtEnrollmentResponse? approveEnrollmentResponse = await AtClientManager.getInstance()
          .atClient
          .enrollmentService
          ?.approve(EnrollmentRequestDecision.approved(ApprovedRequestDecisionBuilder(
      ...
      AtEncryptionKeyPair atEncryptionKeyPair =
          AtEncryptionKeyPair.create(atEnrollmentResponse.atAuthKeys!.defaultEncryptionPublicKey!.toString(), '');

      AtPkamKeyPair atPkamKeyPair = AtPkamKeyPair.create(atEnrollmentResponse.atAuthKeys!.apkamPublicKey!.toString(),
      ...
      atChops.atChopsKeys.apkamSymmetricKey = AESKey(atEnrollmentResponse.atAuthKeys!.apkamSymmetricKey!.toString());
      ...
      atChops.atChopsKeys.atEncryptionKeyPair = AtEncryptionKeyPair.create(
          ...
          await getDefaultEncryptionPrivateKey(currentAtSign, atEnrollmentResponse.enrollmentId, atLookUp));

      String selfEncryptionKey =
          await getDefaultSelfEncryptionKey(currentAtSign, atEnrollmentResponse.enrollmentId, atLookUp);
      ...
      atAuthRequest.rootDomain = AtRootDomain(AtClientManager.getInstance().atClient.getPreferences()!.rootDomain,
          AtClientManager.getInstance().atClient.getPreferences()!.rootPort);
  ...
  tearDownAll(() async {
    AtClientManager.getInstance().removeAllChangeListeners();
    AtClientManager.getInstance().atClient.notificationService.stopAllSubscriptions();
...
void writeAtKeysToFile(String atSign, AtKeys atAuthKeys) {
  ...
  String encryptedEncryptionPublicKey = EncryptionUtil.encryptValue(
      atAuthKeys.defaultEncryptionPublicKey!.toString(), atAuthKeys.defaultSelfEncryptionKey!.toString());
  ...
  String encryptedEncryptionPrivateKey = EncryptionUtil.encryptValue(
      atAuthKeys.defaultEncryptionPrivateKey!.toString(), atAuthKeys.defaultSelfEncryptionKey!.toString());
...
Future<String> getDefaultEncryptionPrivateKey(String atSign, String enrollmentIdFromServer, AtLookUp atLookUp) async {
  ...
  try {
    var getPrivateKeyResult = await atLookUp.executeCommand('$privateKeyCommand\n', auth: true);
  ...
  AtEncryptionResult? atEncryptionResult = atLookUp.atChops?.decryptString(
      ...
      keyName: 'apkamSymmetricKey', iv: AtChopsUtil.generateIVFromBase64String(encryptionPrivateKeyIV));
...
Future<String> getDefaultSelfEncryptionKey(String atSign, String enrollmentIdFromServer, AtLookUp atLookUp) async {
  ...
  try {
    String? encryptedSelfEncryptionKey = await atLookUp.executeCommand('$selfEncryptionKeyCommand\n', auth: true);
    ...
    encryptedSelfEncryptionKey = encryptedSelfEncryptionKey.replaceFirst('data:', '');
  ...
  AtEncryptionResult? atEncryptionResult = atLookUp.atChops?.decryptString(
      ...
      keyName: 'apkamSymmetricKey', iv: AtChopsUtil.generateIVFromBase64String(selfEncryptionKeyIV));
  return atEncryptionResult?.result;
}
