...
| Flutter app | `at_client: ^3.12.0` + `at_client_flutter: ^1.1.2` |
...
);  // null if any ancestor expired; throws if CAncestor.owner is null (cache from subUpdates)
```

For 3+ levels use `watchWithTree` with `SubSpec<U>`. Read
[references/03-query-api.md](references/03-query-api.md) when working with deep hierarchies.
...
**Coming in skill v2.0:** Migration guide for apps using the raw `atClient.put()` /
`atClient.get()` key-value API (includes backward-compatibility analysis).
