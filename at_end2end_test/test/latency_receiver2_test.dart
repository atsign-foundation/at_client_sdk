...
class LatencyTestConfigParameters {
  ...
  static const int maxExpectedLatencyInMillis = 1200;
...
void main() {
  ...
  test('Listen to notification from sharedWith atSign', () async {
    ...
    List<dynamic> entry = [];
    File file = File(
        '/home/shaikirfan/Desktop/client_trunk/at_client_sdk/at_end2end_test/test/latencyWithSync.csv');
    ...
    while (true) {
      var randomKey = AtKey()
        ...
        ..namespace = 'sample';
      var result =
          await firstAtsignClientManager.atClient.put(randomKey, 'testValue');
  ...
  tearDownAll(() async {
    var isExists = await Directory('test2/hive').exists();
    if (isExists) {
      Directory('test2/hive/').deleteSync(recursive: true);
    }
  });
}
