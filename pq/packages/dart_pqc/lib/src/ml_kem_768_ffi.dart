...
/// ML-KEM-768 KEM backed by OpenSSL 3 via Dart FFI.
///
/// **Secret-key encoding:** OpenSSL's EVP API does not expose raw ML-KEM-768
/// secret key bytes. Instead, [generateKeyPair] stores the live `EVP_PKEY*` in
/// an internal registry and returns an 8-byte opaque handle as `secretKey`. The
/// handle is valid only within the same process lifetime. Call
...
final class MlKem768Ffi implements MlKem768Algorithm {
  ...
  Future<PqcKeyPair> generateKeyPair([Uint8List? seed]) async {
    // Seed is not supported by the OpenSSL EVP ML-KEM API; ignored here.
    ...
    try {
      ...
      try {
        ...
        return PqcKeyPair(
          ...
          secretKey: _encodeHandle(handle),
        );
      } finally {
        calloc.free(pkeyPtr);
      }
  ...
  /// Release the native `EVP_PKEY*` held by [kp].
  ///
  /// Call this when a key pair generated by this instance is no longer needed.
  void releaseKeyPair(PqcKeyPair kp) {
  ...
  int _registerKey(Pointer<EVP_PKEY> pkey) {
    int handle;
    do {
      handle = _rng.nextInt(0x7fffffff) |
          (_rng.nextInt(0x7fffffff) << 32);
