## 3.5.0
- feat: `AtSignatureAlgorithm` gains a `signingAlgoType` getter so an algorithm can report its own wire identifier. Previously the identifier travelled beside the algorithm in a caller-managed field (`AtSigningInput.signingAlgoType`, threaded from `AtClientPreference` through `AtLookUp`), which meant the two could disagree — passing a `signingAlgorithm` without also updating `signingAlgoType` labelled the signature with the default `rsa2048`. Typed as `SigningAlgoType` rather than `String` because the `pkam:` verb accepts only that enum's `name` spellings; a free-form name would surface as an atServer syntax error at authentication time
- feat: add `RsaSignatureAlgo` — RSASSA-PKCS1-v1_5 signing on the stateless `AtSignatureAlgorithm` interface, key material passed per call and safe to share as a singleton. Key bytes are DER (X.509 `SubjectPublicKeyInfo` / PKCS#8 `PrivateKeyInfo`), so an existing `RsaKeyPair` crosses over with a `base64Decode`. Modulus size is fixed per instance via `RsaSignatureAlgo.rsa2048()` / `.rsa4096()` — it has to be, because `signingAlgoType` must answer before any key is in scope — and `signBytes`/`verifyBytes` reject a key whose size contradicts the reported identifier instead of signing under the wrong label. Digest is a constructor parameter defaulting to sha256; sha512 is also accepted, anything else throws at construction. Note `SigningAlgoType.rsa4096` is not in the atServer's `pkam:` grammar, so `.rsa4096()` is for data and envelope signing only
- feat: `AtHashingAlgorithm` gains a matching `hashingAlgoType` getter, so both halves of the `pkam:` wire tuple (`signingAlgo`/`hashingAlgo`) can be read off the algorithms that produced them. `SHA256HashingAlgo`, `SHA512HashingAlgo`, `Md5HashingAlgo`, and `Argon2idHashingAlgo` map 1:1 onto `HashingAlgoType`; `DefaultHash` inherits `md5`. Note `HashingAlgoType` is wider than any single verb's grammar — `pkam:` accepts only `sha256`/`sha512` — so reporting a member does not make it valid in every position
