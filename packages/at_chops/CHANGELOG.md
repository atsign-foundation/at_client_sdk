...
- breaking: remove all key-pair wrapper classes (`RsaKeyPair`, `X25519KeyPair`, `MlDsa65KeyPair`, `MlKem768KeyPair`, `XWingKeyPair`) and the deprecated `At*KeyPair` classes, along with the `AbstractKeyPair`/`AsymmetricKeyPair` hierarchy and the `RawKeyPairBytes` mixin — generate key material directly from each algorithm's `generateKeyPair()`, which returns raw `Uint8List` public/secret keys; base64-encode them yourself where a string form is needed
- breaking: `RsaSigningAlgo`, `EccSigningAlgo`, and `Ed25519SigningAlgo` now implement the stateless `AtSignatureAlgorithm` interface — their old stateful constructors/setters and `sign`/`verify` methods are replaced by `generateKeyPair()`/`signBytes()`/`verifyBytes()` with key material passed per call as raw bytes (`RsaSigningAlgo` takes `hashingAlgoType`/`keySize` as named constructor parameters; `keySize` must be 2048 or 4096, and `signBytes`/`verifyBytes` reject a key whose modulus is not that size — `name` is the wire identifier and `SigningAlgoType.strongestFirst` ranks on it, so a mismatched key would both mislabel the signature and move it in the preference order a verifier chooses by). Key bytes are raw: Ed25519 uses the 32-byte seed/public key, ECC uses the 32-byte scalar and 65-byte uncompressed point, RSA uses DER. Also removes the deprecated `AtSigningAlgorithm` interface and the `DefaultHash`/`DefaultSigningAlgo`/`PkamSigningAlgo` compatibility wrappers
...
- breaking: remove the string-keyed key hierarchy — `AbstractKey`, `SymmetricKey`, `AESKey`, the misnamed `Ed25519Key` (it was an AES symmetric key, not an Ed25519 key), `AtPublicKey`, `AtPrivateKey`, and the `EncryptionKeyType` enum that only ever fed the removed `AtChopsImpl`/`AtEncryptionMetaData`. Every algorithm now takes and returns raw `Uint8List` key bytes, so there is nothing left for a base64-carrying wrapper to do: `AESKey.generate(32)` becomes `SymmetricEncryptionAlgorithm.generateKey()`, and `base64Decode(aesKey.key)` / `base64Decode(atPublicKey.publicKey)` become the bytes themselves
...
- chore: `package:at_chops/at_chops.dart` (the web-safe barrel) now compiles to WASM (dart2wasm), enforced by the `wasm_ratchet` CI gate (`.github/wasm_gates.yaml`). FFI-backed algorithms remain in `package:at_chops/at_chops_ffi.dart`, which is not WASM-compatible by design
- fix: `EccSigningAlgo.signBytes` writes the at_chops 3.x wire format — the compact `R ‖ S` pair ASCII-hex encoded, 128 bytes — instead of the raw 64-byte compact form an earlier unreleased build of this signature emitted. `verifyBytes` accepts either encoding (keyed on `signature.length`) and rejects any other length with `ArgumentError`, so a signature already written by 3.x, or by the interim raw form, both still verify
