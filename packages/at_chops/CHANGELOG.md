## 3.5.0
- fix: `MlKem768FfiAlgo`/`MlKem768PureDartAlgo`, `MlDsa65FfiAlgo`/`MlDsa65PureDartAlgo`, and `XWingFfiAlgo`/`XWingPureDartAlgo` now enforce their output-length invariants (ciphertext, shared secret, signature, X-Wing sub-component sizes) with a `StateError` instead of `assert()` — asserts are stripped in release builds, so a wrong-size output from OpenSSL, `pqcrypto`, or `cryptography` would previously go undetected in production.
...
- fix: `MlDsa65KeyPair.create` now throws `AtSigningException` (not a raw `FormatException`) when `publicKey`/`privateKey` isn't valid base64, matching its documented contract for all invalid input, not just correctly-encoded-but-wrong-length input.
...
- breaking: `AtSignatureAlgorithm` gains two abstract members — implementers must add both. `String get name`: `MlDsa65FfiAlgo`/`MlDsa65PureDartAlgo.name` both return `'ml-dsa-65'` (`mlDsa65AlgorithmName`), so a downstream protocol keying on it sees one stable identifier regardless of backend. `SigningAlgoType get signingAlgoType`: previously the identifier travelled beside the algorithm in a caller-managed field (`AtSigningInput.signingAlgoType`, threaded from `AtClientPreference` through `AtLookUp`), which meant the two could disagree — passing a `signingAlgorithm` without also updating `signingAlgoType` labelled the signature with the default `rsa2048`. Both ML-DSA-65 backends report `SigningAlgoType.mldsa65`. `name` and `signingAlgoType` serve different wire positions with different spellings (`'ml-dsa-65'` vs `'mldsa65'`) and must not be assumed interchangeable.
