...
- feat: add `AesGcm256FfiAlgo` — AES-256-GCM authenticated encryption (AEAD) backed by OpenSSL 3 via FFI (`EVP_aes_256_gcm`), fully interoperable with the pure-Dart `AesGcm256EncryptionAlgo`. Exported from `at_chops_ffi.dart` (not the web-safe main barrel), since it carries `dart:ffi` bindings; `AtPqc` does not auto-resolve it (AES-GCM is symmetric AEAD, not a PQ primitive), so callers pick the backend explicitly. Adds the `libCryptoSupportsAesGcm` capability probe
