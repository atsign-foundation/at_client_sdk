## 3.5.0
- breaking: `MlDsa65FfiAlgo.fromLib` now throws `AtSigningException` when the supplied libcrypto lacks ML-DSA-65, instead of every `verifyBytes` silently returning `false` for a misconfigured build indistinguishable from a forged signature; external callers must gate construction on `libCryptoSupportsMlDsa65`, or use `AtPqc.mlDsa65` (which already does).
- fix: `MlKem768PureDartAlgo.encapsulate`/`decapsulate` now validate public-key/ciphertext/secret-key lengths in `at_chops` itself rather than relying on `pqcrypto`'s private `KyberKem` validators, which live behind an `implementation_imports` escape hatch and could silently stop validating in a future patch release.
- fix: `MlKem768PureDartAlgo`/`MlKem768FfiAlgo.generateKeyPair` now enforce the 1184-byte public-key (and, pure-Dart, 2400-byte secret-key) output invariant themselves, so direct callers such as `MlKem768KeyPair.generate` are covered — previously only X-Wing's composition checked it.
- fix: `MlKem768FfiAlgo`/`MlKem768PureDartAlgo`, `MlDsa65FfiAlgo`/`MlDsa65PureDartAlgo`, and `XWingFfiAlgo`/`XWingPureDartAlgo` now enforce output-length invariants (ciphertext, shared secret, signature, X-Wing sub-component sizes) with `StateError` instead of `assert()` — asserts strip in release builds, so a wrong-size output from OpenSSL, `pqcrypto`, or `cryptography` would go undetected in production.
- fix: `MlDsa65FfiAlgo`/`MlDsa65PureDartAlgo.signBytes` now throws `AtSigningException` for a wrong-length secret key (previously the pure-Dart backend silently truncated an over-long key and signed with the wrong material); `verifyBytes` now returns `false` instead of throwing for a wrong-length public key/signature; `MlDsa65KeyPair.create` also validates key lengths at construction.
- fix: `MlDsa65KeyPair.create` now throws `AtSigningException` (not a raw `FormatException`) for non-base64 `publicKey`/`privateKey`, matching its documented contract for all invalid input.
- fix: `MlKem768FfiAlgo.encapsulate`/`decapsulate` now throw `ArgumentError` for a wrong-length public key/ciphertext, matching the pure-Dart backend instead of surfacing a generic OpenSSL `StateError`.
- refactor: consolidate X-Wing's size constants (`seedLength`/`publicKeyLength`/`ciphertextLength`/`sharedSecretLength`, duplicated in `XWingFfiAlgo` and `XWingPureDartAlgo`) into a single `XWingSizes` source of truth. No behavior change.
- breaking: `AtSignatureAlgorithm` gains two abstract members — implementers must add both. `String get name`: a free-form identifier (`MlDsa65FfiAlgo`/`MlDsa65PureDartAlgo.name` return `SigningAlgoType.mldsa65.name`, i.e. `'mldsa65'`). `SigningAlgoType get signingAlgoType`: the `pkam:`/envelope wire identifier (`SigningAlgoType.mldsa65` for both backends), now available on the algorithm itself rather than only on a caller-managed field (`AtSigningInput.signingAlgoType`).
- breaking: `AtHashingAlgorithm` gains an abstract `String get name` — external `implements` users must add it. `SHA256HashingAlgo`/`SHA512HashingAlgo`/`Md5HashingAlgo`/`Argon2idHashingAlgo.name` return the corresponding `HashingAlgoType.name` spelling (`'sha256'`, `'sha512'`, `'md5'`, `'argon2id'`) — the `pkam:`/envelope wire tuple's `hashingAlgo` identifier, previously only available via a caller-managed `HashingAlgoType` field.
