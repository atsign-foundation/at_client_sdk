...
import 'package:at_chops/src/hashing/hkdf_core.dart';

/// HMAC-SHA256 (RFC 2104). The keyed-hash primitive HKDF is built on; also
/// usable directly for MACs. Wraps the shared `Hkdf` core so consumers depend
/// only on at_chops for keyed hashing.
class HmacSha256 {
  /// `HMAC-SHA256(key, data)` → 32 raw bytes.
  static Uint8List compute(Uint8List key, Uint8List data) =>
      Hkdf.sha256.hmac(key, data);
}
...
class HkdfSha256 {
  ...
  static Uint8List extract(Uint8List ikm, {Uint8List? salt}) =>
      Hkdf.sha256.extract(ikm, salt: salt);
  ...
  static Uint8List expand(Uint8List prk,
      ...
      Hkdf.sha256.expand(prk, info: info, length: length);
  ...
  static Uint8List deriveKey(
    ...
    required int length,
      ...
      Hkdf.sha256.deriveKey(ikm, salt: salt, info: info, length: length);
}
...
class HkdfSha384 {
  ...
  static Uint8List extract(Uint8List ikm, {Uint8List? salt}) =>
      Hkdf.sha384.extract(ikm, salt: salt);

  /// RFC 5869 Expand, truncated to [length].
  static Uint8List expand(Uint8List prk,
          {Uint8List? info, required int length}) =>
      Hkdf.sha384.expand(prk, info: info, length: length);
  ...
  static Uint8List deriveKey(
    ...
    required int length,
      ...
      Hkdf.sha384.deriveKey(ikm, salt: salt, info: info, length: length);
}
