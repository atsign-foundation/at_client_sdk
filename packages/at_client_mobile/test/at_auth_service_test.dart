...
import 'package:at_client_mobile/src/at_auth_service_impl.dart';
...
class MockCramAuthenticator extends Mock implements CramAuthenticator {}

class MockAtChopsImpl extends Mock implements AtChopsImpl {}
...
void main() {
  ...
  late PkamAuthenticator mockPkamAuthenticator;
  late CramAuthenticator mockCramAuthenticator;
  ...
  void setUpMethod() {
    ...
    mockCramAuthenticator = MockCramAuthenticator();
    mockSyncService = MockSyncService();
    mockAtChops = MockAtChopsImpl();
    ...
    when(() => mockAtClient.persistKeys(any(), any(), any(), any(), any()))
    ...
    when(() => mockKeyChainManager.getPkamPublicKey(any()))
        .thenAnswer((_) => Future.value('dummy_pkam_public_key'));
    when(() => mockKeyChainManager.getPkamPrivateKey(any()))
        .thenAnswer((_) => Future.value('dummy_pkam_private_key'));
    ...
    when(() => mockKeyChainManager.getSelfEncryptionAESKey(any()))
        .thenAnswer((_) => Future.value('dummy_self_encryption_key'));
    when(() => mockKeyChainManager.generateSelfEncryptionKey())
        .thenReturn('dummy_self_encryption_key');
    when(() => mockKeyChainManager.generateKeyPair())
        .thenAnswer((_) => RSAKeypair.fromRandom());
  }

  group('A group of test to validate onboard method', () {
    setUp(() => setUpMethod());

    test('A test to verify onboard method', () async {
      var atAuthService = AtAuthServiceImpl();
      atAuthService.keyChainManager = mockKeyChainManager;
      atAuthService.atClientManager = mockAtClientManager;
      atAuthService.pkamAuthenticator = mockPkamAuthenticator;
      atAuthService.cramAuthenticator = mockCramAuthenticator;
      atAuthService.atChops = mockAtChops;

      when(() => mockPkamAuthenticator.authenticate(any())).thenAnswer(
          (_) => Future.value(AtAuthResponse(atSign)..isSuccessful = true));

      when(() => mockCramAuthenticator.authenticate(any())).thenAnswer(
          (_) => Future.value(AtAuthResponse(atSign)..isSuccessful = true));
      when(() => mockCramAuthenticator.atLookup).thenReturn(mockAtLookupImpl);

      when(() => mockAtLookupImpl.executeVerb(any()))
          .thenAnswer((_) => Future.value('data:-1'));
      final atChopsKeys = AtChopsKeys.create(
          AtEncryptionKeyPair.create(
              'dummy_encryption_public_key', 'dummy_encryption_private_key'),
          AtPkamKeyPair.create(
              'dummy_pkam_public_key', 'dummy_pkam_private_key'));
      atChopsKeys.symmetricKey = AESKey('dummy_self_encryption_key');
      when(() => mockAtChops.atChopsKeys).thenReturn(atChopsKeys);

      AtOnboardingRequest onboardingRequest =
          AtOnboardingRequest(atSign, atClientPreference);
      var onboardResult = await atAuthService.onboard(onboardingRequest);
      expect(onboardResult.isSuccessful, true);
    });
