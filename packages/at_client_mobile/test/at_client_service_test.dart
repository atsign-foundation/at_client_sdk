...
void main() {
  ...
  void setUpMethod() {
    ...
    when(
      () => mockAtLookUp.authenticate(any()),
    ).thenAnswer((_) => Future.value(true));
    when(
      () => mockAtLookUp.pkamAuthenticate(),
    ).thenAnswer((_) => Future.value(true));
    when(() => mockAtLookUp.close()).thenAnswer((_) async => {});

    when(
      () => mockAtClientManager.setCurrentAtSign(
        any(),
        any(),
        any(that: CustomAtClientPref()),
    ...
    when(() => mockRemoteSecondary.atLookUp).thenAnswer((_) => mockAtLookUp);

    when(() => mockSyncService.sync()).thenAnswer((_) => {});

    when(
      () => mockLocalSecondary.putValue(any(), any()),
    ).thenAnswer((_) => Future.value(true));
    when(
      () => mockLocalSecondary.executeVerb(
        any(that: CustomVerbBuilder()),
        sync: true,
      ),
    ).thenAnswer((_) => Future.value('data:1'));
    when(
      () => mockLocalSecondary.getPrivateKey(),
    ).thenAnswer((_) => Future.value('dummy_private_key'));
    when(
      () => mockLocalSecondary.getPublicKey(),
    ).thenAnswer((_) => Future.value('dummy_public_key'));
    when(
      () => mockLocalSecondary.getEncryptionPrivateKey(),
    ).thenAnswer((_) => Future.value('dummy_encryption_private_key'));
    when(
      () => mockLocalSecondary.getEncryptionPublicKey(any()),
    ...
    when(
      () => mockKeyChainManager.storePkamKeysToKeychain(
        any(),
    ...
    when(
      () => mockKeyChainManager.readAtsign(name: any(named: 'name')),
    ).thenAnswer((_) => Future.value(AtsignKey(atSign: '@bob')));
    when(
      () => mockKeyChainManager.storeAtSign(
        atSign: any(named: 'atSign', that: CustomAtSignMatcher()),
      ),
    ).thenAnswer((_) => Future.value(true));
    when(
      () => mockKeyChainManager.storeCredentialToKeychain(
        any(),
        privateKey: any(named: 'privateKey'),
        publicKey: any(named: 'publicKey'),
      ),
    ...
    when(
      () => mockKeyChainManager.getPkamPublicKey(any()),
    ).thenAnswer((_) => Future.value('dummy_public_key'));
    when(
      () => mockKeyChainManager.getEncryptionPrivateKey(any()),
    ).thenAnswer((_) => Future.value('dummy_encryption_private_key'));
    when(
      () => mockKeyChainManager.getEncryptionPublicKey(any()),
    ).thenAnswer((_) => Future.value('dummy_encryption_public_key'));
    when(
      () => mockKeyChainManager.getSelfEncryptionAESKey(any()),
    ).thenAnswer((_) => Future.value('dummy_self_encryption_key'));
  }
  ...
  group('A group of tests to validate authenticate method', () {
    ...
    test(
      ...
      () async {
        ...
        when(
          () => mockKeyChainManager.getPkamPrivateKey(any()),
        ).thenAnswer((_) => Future.value('dummy_private_key'));

        var atClientService = AtClientService();
        atClientService.AtLookUp = mockAtLookUp;
        ...
        expect(
          atChopsImpl!.atChopsKeys.atPkamKeyPair?.atPrivateKey.privateKey,
          decryptedPKAMPrivateKey,
        );
        expect(
          atChopsImpl!.atChopsKeys.atPkamKeyPair?.atPublicKey.publicKey,
    ...
    test('A test to verify when cram secret is populated', () async {
      ...
      AtClientPreference atClientPreference = AtClientPreference()
        ..cramSecret = '123';
      when(
        () => mockAtClientAuthenticator.performInitialAuth(
          any(),
          atClientPreference,
        ),
      ).thenAnswer((_) {
      ...
      when(
        () => mockAtClientAuthenticator.atLookUp,
      ).thenAnswer((_) => mockAtLookUp);
      when(
        () => mockKeyChainManager.getPkamPrivateKey(any()),
    ...
    test('A test to verify atChops is set from the keychain manager', () async {
      ...
      var keysFromChainManager = await atClientService
          .getKeysFromKeyChainManager(atSign);
  ...
  group('A group of test to validate onboard method', () {
    ...
    test('A test to verify onboard method when atSign is populated', () async {
      ...
      when(
        () => mockAtLookUp.executeCommand(any()),
    ...
    test(
      'A test to verify ttr is -1 when storing the public encryption key',
      () async {
        var atClientService = AtClientService();
        ...
        when(
          ...
          if (verbBuilder is UpdateVerbBuilder &&
            ...
            ttrOfPublicKeyFromUpdateVerbBuilder =
                verbBuilder.atKey.metadata.ttr;
          }
          return 'data:1';
        });
