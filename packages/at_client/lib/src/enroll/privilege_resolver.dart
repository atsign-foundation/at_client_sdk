...
///
/// Implementations answer from the enrollment record — the server's word —
/// never from anything the client asserts about itself, so an enrollment
/// cannot grant itself a privilege it was never given.
abstract interface class EnrollmentPrivilegeResolver {
  ...
  /// and `__manage` — the question [isFullyPrivileged] asks about this
  /// client itself, asked about another enrollment. One the implementation
  /// cannot find answers `false`: privilege is granted by the record, so no
  /// record is no privilege. May cost a round trip; callers treat it as such.
  Future<bool> isEnrollmentFullyPrivileged(String enrollmentId);
}

/// Whether [namespaces] grant `rw` on both `*` and `__manage` — the class
/// that may hold the signing root.
///
