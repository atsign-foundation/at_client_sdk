  ...
  test(
    ...
    () async {
      ...
      var aliceEncryptionPublicKey =
          aliceEncryptionKeyPair.atPublicKey.publicKey;
      ...
      var sharedKeyEncryptedWithBobPublicKey = EncryptionUtil.encryptKey(
        aesSharedKey,
        bobEncryptionPublicKey,
      );

      var bobPublicKeyCheckSum = EncryptionUtil.md5CheckSum(
        bobEncryptionPublicKey,
      );
      ...
      when(
        () => mockLocalSecondary.getEncryptionPublicKey('@alice'),
      ).thenAnswer((_) => Future.value(aliceEncryptionPublicKey));
      when(
        () => mockLocalSecondary.executeVerb(any<LLookupVerbBuilder>()),
      ...
      var encryptionResult = await sharedKeyEncryption.encrypt(
        sharedKey,
        location,
      );
      expect(encryptionResult != location, true);
      var decryptionResult = await sharedKeyDecryption.decrypt(
        sharedKey,
        encryptionResult,
      );
    ...
    () async {
      ...
      when(
        ...
        (_) => throw KeyNotFoundException(
          'local key shared_key.bob@alice not found',
        ),
      );
      when(
        () => mockLocalSecondary.executeVerb(
      ...
      when(
        () => mockLocalSecondary.executeVerb(
          any(that: LLookupTheirSharedKeyMatcher()),
        ),
      ).thenAnswer(
      ...
      when(
        () => mockLocalSecondary.executeVerb(
          any(that: LLookupCachedBobPublicKeyMatcher()),
        ),
...
class LLookupLocalSharedKeyMatcher extends Matcher {
  ...
  Description describe(Description description) => description.add(
    'A custom matcher to match LOOKUPS of encrypted shared key',
  );
...
class UpdateLocalSharedKeyMatcher extends Matcher {
  @override
  Description describe(Description description) => description.add(
    'A custom matcher to match UPDATE of encrypted shared key',
  );
...
class DeleteLocalSharedKeyMatcher extends Matcher {
  @override
  Description describe(Description description) => description.add(
    'A custom matcher to match Deletes of encrypted shared key',
  );
