...
void main() {
  ...
  group('A group of test to validate notification request transformer', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          () async => await notificationRequestTransformer.transform(
            notificationParams,
          ),
          throwsA(
            predicate(
  ...
  group('A group of test to validate notification response transformer', () {
    setUp(() {
      registerFallbackValue(FakeAtKey());
      when(
        () => mockSharedKeyDecryption.decrypt(any(), 'encryptedValue'),
    ...
    test(
      ...
      () async {
        ...
        notificationResponseTransformer.atKeyDecryption =
            mockSharedKeyDecryption;

        var transformedNotification = await notificationResponseTransformer
            .transform(
              Tuple()
                ..one = atNotification
  ...
  group('A group of tests to validate notification exception chaining', () {
    ...
    setUp(() {
      ...
      when(
        () => mockAtClientImpl.getPreferences(),
      ).thenAnswer((_) => AtClientPreference()..namespace = 'wavi');
    });
    test('A test to validate exception chaining on encryption failure', () async {
      ...
      when(
        () => mockSecondaryAddressFinder.findSecondary('@bob'),
      ).thenAnswer((_) => Future.value(SecondaryAddress('dummyhost', 9001)));
      when(
        () => mockAtClientManager.secondaryAddressFinder,
      ).thenAnswer((_) => mockSecondaryAddressFinder);
      when(
        () => mockAtKeyEncryptionManager.get(atKey, currentAtSign),
      ...
      when(
        () => mockAtClientManager.atClient,
      ).thenAnswer((_) => mockAtClientImpl);

      var notificationServiceImpl =
          await NotificationServiceImpl.create(
                mockAtClientImpl,
      ...
      expect(
        notificationResult.atClientException!.getTraceMessage(),
        'Failed to notifyData caused by\nFailed to encrypt the data caused by\npublic:publickey@bob does not exist in keystore',
      );
    });

    test(
      'A test to verify exception from cloud secondary is chained',
      () async {
        var currentAtSign = '@alice';
        AtKey atKey =
            (AtKey.shared('phone', namespace: 'wavi', sharedBy: currentAtSign)
    ...
    test(
      'A test to verify buffer over flow exception for messageType key',
      () async {
        var notificationServiceImpl =
            await NotificationServiceImpl.create(
                  mockAtClientImpl,
        ...
        var atKey = (AtKey.shared(
          'phone',
          namespace: 'wavi',
          sharedBy: '@bob',
        )..sharedWith('@alice')).build();
        var value = '+91-9856745453';

        NotificationParams notificationParams = NotificationParams.forUpdate(
          atKey,
          value: value,
        );
    ...
    test(
      'A test to verify buffer over flow exception for messageType text',
      () async {
        when(
          () => mockAtClientImpl.getPreferences(),
        ).thenAnswer((_) => AtClientPreference()..maxDataSize = 1);
    ...
    test(
      ...
      () async {
        ...
        when(
          () => mockSecondaryAddressFinder.findSecondary('@colin'),
        ).thenAnswer((_) => Future.value(SecondaryAddress('dummyhost', 9001)));
        when(
          () => mockAtClientImpl.notifyStatus(any()),
        ...
        expect(notificationResult.atClientException, isA<AtClientException>());
        expect(
          notificationResult.atClientException?.message,
  ...
  group('A group of tests to validate notification subscribe method', () {
    test(
      'NotificationService subscribe returns a new stream for a new regex',
      () async {
        var notificationServiceImpl =
            await NotificationServiceImpl.create(
                  mockAtClientImpl,
  ...
  group('A group of tests to validate notification fetch', () {
    test(
      'A test to verify notification fetch for non-existing notification',
      () async {
        var notificationServiceImpl =
            await NotificationServiceImpl.create(
                  mockAtClientImpl,
        ...
        when(() => mockAtLookUp.executeVerb(any())).thenAnswer(
          (_) => Future.value(
            'data:${jsonEncode({'id': '123', 'notificationStatus': 'NotificationStatus.expired'})}',
          ),
        );
    ...
    test(
      'A test to verify notification fetch for an existing notification',
      () async {
        var notificationServiceImpl =
            await NotificationServiceImpl.create(
                  mockAtClientImpl,
        ...
        when(() => mockAtLookUp.executeVerb(any())).thenAnswer(
          (_) => Future.value(
            'data:{"id":"fb948e15-128f-408d-b73f-0ab1372da4a3",'
            '"fromAtSign":"@alice",'
    ...
    test('A test to verify remote secondary timeouts to respond', () async {
      ...
      registerFallbackValue(FakeNotifyFetchVerbBuilder());
      when(() => mockAtLookUp.executeVerb(any())).thenAnswer(
        (_) => throw AtLookUpException(
          'AT0023',
          'Waited for 10000 millis. No response after 100000',
        ),
      );
      ...
      expect(
        () async => await notificationServiceImpl.fetch('123'),
    ...
    test('A test to verify remote secondary is not reachable', () async {
      ...
      registerFallbackValue(FakeNotifyFetchVerbBuilder());
      when(() => mockAtLookUp.executeVerb(any())).thenAnswer(
        (_) => throw AtLookUpException('AT0021', 'Secondary Connect Exception'),
      );
  ...
  group('Tests of getLastNotificationTime()', () {
    ...
    test(
      ...
      () async {
        when(() => mockAtClientImpl.getPreferences()).thenAnswer(
          (_) => AtClientPreference()
            ..namespace = 'wavi'
            ..fetchOfflineNotifications = false,
        );

        when(
          () => mockAtClientImpl.getLocalSecondary()!.keyStore!.isKeyExists(
            any(),
          ),
        ).thenAnswer((_) => true);
    ...
    test(
      ...
      () async {
        ...
        when(
          () => mockAtClientImpl.get(any()),
        ).thenAnswer((_) async => Future.value(AtValue()));

        when(
          () => mockAtClientImpl.put(any(), any()),
    ...
    test(
      ...
      () async {
        ...
        when(
          () => mockAtClientImpl.get(
            notificationServiceImpl.lastReceivedNotificationAtKey,
          ),
        ).thenAnswer((_) async => Future.value(AtValue()));

        when(
          () => mockAtClientImpl.get(any(that: StatsAtKeyMatcher())),
        ).thenAnswer(
    ...
    test(
      ...
      () async {
        ...
        when(() => mockAtClientImpl.get(lastNotificationAtKey)).thenAnswer(
          (_) async => Future.value(
            AtValue()
              ..value = jsonEncode(atNotification)
              ..metadata = Metadata(),
  ...
  group('A group of tests for lastNotificationReceived key', () {
    test('test to verify lastNotificationReceived toString', () {
      ...
      expect(
        lastReceivedNotification.toString(),
    ...
    test('test to verify lastNotificationReceived fromString', () async {
      var lastReceivedNotification = AtKey.fromString(
        'local:lastReceivedNotification.wavi@alice',
      );
  ...
  group('Tests for monitorRetry', () {
    ...
    test(
      ...
      () async {
        ...
        notificationServiceImpl.monitorRetryInterval = Duration(
          milliseconds: 50,
        );
    ...
    test(
      ...
      () async {
        ...
        await (Future.delayed(
          notificationServiceImpl.monitorRetryInterval +
