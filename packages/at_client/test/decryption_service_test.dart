...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockAtLookup.executeVerb(lookupVerbBuilder)).thenAnswer(
      (_) async => throw AtExceptionUtils.get('AT0015', 'Connection timeout'),
    );
    when(
      () => mockAtClientImpl.getLocalSecondary(),
    ).thenAnswer((_) => mockLocalSecondary);
    when(() => mockAtClientImpl.getCurrentAtSign()).thenAnswer((_) => '@xyz');
    when(
      () => mockLocalSecondary.getEncryptionPublicKey('@xyz'),
  ...
  group('A group of positive test mock test to verify decryption service', () {
    test(
      'A test to verify decryption is successful when all keys are found',
      () async {
        ...
        final atChopsKeys = AtChopsKeys.create(
          AtEncryptionKeyPair.create('', encryptionPrivateKey),
          null,
        );
        when(
          () => mockAtClientImpl.atChops,
        ...
        expect(
          await sharedKeyDecryption.decrypt(atKey, encryptedValue),
          'hello',
        );
      },
    );
  ...
  group('A group of tests to verify exceptions in decryption service', () {
    ...
    test(
      ...
      () {
        ...
        var sharedKeyDecryptionWithAtChops = SharedKeyDecryption(
          mockAtClientImpl,
        );
        final atChopsKeys = AtChopsKeys.create(
  ...
  group('A group of test to validate the decryption service manager', () {
    test(
      ...
      () {
        ...
        var decryptionService = AtKeyDecryptionManager(
          mockAtClientImpl,
    ...
    test('Test to verify SelfKeyDecryption instance is returned for '
