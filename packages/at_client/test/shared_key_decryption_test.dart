...
void main() {
  ...
  test(
    'test to check AtDecryptionException when encrypted value is null',
    () async {
  ...
  test('test to check SharedKeyNotFound exception', () async {
    ...
    when(
      () => mockRemoteSecondary.executeVerb(any<LLookupVerbBuilder>()),
    ).thenAnswer((_) => Future.value('data:null'));
    var sharedKey = AtKey()
      ..sharedBy = '@alice'
      ..sharedWith = '@bob'
  ...
  test(
    ...
    () async {
      var sharedKeyDecryption = SharedKeyDecryption(mockAtClient);
      when(() => mockAtClient.getCurrentAtSign()).thenReturn('@alice');
      when(() => mockLocalSecondary.getEncryptionPublicKey('@alice')).thenThrow(
  ...
  test('test to check shared key decryption - without IV', () async {
    ...
    var encryptedSharedKey = EncryptionUtil.encryptKey(
      aesSharedKey,
      aliceEncryptionPublicKey,
    );
    var publicKeyCheckSum = EncryptionUtil.md5CheckSum(
      aliceEncryptionPublicKey,
    );
    ...
    var decryptedLocation = await sharedKeyDecryption.decrypt(
      sharedKey,
      encryptedLocation,
    );
  ...
  test('test to check shared key decryption with IV', () async {
    ...
    SymmetricKey selfEncryptionKey = AtChopsUtil.generateSymmetricKey(
    ...
    var decryptedValue = await selfKeyDecryption.decrypt(
      selfKey,
      encryptedLocation,
    );
