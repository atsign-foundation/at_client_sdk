...
void main() {
  ...
  Future<void> pumpList(WidgetTester tester) async {
    await tester.pumpWidget(MaterialApp(
        home:
            Scaffold(body: EnrollmentRequestList(enrollmentService: service))));
  ...
  testWidgets('a conveyance refusal shows the truth and clears the row',
    ...
    expect(find.textContaining('Revoke it unless this is understood'),
        findsOneWidget,
        reason: 'the refusal\'s own prose is the truth: approved, cannot '
            'decrypt, consider revoking');
    expect(find.textContaining('Failed to approve'), findsNothing,
        reason: 'the approval did not fail; saying so invites a retry of an '
  ...
  testWidgets('a pq-mode request that wrapped no key is approvable',
      (tester) async {
    when(() => service.approve(any(), any())).thenAnswer((_) async =>
    ...
    final decision = verify(() => service.approve(captureAny(), any()))
        .captured
        .single as EnrollmentRequestDecision;
    expect(decision.encryptedAPKAMSymmetricKey, isEmpty,
        reason: 'the enrollee wrapped no key — the approver mints one, and '
