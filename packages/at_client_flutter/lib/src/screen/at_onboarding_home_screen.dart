...
import 'package:at_client_flutter/src/screen/at_onboarding_generate_screen.dart';
import 'package:at_client_flutter/src/screen/at_onboarding_input_atsign_screen.dart';
...
class AtOnboardingHomeScreen extends ConsumerStatefulWidget {
  ...
  final onboardStatus = AtOnboardingStatus.activate;
  ...
  @override
  ConsumerState<AtOnboardingHomeScreen> createState() =>
      _AtOnboardingHomeScreenState();
}

class _AtOnboardingHomeScreenState
    extends ConsumerState<AtOnboardingHomeScreen> {
  ...
  Future<void> _processAESKey(
    ...
    try {
      ...
      _onboardingService
          .setAtClientPreference(widget.config.atClientPreference);

      Map<String, dynamic> keyMap = jsonDecode(contents);
      keyMap[defaultSelfEncryptionKey] = aesKey;

      AtAuthRequest request = AtAuthRequest(atsign!)
        ..rootDomain = AtRootDomain(
          widget.config.atClientPreference.rootDomain,
          widget.config.atClientPreference.rootPort,
        )
        ..encryptedKeysMap = keyMap;
      authResponse = await _onboardingService.authenticate(request);
      _inprogressDialog.close();
      ...
      } else if (authResponse == AtOnboardingStatus.serverNotReached) {
        await _showAlertDialog(
          AtOnboardingLocalizations.current.msg_atSign_unreachable,
        );
      } else if (authResponse == AtOnboardingStatus.authFailed) {
        await _showAlertDialog(
      ...
      _inprogressDialog.close();
      if (e == AtOnboardingStatus.serverNotReached && _isContinue) {
        await _processAESKey(atsign, aesKey, contents);
      } else if (e == AtOnboardingStatus.authFailed) {
  ...
  Future<dynamic> _processSharedSecret(String atsign, String secret) async {
    ...
    try {
      ...
      } else if (e == AtOnboardingStatus.serverNotReached && _isContinue) {
  ...
  void _showActiveScreen({
    ...
    if ((result ?? '').isNotEmpty) {
      ...
      if (result2 is AtOnboardingStatus) {
        switch (result2) {
          case AtOnboardingStatus.authSuccess:
            if (!mounted) return;
            Navigator.of(context).pop(result2);
            break;
          case AtOnboardingStatus.serverNotReached:
            if (!mounted) return;
  ...
  Future<void> _uploadKeyFile() async {
    try {
      ...
      if (fileContents == null || (aesKey == null && atsign == null)) {
        ...
        return;
      } else if (atsign != _pairingAtsign && _pairingAtsign != null) {
        await showErrorDialog(
