...
AI agent skill for the [Atsign](https://atsign.com) `at_client` SDK. Gives AI coding agents accurate, up-to-date knowledge of `at_client` and `at_client_flutter` — covering `AtCollection<T>`, auth flows, querying, sub-collections, testing patterns, and common pitfalls.

Works with Claude Code, Cursor, GitHub Copilot, Cline, and any agent supporting the [agentskills.io](https://agentskills.io) specification.
...
When the official Dart/Flutter MCP server ships resource support ([flutter.dev/go/packaged-ai-assets](https://flutter.dev/go/packaged-ai-assets)), this package will be updated to expose the skill content as MCP resources — no action needed on your end.
