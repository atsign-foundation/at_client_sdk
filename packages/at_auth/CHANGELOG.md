## 3.3.0
- chore(deps): require `at_chops` ^3.4.1 — the actual minimum this package compiles against: `HashingAlgoType.argon2id` and the hashing algorithm barrel exports used by `AtKeys` passphrase handling landed in at_chops 3.4.1, so this version does not compile against at_chops ≤3.4.0. `at_lookup` stays ^3.6.0; at_auth still authenticates by handing `AtLookUp` an `AtChops`.
