## 3.2.0
- feat: add `AtKeysMaterial` — the only key type `AtKeys`'s API deals in (`addKey`, `getMaterial`, `materialsForKeyId`, `keysForEnrollment`, the `keysList` constructor param, ...). It's fully self-describing: `keyId`/`keyGroup`/`enrollmentId` plus `keyPartType` (`CryptographicKeyType` — the crypto role, PQ-ready: classical/post-quantum/hybrid/stateful-hash variants), `visibility` (`AtKeyVisibility.public`/`.private`/`.symmetric`, used only to derive its `atKey`), `keyAlgorithmType` (`KeyAlgorithmType` — the algorithm family: AES/DES/RSA/ECC/ML-KEM/ML-DSA/SLH-DSA/Falcon/user-defined), `bytes`, `operations`, `createdAt`, and `status` (`active`/`retired`). `AtKeysRecord` (also exported) groups the materials sharing a `keyId` (e.g. the public+private halves of a keypair) purely for the wire's nested shape and cross-material validation — it never appears in `AtKeys`'s own method signatures. Keys produced by one enrollment are grouped by an optional `enrollmentId` and queried via `AtKeys.keysForEnrollment(...)`; at most one material of a given `CryptographicKeyType` may share an `enrollmentId`.
- feat: `AtKeys` gains `AtKeys.fromDocumentJson(...)`/`.toDocumentJson()` for the versioned (v1) document shape (`version`, `atSign`, `keys`, with legacy fields flat at the top level — upgrading a legacy file to v1 is additive, not a format swap), replacing the former codec/resolver/document layer. `AtKeys.materialsForKeyId(keyId)`/`.getMaterial(keyId, type)` replace the old `getKey(id, kind)` lookup.
- feat: `AtKeysAssurance` is now the single home for all atKeys validation — both the low-level `expect*`/`optional*` value/type checks used by `AtKeysMaterial.fromJson`/`AtKeysRecord.fromJson`/`AtKeys.fromDocumentJson` (including verifying a material's wire `atKey` matches its derived `'$visibility:$keyId'`), and the structural invariants (duplicate `keyId`, one material of each `CryptographicKeyType` per enrollment, the append-safety check `validateMapUpdate`).
- feat: add passphrase envelope support using `argon2id`, and add `InMemoryAtKeysIo` for in-memory/test flows (both exported).
- feat: add `FileAtKeysIo.append(...)`, which safety-checks the rewrite (`AtKeysAssurance`) and archives the prior file before overwriting. Appending to a legacy `.atKeys` file upgrades it in place to the v1 document format (legacy fields preserved at the top level).
...
- fix: `AtKeys.toAtChops()` now throws `AtKeyNotFoundException` when `apkamPublicKey` is missing (the exception was previously constructed but never thrown).
- chore(deps): require `at_chops` ^3.4.1 for hashing algorithm barrel exports used by AtKeys passphrase handling.
- note: the passphrase crypto helper is named `AtKeysPassphraseCrypto` to avoid colliding with at_chops' exported `AtKeysCrypto`.
