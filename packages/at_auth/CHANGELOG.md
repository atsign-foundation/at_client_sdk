## 3.2.0
- feat: add `AtKeysMaterial` — the only key type `AtKeys`'s API deals in (`addKey`, `getMaterial`, `materialsForKeyId`, `keysForEnrollment`, the `keysList` constructor param, ...). It's fully self-describing: `keyId`/`keyGroup`/`enrollmentId` plus `keyPartType` (`CryptographicKeyType` — the crypto role, PQ-ready: classical/post-quantum/hybrid/stateful-hash variants), `keyAlgorithmType` (`KeyAlgorithmType` — the algorithm family: AES/DES/RSA/ECC/ML-KEM/ML-DSA/SLH-DSA/Falcon/user-defined), `bytes`, `operations`, `createdAt`, and `status` (`active`/`retired`). The wire's nested `keys[].keyParts[]` document shape — grouping the materials sharing a `keyId` (e.g. the public+private halves of a keypair) — is produced/consumed by `encodeAtKeysDocument`/`parseAtKeysDocument` (also exported), not a separate model type. Keys produced by one enrollment are grouped by an optional `enrollmentId` and queried via `AtKeys.keysForEnrollment(...)`; at most one material of a given `CryptographicKeyType` may share an `enrollmentId`.
- feat: `AtKeys.toJson()`/`.fromJson(...)` now produce/consume the versioned (v1) document shape (`version`, `atSign`, `keys`, with legacy fields flat at the top level — upgrading a legacy file to v1 is additive, not a format swap), replacing the former codec/resolver/document layer. Backward compatible: `fromJson` accepts json without a `version` field as the pre-v1 flat shape (delegating to the new `AtKeys.fromLegacyJson(...)`), and throws `AtKeysUnsupportedVersionException` on an unknown version. `AtKeys.materialsForKeyId(keyId)`/`.getMaterial(keyId, type)` replace the old `getKey(id, kind)` lookup.
- note: `toJson()` output for an `AtKeys` with an atSign gains `version`/`atSign`/`keys` at the top level (the legacy fields stay flat beside them, so pre-v1 consumers keep working); the exact pre-v1 output shape remains available as `AtKeys.toLegacyJson()`.
- feat: add `WrittenAtKeysIo.flush(Atsign, AtKeys)` — the runtime persist operation: mutate the in-memory `AtKeys` (`addKey`, ...), then flush the complete state. On an existing file, flush safety-checks the rewrite (`AtKeysAssurance.validateMapUpdate` — nothing may be lost), then rewrites; flushing a legacy `.atKeys` file upgrades it in place to the v1 document format (legacy fields preserved byte-for-byte). On a missing file, flush creates it. `write(...)` stays the create-only initial persist. (The `append`/`save` methods that existed briefly during this release's development are gone — never published.)
- feat: at rest, a v1 document self-encrypts its legacy RSA fields exactly like a legacy-only file does (same key, same deterministic IV) — the legacy portion of a v1 file is byte-identical to its legacy-only equivalent, so pre-v1 readers can still consume it. In memory, `AtKeys` always holds plaintext; `FileAtKeysIo` applies/peels all at-rest layers. See the README's "The `.atKeys` file format" section.
...
- fix: preserve legacy `.atKeys` read/write behavior, including legacy self-encrypted key files and generated keys after write/read. Two intentional deltas: reading a legacy file now also populates `AtKeys.metadata` (non-schema entries such as the `"<atsign>": "<selfEncryptionKey>"` trailer), and a legacy file without an `apkamSymmetricKey` now yields `null` instead of empty bytes (which previously mis-routed `toAtChops()` into the APKAM path).
- fix: `AtKeys.toAtChops()` now throws `AtException` subtypes instead of null-dereferencing when any required key is missing (`apkamPublicKey`, `apkamPrivateKey`, `defaultEncryptionPublicKey`, `defaultEncryptionPrivateKey`, `defaultSelfEncryptionKey`, depending on APKAM/PKAM mode).
- fix: `AtKeys` equality/hashCode no longer depend on the order key materials were added in.
