...
void main() {
  group('Functional tests for at_activate --yes', () {
    final tempDir =
        Directory.systemTemp.createTempSync('at_activate_functional_test');
    ...
    test('Process-based test: CLI exits with code 0 on N response', () async {
      final result = await Process.run(
        ...
        [
          '-c',
          'echo "N" | dart bin/activate_cli.dart onboard --atsign @8incanteater --keys $testKeysPath'
        ],
    ...
    test('Process-based test: CLI accepts Y response and continues', () async {
      final result = await Process.run(
        ...
        [
          '-c',
          'timeout 5s bash -c "echo Y | dart bin/activate_cli.dart onboard --atsign @8incanteater --keys $testKeysPath"'
        ],
        workingDirectory: Directory.current.path,
      );
    ...
    test('CLI with --cramkey bypasses prompt (implies --yes)', () async {
      final result = await Process.run(
        ...
        [
          '-c',
          'timeout 5s dart bin/activate_cli.dart onboard --atsign @8incanteater --keys $testKeysPath --cramkey dummy-key'
        ],
        workingDirectory: Directory.current.path,
      );
    ...
    test('CLI with --cramkey and --yes behaves the same as without --yes',
        () async {
      final result = await Process.run(
        'bash',
        [
          '-c',
          'timeout 5s dart bin/activate_cli.dart onboard --atsign @8incanteater --keys $testKeysPath --cramkey dummy-key --yes'
        ],
        workingDirectory: Directory.current.path,
      );
    ...
    test('CLI with --yes bypasses prompt and continues', () async {
      final result = await Process.run(
        ...
        [
          '-c',
          'timeout 5s dart bin/activate_cli.dart onboard --atsign @8incanteater --keys $testKeysPath --yes'
        ],
        workingDirectory: Directory.current.path,
      );
