...
/// `AtOnboardingPreference` carries the flags its superclass fixes at
/// construction.
void main() {
  group('AtOnboardingPreference forwards the construction-final flags', () {
    ...
    test('and each axis can be named against the posture that implies it', () {
      // NOTE: a signing set weaker than the posture is allowed — pqActive with
      // {rsa2048} mints rsa2048 and keeps `_apsk` bare. It is the
      // AUTHENTICATION axis that may not be lowered.
      final preference = AtOnboardingPreference(
    ...
    test('and the superclass\'s coherence rules see the forwarded axis', () {
      // NOTE: the refusal comes from AtClientPreference, so a subclass that
      // accepted the argument and dropped it would construct happily.
      expect(
    ...
    test('the mutable fields are untouched by the new constructor', () {
      // NOTE: the constructor forwards only what is final; shadowing an
      // assignable field would break every consumer that sets it after the
      // fact.
      final preference = AtOnboardingPreference()
        ..namespace = 'my_app'
